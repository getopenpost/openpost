...
>
		...
		<div class="border-t border-white/10 bg-[#17171a]">
			...
			>
				...
				<Button
					...
					>{m.video_studio_quick_mark_in()}</Button
				>
				<Button
					variant="ghost"
					size="sm"
					...
					>{m.video_studio_quick_mark_out()}</Button
				>
				<Button variant="ghost" size="sm" onclick={onSplit} disabled={!activeClip}
					><ScissorsIcon class="size-4" />{m.video_studio_quick_split()}</Button
				>
				<Button variant="ghost" size="sm" onclick={onDelete} disabled={!activeClip}
					><TrashIcon class="size-4" />{m.video_studio_quick_remove()}</Button
				>
			</div>

			<div class="space-y-3 p-3 sm:p-4" aria-label={m.video_studio_quick_source_timeline()}>
				<div class="flex items-center justify-between gap-3">
					<div>
						<h2 class="text-xs font-semibold text-zinc-200">
							{m.video_studio_quick_source_timeline()}
						</h2>
						<p class="mt-0.5 text-[11px] text-zinc-500">
							{m.video_studio_quick_source_time({ time: formatTime(sourcePlayheadUS) })}
						</p>
					</div>
					{#if activeClip && keyframesUS.length}
						<Button variant="ghost" size="xs" onclick={snapInPoint} disabled={keyframeSafe}>
							<SparklesIcon class="size-3.5" />{m.video_studio_quick_snap_keyframe()}
						</Button>
					{/if}
				</div>
				<Slider
				...
				<button
					...
					aria-label={m.video_studio_quick_source_timeline()}
				>
					{#if thumbnailURL}
						<div
	...
	<aside
		...
		aria-label={m.video_studio_quick_kept_sections()}
	>
		<div class="flex items-center gap-2">
			<FilmIcon class="size-4 text-orange-400" />
			<h2 class="font-semibold">{m.video_studio_quick_kept_sections()}</h2>
		</div>
		...
		<div class="mt-5 border-t border-white/10 pt-4">
			...
			{#if compatibility.compatible}
				<p class="flex gap-2 text-xs leading-5 text-zinc-400">
					...
					/>{m.video_studio_quick_lossless_ready()}
				</p>
				{#if keyframesUS.length}
					...
					>
						{allKeyframesSafe
							? m.video_studio_quick_keyframe_ready()
							: m.video_studio_quick_keyframe_adjust()}
					</p>
				{/if}
			{:else}
				<p class="text-xs leading-5 text-amber-300">{m.video_studio_quick_lossless_blocked()}</p>
			{/if}
			{#if fastExportBusy}
				<div class="mt-4" aria-live="polite">
					...
					<p class="mt-2 text-xs text-zinc-400">
						{m.video_studio_quick_exporting()}
