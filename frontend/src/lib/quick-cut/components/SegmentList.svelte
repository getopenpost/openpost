<script lang="ts">
	...
	function commitTime(id: string, field: 'start' | 'end', value: string) {
...
<ul class="flex flex-col gap-2" role="list" aria-label={m.quick_cut_segments_label()}>
	{#each segments as seg, index (seg.id)}
		...
		>
			<button
				type="button"
				class="flex min-h-11 min-w-11 items-center justify-center rounded-md border border-transparent bg-muted px-2 py-2 text-xs font-mono tabular-nums hover:bg-accent sm:min-h-9 {selectedId ===
				seg.id
			...
			<div class="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center sm:gap-2">
				<label class="flex flex-1 flex-col gap-1 text-xs">
					<span class="text-muted-foreground">{m.quick_cut_in()} · {formatTimecode(seg.start)}</span>
					<input
						class="h-11 min-h-11 rounded-md border border-input bg-background px-2 text-sm font-mono tabular-nums focus-visible:outline-2 focus-visible:outline-primary md:h-9 md:min-h-9"
						type="text"
						inputmode="decimal"
						value={formatTimecode(seg.start)}
						aria-label={`${m.quick_cut_in()} ${index + 1}`}
						onchange={(e) => commitTime(seg.id, 'start', e.currentTarget.value)}
					/>
				</label>
				<span class="hidden self-center text-muted-foreground sm:block">→</span>
				<label class="flex flex-1 flex-col gap-1 text-xs">
					<span class="text-muted-foreground">{m.quick_cut_out()} · {formatTimecode(seg.end)}</span>
					<input
						class="h-11 min-h-11 rounded-md border border-input bg-background px-2 text-sm font-mono tabular-nums focus-visible:outline-2 focus-visible:outline-primary md:h-9 md:min-h-9"
						type="text"
						inputmode="decimal"
						...
						onchange={(e) => commitTime(seg.id, 'end', e.currentTarget.value)}
					/>
				</label>
				<span
					class="rounded bg-muted px-2 py-1 text-center font-mono text-xs tabular-nums text-muted-foreground"
