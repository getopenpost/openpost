...
| `OPENPOST_POSTHOG_BROWSER_HOST`        | Required in cloud mode                | falls back to the API host                  | Browser ingestion endpoint or first-party reverse proxy. The managed service requires an explicit browser host.                                                                                                                    |
| `OPENPOST_POSTHOG_UI_HOST`             | Required when telemetry is enabled    | empty                                       | Real PostHog application host, such as `https://eu.posthog.com`, used separately from an ingestion proxy.                                                                                                                          |
