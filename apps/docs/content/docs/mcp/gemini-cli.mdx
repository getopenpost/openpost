...
Run `/mcp auth openpost` again for an expired login. The browser callback must reach the machine running Gemini CLI. For a remote shell or private instance, consider the [local bridge](/mcp/mcp-guide/self-hosted-and-local).
