...
test("Studio creates from an original template, adapts to mobile, and exports to Media", async ({
  ...
  await curveSelect.click();
  await page.getByRole("option", { name: "Ellipse" }).click();
  await expect(
    page.getByRole("spinbutton", { name: "H", exact: true }),
  ).toHaveValue("487");
  await curveSelect.click();
  ...
  await page.getByRole("combobox", { name: "Mask" }).click();
  await page.getByRole("option", { name: "Diamond" }).click();
  await page.getByRole("button", { name: "Add border" }).click();
  await page.getByRole("combobox", { name: "Border position" }).click();
  ...
  await page.reload();
  await page.getByRole("treeitem", { name: /Accent, shape/ }).click();
  await expect(page.getByRole("combobox", { name: "Mask" })).toHaveText(
    "Diamond",
  );
  await expect(page.getByRole("combobox", { name: "Blend mode" })).toHaveText(
    "Multiply",
  );
  await expect(
  ...
  await page.getByRole("treeitem", { name: /A clear update, text/ }).click();
  await expect(page.getByRole("combobox", { name: "Text curve" })).toHaveText(
    "Wave",
  );
  await expect(
    page.getByRole("button", { name: "Remove drop shadow" }),
