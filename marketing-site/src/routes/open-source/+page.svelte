<script lang="ts">
  ...
  const deploymentFacts = [
    ...
    },
    {
      ...
      detail:
        "Store media on the local filesystem or a configured object-storage backend.",
      icon: Server,
    },
  ] as const;
    ...
    <div class="grid gap-12 lg:grid-cols-[1fr_22rem] lg:items-end">
      <div class="max-w-4xl">
        ...
        <p class="marketing-copy mt-6">
          Use the managed app or deploy the AGPL-licensed server yourself. Both
          use the same posting tools, account settings, and status tracking.
        </p>
      ...
      <div class="border-l pl-6">
        ...
        <p class="mt-2 text-sm leading-6 text-muted-foreground">
          You may use, inspect, and modify the software under the licence terms.
          Network users of a modified service must be able to receive its
  ...
  <div class="marketing-shell">
    ...
    <div class="mt-10 grid border-y lg:grid-cols-2 lg:divide-x">
      {#each choices as choice (choice.name)}
        <article
          class="flex h-full flex-col py-8 lg:px-8 lg:first:pl-0 lg:last:pr-0"
        >
    ...
    <div class="grid gap-10 lg:grid-cols-[0.65fr_1.35fr]">
      <div>
        ...
        <p class="mt-4 text-sm leading-6 text-muted-foreground">
          The default setup is small. You still need safe settings, monitoring,
          backups, and social network access.
        </p>
      </div>
      <div
        class="grid divide-y border-y sm:grid-cols-2 sm:divide-x sm:divide-y-0"
      >
  ...
  <div class="marketing-shell grid gap-12 lg:grid-cols-2">
    <div>
      ...
      <ol class="mt-8 space-y-5">
        ...
        <li class="grid grid-cols-[2rem_1fr] gap-4">
          ...
          <p class="text-sm leading-6 text-muted-foreground">
            Generate strong JWT and encryption keys and keep them outside the
        ...
        <li class="grid grid-cols-[2rem_1fr] gap-4">
          ...
          <p class="text-sm leading-6 text-muted-foreground">
            Choose SQLite or PostgreSQL and local or object media storage for
        ...
        <li class="grid grid-cols-[2rem_1fr] gap-4">
          ...
          <p class="text-sm leading-6 text-muted-foreground">
            Back up the database, media, and required secrets together, then
    ...
    <div class="border-t pt-8">
      ...
      <h2 class="mt-5 text-2xl font-semibold">
        Contribute from the same project environment
      </h2>
      <p class="mt-3 text-sm leading-6 text-muted-foreground">
        The repository includes Devenv commands for installation, setup, checks,
        tests, builds, and release verification. Issues and pull requests are
        public.
      </p>
      <div class="mt-6 flex flex-wrap gap-3">
        <Button href={developerDocsUrl} variant="outline"
          >Development guide</Button
        >
