...
<section class="section-pad">
	<article class="reading-shell prose dark:prose-invert">
		...
		<aside class="not-prose my-10 rounded-xl border bg-muted/30 p-6 sm:p-8">
			...
			<p class="mt-5 text-sm leading-6 text-muted-foreground">
				...
				> set the contract for the hosted service. The full sections below state the privacy rules.
			</p>
		</aside>
		...
		<ul>
			...
			<li>
				<strong>Post data:</strong> drafts, scheduled and published content, media, account versions,
				OpenPost Image Editor designs and saved versions, explicitly cloud-saved OpenPost Video Editor
				projects, templates, brand settings, prompts, schedules, post status, and job history.
			</li>
			<li>
				<strong>Analytics data:</strong> account and content numbers for posts published through OpenPost,
				past results, update status, and safe platform error codes. This lets you inspect measured evidence
				and prepare a repurpose draft for review. Analytics records do not contain raw platform replies,
				remote media, access tokens, bot tokens, webhook secrets, or direct messages.
			</li>
			<li>
			...
			<li>
				<strong>Connected-account data:</strong> social network, account ID, username, profile, supported
				features, X plan when relevant, granted access, and encrypted access or refresh tokens. OpenPost
			...
			<li>
				...
				TOTP settings, API token details, IP address, browser details, sign-in and token activity, and
				password-reset records. OpenPost never stores a readable account password.
			</li>
			<li>
				<strong>Billing data:</strong> plan access, subscription, checkout, customer, transaction, and
				billing-status identifiers received from Paddle or the payment provider identified at checkout.
				OpenPost does not receive or store full payment-card details.
			</li>
			<li>
				<strong>Support and service data:</strong> messages you send, service logs, errors, request timing,
				usage counters, and abuse-prevention signals.
			</li>
		</ul>

		<h2>3. How we receive data</h2>
		<p>
			We receive data from you when you register, set up a workspace, upload content, connect a
			social network, buy a plan, or contact support. We also receive account and post data from the
			networks you connect, billing status from Paddle or the payment provider identified at
			checkout, and limited technical data when your browser or client uses the service.
		</p>
		<p>
			Camera access is requested only after you choose a camera action. A live preview stays in the
			browser and is not saved. A still photo is uploaded to the selected workspace only after you
			choose to use it. OpenPost Video Editor can optionally record a camera track; that recording
			and its local project stay in browser storage until you choose a cloud save, Media save, or
			post handoff.
		</p>
		...
		<ul>
			<li>
				<strong>Social networks you choose</strong>, such as X, Mastodon servers, Bluesky, LinkedIn,
				Meta services, TikTok, or YouTube, to connect accounts and perform requested publishing
				actions. Their terms and privacy policies then apply.
			</li>
			<li>
				<strong>An identity provider you choose</strong>, such as Google, when you use that provider
				to sign in or link an identity.
			</li>
			<li>
				<strong>Hosting companies</strong> that run the app, database, media, email, network, analytics,
				and backups under their service terms.
			</li>
			<li>
				<strong>OpenRouter and selected AI model providers</strong>, when automatic image captioning
				is enabled, to process a 400-pixel JPEG thumbnail and up to 1,000 characters of the relevant
				draft or thread segment as untrusted context. OpenPost does not send the original image for
				this feature. Their service and privacy terms apply.
			</li>
			<li>
				<strong>Paddle and prior payment providers</strong> for checkout, subscriptions, tax, billing
				records, refunds, fraud prevention, and customer portals. Paddle acts as Merchant of Record for
				orders it processes.
			</li>
			<li>
				<strong>Stock-media services you search</strong> to return requested photo or video results and
				record provider-required selection or download events.
			</li>
			<li>
				<strong>The disclosed feedback recipient</strong> when you choose to send the in-app report, including
				its text, optional screenshot, and the diagnostics shown before submission.
			</li>
			<li>
		...
		<p>
			OpenPost does not sell personal data, build advertising profiles, or share personal data for
		...
		<p>
			...
			credit decisions, or unrelated profiling.
		</p>
		<p>
			OpenPost uses YouTube API Services. Google's handling of information is described in the
		...
		<p>
			...
			snapshots follow a separate review and are kept only while needed for recovery, security,
			legal duties, fraud prevention, or enforcing legal rights. OpenPost does not add a separate
			...
			<table class="min-w-[76rem] border-collapse text-left text-sm">
				...
				<thead class="bg-muted/50">
					<tr>
						...
						<th class="w-72 border-b p-4 font-semibold" scope="col">Owner and purpose</th>
						<th class="w-80 border-b p-4 font-semibold" scope="col">Duration</th>
						<th class="w-96 border-b p-4 font-semibold" scope="col">Deletion trigger and limits</th>
					</tr>
				</thead>
					...
					{#each privacyInventory.managed_retention as entry (entry.id)}
						<tr class="align-top">
							<th class="p-4 font-semibold" scope="row">
								{entry.category}
								<span class="mt-2 block text-xs leading-5 font-normal text-muted-foreground"
									>{entry.includes}</span
								>
							</th>
							<td class="p-4 leading-6">
								<span class="font-medium">{entry.owner}</span>
								<span class="mt-2 block text-muted-foreground">{entry.purpose}</span>
							</td>
							<td class="p-4 leading-6 text-muted-foreground">
								{entry.duration}
							</td>
							<td class="p-4 leading-6">
								{entry.deletion_trigger}
								<span class="mt-2 block text-muted-foreground"
									><strong class="font-medium text-foreground">Limits:</strong>
		...
		<p>
			The inventory below covers first-party cookies, localStorage, sessionStorage, IndexedDB
			databases and object stores, Origin Private File System (OPFS) roots and patterns, and Cache
			Storage names used by the app, marketing site, and documentation site. An exact identifier is
			one fixed name. A prefix covers names with a generated suffix. The inventory was reviewed on {inventoryReviewedOn}.
		</p>
		<p>
			OpenPost Image Editor background removal runs in your browser with model and runtime files
			served by the OpenPost operator. Source pixels are not sent to a background-removal service. A
		...
		<p>
			...
			transcription, voice detection, framing analysis, and export run in the browser. Model files
			are downloaded only after the user starts the related tool. Local project data, recordings,
			sources, and exports are uploaded only when the user chooses a cloud save, Media save, or post
			handoff.
		</p>
		...
		{#each storageTechnologies as technology (technology)}
			...
			>
				<table class="min-w-[70rem] border-collapse text-left text-sm">
					...
					<thead class="bg-muted/50">
						<tr>
							<th class="w-64 border-b p-4 font-semibold" scope="col">Identifier</th>
							<th class="w-80 border-b p-4 font-semibold" scope="col">Owner and purpose</th>
							<th class="w-96 border-b p-4 font-semibold" scope="col">Scope and duration</th>
							<th class="w-40 border-b p-4 font-semibold" scope="col">Necessity</th>
						</tr>
					</thead>
					<tbody class="divide-y">
						{#each entries as entry (entry.id)}
							<tr class="align-top">
								...
								<td class="p-4 leading-6">
									{entry.scope}
									<span class="mt-2 block text-muted-foreground"
										><strong class="font-medium text-foreground">Duration:</strong>
										{entry.duration}</span
									>
								</td>
								<td class="p-4 leading-6">
		...
		<p>
			Depending on where you live, you may have rights to access, correct, export, delete, restrict,
			or object to processing, and to withdraw consent. Use account settings for export and deletion
			or email <a href="mailto:openpost@rgo.pt">openpost@rgo.pt</a>. We may need to verify your
			identity before completing a request.
		</p>
		<p>
			You may also complain to your local data-protection authority. In Portugal, this is the
			<a href="https://www.cnpd.pt/">Comissão Nacional de Proteção de Dados (CNPD)</a>.
		</p>

		<h2>11. Security</h2>
		<p>
			OpenPost uses encrypted social account tokens, password hashing, secure session cookies,
			limited tool tokens that you can remove, access controls, TOTP, passkeys, request limits,
			backups, and checks for unsafe software packages. No internet service can guarantee full
			security. Report a suspected security flaw to
			<a href="mailto:openpost+security@rgo.pt">openpost+security@rgo.pt</a>.
		</p>

		<h2>12. Children</h2>
		<p>
			The hosted service is not directed to anyone under 18. Do not create an account if you are
			under 18.
		</p>

		<h2>13. Changes and contact</h2>
		<p>
			We may update this policy when the service or legal requirements change. Material changes will
			be announced in the service or by email when appropriate, and the policy version and effective
			date will change. Corrections to spelling, formatting, or links that do not change meaning
			...
			{#each legalChangeHistory.entries as entry (`${entry.document}:${entry.version}`)}
				<section class="py-6">
					<h3 class="text-base font-semibold">
						<a
							class="focus-ring rounded-sm text-primary underline underline-offset-4"
							href={resolve(policyPath(entry.document))}>{policyName(entry.document)}</a
						>
						<span class="font-normal text-muted-foreground">version {entry.version}</span>
					</h3>
