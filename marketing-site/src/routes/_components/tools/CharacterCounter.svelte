...
<div class="mt-8 grid gap-5 lg:grid-cols-[minmax(0,1.08fr)_minmax(22rem,0.92fr)]">
	<section class="rounded-lg border bg-card p-4 sm:p-6" aria-labelledby="counter-input-title">
		...
		<textarea
			...
			bind:value={draft}
			class="mt-5 min-h-72 w-full resize-y rounded-xl border bg-background p-4 text-base leading-7 focus-visible:ring-2 focus-visible:ring-ring"
		...
		<dl class="mt-4 grid grid-cols-3 divide-x rounded-xl border bg-muted/20 py-3 text-center">
			<div class="px-2">
				...
				<dd class="mt-1 font-mono text-lg font-semibold">{visibleCharacters.toLocaleString()}</dd>
			</div>
			<div class="px-2">
				<dt class="text-xs text-muted-foreground">Words</dt>
				<dd class="mt-1 font-mono text-lg font-semibold">{words.toLocaleString()}</dd>
			</div>
			<div class="px-2">
				<dt class="text-xs text-muted-foreground">Lines</dt>
				<dd class="mt-1 font-mono text-lg font-semibold">{lines.toLocaleString()}</dd>
			</div>
		</dl>
	</section>
	...
	<section class="rounded-lg border bg-card p-4 sm:p-6" aria-labelledby="platform-counts-title">
		<div>
			<h2 id="platform-counts-title" class="text-lg font-semibold">Platform limits</h2>
			<p class="mt-1 text-sm text-muted-foreground">Compare one draft before adapting it per platform.</p>
		</div>
