...
// Ported from FreeCut (MIT) and the OpenMOSS browser ONNX reference runtime.
// MOSS runs only on CPU, so use the matching CPU-only ORT build instead of
// making Vite emit the 26 MB WebGPU/JSEP binary used by other editor models.
const ORT_MODULE_URL = "/moss-tts/vendor/ort/ort.wasm.min.mjs";
const ort = await import(/* @vite-ignore */ ORT_MODULE_URL);
...
async function postTokenizerSandboxRequest(action, data = {}) {
    const bridge = globalThis.__openpostMossTokenizerBridge;
...
var BrowserOnnxTtsRuntime = class {
  ...
  async ensureOrtConfigured() {
    ...
    ort.env.wasm.wasmPaths = new URL("/moss-tts/vendor/ort/", globalThis.location.origin).href;
