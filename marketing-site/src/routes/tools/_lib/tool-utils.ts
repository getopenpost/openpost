...
export const COUNTER_PLATFORMS: PlatformCountDefinition[] = [
	...
	},
	{
		...
		name: 'Bluesky',
		limit: 300,
		note: 'Counted as visible Unicode graphemes.',
		countMode: 'graphemes'
	},
	{
		key: 'mastodon',
	...
	},
	{
		...
		name: 'TikTok',
		limit: 2_200,
		note: 'Conservative video-caption limit; photo-post support can differ.',
		countMode: 'graphemes'
	},
	{
		key: 'youtube',
	...
	},
	{
		...
		name: 'Facebook Pages',
		limit: 63_206,
		note: 'Technical post limit; shorter copy is usually easier to scan.',
		countMode: 'graphemes'
	}
];
...
function splitUnit(value: string, limit: number, platform: PlatformKey): string[] {
	const output: string[] = [];
	let current = '';
...
function splitWithoutNumbering(value: string, limit: number, platform: PlatformKey): string[] {
	...
	for (const paragraph of paragraphs) {
		...
		for (const sentence of sentenceSegments(paragraph)) {
			...
			const sentenceCandidate = current ? `${current} ${cleanSentence}` : cleanSentence;
...
export function splitSmartThread(
	...
	for (let attempt = 0; attempt < 6; attempt += 1) {
		...
		const contentLimit = Math.max(20, limit - platformTextCount(suffix, platform));
...
const blueskyHandlePattern = /^([a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?$/u;

function validHostname(host: string): boolean {
	if (host.length > 253 || !host.includes('.') || !blueskyHandlePattern.test(host)) return false;
	try {
...
export function parseSocialHandle(value: string): ParsedHandle {
	...
	if (separator > 0) {
		...
		if (/^[a-zA-Z0-9_]+$/u.test(username) && validHostname(host)) {
			...
			return {
				...
				message: 'The syntax is valid. A live check can confirm that the server resolves it.'
			};
		}
	}
	...
	if (candidate.length <= 253 && blueskyHandlePattern.test(candidate)) {
		return {
			...
			message: 'The DNS-style syntax is valid. A live check can resolve it to a DID.'
		};
	}
...
export function formatLinkedInText(
	...
	for (const paragraph of originalParagraphs) {
		...
		const sentences = sentenceSegments(paragraph).map((sentence) => sentence.trim()).filter(Boolean);
		for (let index = 0; index < sentences.length; index += options.sentencesPerParagraph) {
			output.push(sentences.slice(index, index + options.sentencesPerParagraph).join(' '));
		}
	}
...
function zonedWallTimeToInstant(date: Date, time: string, timeZone: string): Date {
	const [hour, minute] = time.split(':').map(Number);
	let candidate = Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), hour, minute);
...
export function buildPostingPlan(input: {
		...
		input.postsPerWeek <= days.length
			? input.postsPerWeek === 1
				...
				: Array.from(
						...
						(_, index) => days[Math.round((index * (days.length - 1)) / (input.postsPerWeek - 1))]
					)
			: Array.from({ length: input.postsPerWeek }, (_, index) => days[index % days.length]);
	...
	for (const day of days) {
		...
		for (let position = 0; position < count; position += 1) {
			const minutes = Math.round(start + ((end - start) * (position + 1)) / (count + 1));
			const requestedAudienceTime = timeFromMinutes(minutes);
			const instant = zonedWallTimeToInstant(date, requestedAudienceTime, input.audienceTimezone);
			...
			const audienceTime = timeFromMinutes(actualAudience.hour * 60 + actualAudience.minute);
			...
			output.push({
				...
				adjustedForTimezone,
				requestedAudienceTime: adjustedForTimezone ? requestedAudienceTime : undefined
			});
		}
	}
...
export function postingPlanCsv(slots: PostingSlot[], audienceTimezone: string, localTimezone: string) {
	const rows = [
		[
			'Day',
