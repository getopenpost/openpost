...
- Release candidates prove themselves on their own tag: tag CI forces the full core gates (backend, frontend, security, image, browser suites) while distributions follow the tag diff. Main CI is development feedback, never release proof. Never infer coverage from a green path-filtered or canceled main run.
