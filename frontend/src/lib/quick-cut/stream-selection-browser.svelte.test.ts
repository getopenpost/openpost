...
async function createAvMp4(): Promise<File> {
	...
	const audio = new (await import('mediabunny')).AudioSampleSource({ codec: 'aac', bitrate: 64_000 });
	...
	const sample = new AudioSample({ data: pcm, format: 'f32', numberOfChannels: 1, sampleRate: sr, timestamp: 0 });
...
describe('quick-cut stream selection browser', () => {
	it('video-off selection produces audio-only output', async () => {
		...
		const [art] = await exportSegments({ sources: [src], segments: [seg], cutMode: 'nearestKeyframe', merge: false });
