...
function parseInput(url: URL): OgInput {
  const entry = resolveSocialImageEntry(
...
function ActivityGrid({
  ...
  return (
    <div
      style={{ display: "flex", flexWrap: "wrap", width: columns * 29, gap: 8 }}
    >
  ...
  return (
    ...
    >
      ...
      >
        {platform ? (
          <img
            src={`${origin}/assets/logos/${platform}.svg`}
            width="76"
            height="76"
          />
        ) : null}
...
function Motif({ input, origin }: { input: OgInput; origin: string }) {
  ...
  if (input.kind === "comparison")
    return <ComparisonMotif subject={input.subject} />;
