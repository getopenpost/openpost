...
describe('PlatformPreview platform views', () => {
	...
	it('renders a complete destination page without OpenPost application chrome', async () => {
		...
		await expect.element(screen.getByText('What’s happening?')).toBeVisible();
	...
	it.each([
		...
		async (platform, format, chromeText) => {
			...
			await expect.element(screen.getByText(chromeText, { exact: true }).first()).toBeVisible();
			await expect.element(screen.getByText(`Authored ${platform} post.`)).toBeVisible();
		}
	);
