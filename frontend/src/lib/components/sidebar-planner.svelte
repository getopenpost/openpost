<script lang="ts">
	...
	const keyboardFocusDayKey = $derived(focusedDayKey || plannerToday.toString());
	$effect(() => {
	...
	async function moveCalendarFocus(event: KeyboardEvent, date: CalendarDate) {
		...
		grid
			?.querySelector<HTMLButtonElement>(`button[data-calendar-date="${target.key}"]`)
			?.focus();
	}
...
<div class="flex min-h-0 flex-1 flex-col" data-testid="desktop-sidebar-planner">
	<section class="shrink-0 border-b border-sidebar-border px-2 pb-3">
		<div
			class="h-52 overflow-y-auto overscroll-contain pt-1 select-none"
