...
class EntryAutomation {
    ...
    this.spans = entry.transitionGainSpans
      ...
      .map((span) => ({
        ...
        endSample: Math.round(
          (span.startSeconds + span.durationSeconds) * MIX_SAMPLE_RATE,
        ),
        isIncoming: span.isIncoming,
        dipToSilence: span.dipToSilence,
      }))
      .sort((left, right) => left.startSample - right.startSample);
    diagnostics?.onAutomationPrepared?.(
      this.gainPoints.length,
      this.spans.length,
    );
  }
  ...
  gainAt(sample: number): number {
    ...
    if (this.gainPoints.length > 0) {
      ...
      if (sample <= this.gainPoints[0]!.sample)
        gain = this.gainPoints[0]!.value;
        ...
        gain =
          duration <= 0
            ? right.value
            : left.value +
              ((right.value - left.value) * (sample - left.sample)) / duration;
      }
    }
    while (
      this.spanIndex < this.spans.length &&
      this.spans[this.spanIndex]!.endSample <= sample
    ) {
      this.spanIndex++;
    }
    for (let index = this.spanIndex; index < this.spans.length; index++) {
      ...
      const progress =
        duration <= 1 ? 1 : (sample - span.startSample) / (duration - 1);
      gain *= transitionGainAtProgress(
        progress,
        span.isIncoming,
        span.dipToSilence,
      );
...
async function* streamEntryAudio(
  ...
  const targetFrames = Math.max(
    0,
    Math.ceil(entry.durationSeconds * MIX_SAMPLE_RATE),
  );
  const sourceDuration =
    entry.durationSeconds * entry.playbackRate + SOURCE_GUARD_SECONDS;
  ...
  const sourceWindowSeconds =
    SOURCE_WINDOW_SECONDS * Math.min(1, entry.playbackRate);
  ...
  while (emittedFrames < targetFrames) {
    ...
    const chunkStart = entry.reversed
      ...
      : cursor;
    const chunkEnd = entry.reversed
      ? cursor
      : Math.min(sourceEnd, cursor + sourceWindowSeconds);
    ...
    const sourceFinished = entry.reversed
      ? cursor <= sourceStart
      : cursor >= sourceEnd;
    ...
    if (sampleRate === 0) {
      ...
      effectChain = new StreamingAudioEffectChain(
        entry.audioEffects,
        sampleRate,
        channelCount,
      );
    ...
    if (effectChain && !effectChain.isEmpty())
      channels = effectChain.process(channels);
    if (resamplers) {
  ...
  if (
    ...
    while (remainingMix > 0) {
      ...
      const countSrc = Math.max(
        1,
        Math.ceil((countMix * sampleRate) / MIX_SAMPLE_RATE),
      );
      ...
      if (mapped[0]!.length > remainingMix)
        mapped = mapped.map((c) => c.slice(0, remainingMix));
      yield mapped;
...
class EntryAudioReader {
  ...
  private async read(frames: number, copy: boolean): Promise<Float32Array[]> {
    while (this.availableFrames < frames) {
      ...
      if (next.done)
        throw new Error(
          "A timeline clip ended before its planned audio duration.",
        );
    ...
    while (remaining > 0) {
      ...
      if (copy) {
        for (let channel = 0; channel < MIX_CHANNELS; channel++) {
          output[channel]!.set(
            chunk.channels[channel]!.subarray(
              chunk.frameOffset,
...
export async function* mixAudioWindows(
  ...
  const maxTailSeconds = entries.reduce(
    (max, e) => Math.max(max, getAudioEffectTailSeconds(e.audioEffects)),
    0,
  );
  const totalSamples = Math.ceil(
    (durationSeconds + maxTailSeconds) * MIX_SAMPLE_RATE,
  );
  const prepared: PreparedEntry[] = entries.map((entry) => {
    ...
    const tail = Math.ceil(
      getAudioEffectTailSeconds(entry.audioEffects) * MIX_SAMPLE_RATE,
    );
    return {
      entry,
      startSample,
      endSample:
        startSample + Math.ceil(entry.durationSeconds * MIX_SAMPLE_RATE) + tail,
  ...
  try {
    for (
      ...
      windowStart += MIX_WINDOW_SAMPLES
    ) {
      throwIfAborted(signal);
      ...
      for (const current of prepared) {
        ...
        if (current.reader.position !== entryOffset) {
          throw new Error(
            "Audio mix state advanced past its timeline position.",
          );
        }
      ...
      for (const channel of mix) {
        for (let sample = 0; sample < channel.length; sample++) {
          if (Math.abs(channel[sample]!) > 1)
            channel[sample] = Math.tanh(channel[sample]!);
        }
      }
      yield {
        samples: mix,
