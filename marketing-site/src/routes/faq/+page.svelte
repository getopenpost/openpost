  ...
  <div class="marketing-shell">
    ...
    <div class="faq-groups">
      {#each faqCategories as category (category.id)}
        ...
        <section id={category.id} class="faq-group scroll-mt-28" aria-labelledby={`${category.id}-title`}>
              ...
              <details id={item.id} class="group scroll-mt-28">
                ...
                <div class="faq-answer">
                  ...
                  >
                    ...
                    {#if isExternal(item.learnMore.href)}<ExternalLink aria-hidden="true" />{:else}<ArrowRight
                        aria-hidden="true"
                      />{/if}
                  </a>
                </div>
...
<section class="contact-section border-y" aria-labelledby="faq-contact-title">
  <div class="marketing-shell grid gap-10 py-14 lg:grid-cols-[0.72fr_1.28fr] lg:items-center">
    <div>
      ...
      <h2 id="faq-contact-title" class="mt-4 text-3xl font-semibold tracking-[-0.035em] text-balance">
    ...
    <div class="contact-actions">
      <a class="focus-ring contact-action" {...linkAttributes(discordCommunityUrl)} target="_blank" rel="noreferrer">
...
<section class="section-pad text-center" aria-labelledby="faq-next-title">
  <div class="marketing-shell">
    ...
    <h2 id="faq-next-title" class="marketing-heading mx-auto mt-4">See the workflow or compare its limits.</h2>
...
<style>
  .faq-hero {
    background:
      radial-gradient(circle at 72% 20%, color-mix(in oklch, var(--primary) 10%, transparent), transparent 22rem),
      var(--background);
  }
