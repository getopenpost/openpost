...
func (s *Service) createFirstWorkspace(ctx context.Context, tx bun.Tx, input ConfirmFirstWorkspaceInput) (FirstWorkspaceConfirmation, error) {
	...
	if err := workspaceprovisioning.Create(ctx, tx, workspaceprovisioning.Boundary{
		Organization:       &models.Organization{ID: organizationID, Name: input.WorkspaceName, CreatedByID: input.UserID, CreatedAt: now, UpdatedAt: now},
		OrganizationMember: &models.OrganizationMember{OrganizationID: organizationID, UserID: input.UserID, Role: models.OrganizationRoleOwner, CreatedAt: now},
		Workspace:          &workspace,
		WorkspaceMember:    &models.WorkspaceMember{WorkspaceID: workspace.ID, UserID: input.UserID, Role: models.WorkspaceRoleAdmin, Status: models.WorkspaceMemberStatusActive, CreatedAt: now, UpdatedAt: now},
	}); err != nil {
		return FirstWorkspaceConfirmation{}, err
	}
