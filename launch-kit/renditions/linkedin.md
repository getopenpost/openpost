# LinkedIn Rendition

> **Illustrative copy — not a published post.** Include LinkedIn in a real campaign only after the exact account and format pass the verification log.

AI can write a plausible social post in seconds. The harder part is deciding which account it belongs to, how it should change for that destination, and whether it is ready to publish.

That is the problem OpenPost is built around.

An authorized agent can inspect workspace context and prepare a canonical campaign with account-specific renditions. It does not receive the social provider credentials. I can then open the work in OpenPost, review the copy, media, accessibility text, destinations, and timing, and approve the exact operation that enters the queue.

The queue keeps scheduled work, provider results, failures, and retries visible. The managed app and self-hosted edition are the same AGPL product; the self-hosted runtime is one binary or container with SQLite by default and no required Redis service.

See the workflow: https://openpost.social

`[ATTACH THE VERIFIED DEMO VIDEO]`

## Why this version differs

- Starts with the operational problem rather than the product name.
- Adds enough context for a professional and technical audience.
- Explains the security and review boundary without claiming the agent is powerless.
- Ends with the implementation and deployment detail most relevant to evaluators.
