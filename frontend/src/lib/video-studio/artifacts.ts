...
export type SourceArtifactProfile = 'index' | 'studio';
...
export async function ensureSourceArtifacts(
	...
	const profile = options.profile ?? 'studio';
...
async function buildSourceArtifacts(
	...
	if (artifact?.index_complete && (profile === 'index' || artifact.studio_complete)) {
	...
	try {
		...
		return await runHeavyTask(async () => {
			try {
				...
				artifact = {
					...artifact,
					complete: studioComplete,
					studio_complete: studioComplete,
					phase: studioComplete ? 'ready' : 'failed',
					progress: studioComplete ? 1 : artifact.progress
				};
				await saveArtifactIndex(projectID, source.id, identity, artifactKey, artifact);
				notifyArtifact(projectID, artifact, options.onProgress);
				return artifact;
