<script lang="ts">
	...
	import { readScopeImage, type ScopeSample } from '$lib/editor-color-grade/scopes';
	import {
	...
	let {
		...
		sample = null,
		embedded = false
	}: { itemId: string | null; sample?: ScopeSample | null; embedded?: boolean } = $props();
	...
	const active = $derived(sample?.itemId === itemId ? sample : null);
		...
		if (renderer && gpuReady && gpuCanvas) {
			try {
				...
				if (sample.source) renderer.uploadFromCanvas(sample.source);
					...
					const image = readScopeImage(sample);
					if (!image) return;
	...
	function scopeCanvasSize(selectedScope: ColorScope): ScopeCanvasSize {
	...
	function ensureCanvasSize(canvas: HTMLCanvasElement, selectedScope: ColorScope): void {
		const fallback = scopeCanvasSize(selectedScope);
