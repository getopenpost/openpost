...
function releaseOrtSession(session: OrtSession): Promise<void> {
...
export interface SupertonicGenerateOptions {
	text: string;
...
interface UnicodeBatch {
	textIds: number[][];
	textMask: number[][];
	unsupportedChars: string[];
}

interface NoisyLatent {
	latentBuffer: Float32Array;
	latentMask: Float32Array;
	latentDimensions: [number, number, number];
	latentMaskDimensions: [number, number, number];
}

const MODEL_LABEL = 'Supertonic 3';
const MODEL_REVISION = '4402acca094e2ff2b3dff446e0bf915f3fccc462';
...
const MODEL_BASE_URL = `https://huggingface.co/spaces/Supertone/supertonic-3/resolve/${MODEL_REVISION}/assets`;
...
export function getSupertonicVoiceOption(voice: SupertonicTtsVoice): {
...
function isSupportedLanguageCode(value: string): value is SupertonicLanguageCode {
	return SUPERTONIC_TTS_SUPPORTED_LANGUAGE_CODES.some((code) => code === value);
}
...
class UnicodeProcessor {
	...
	call(textList: string[], lang: SupertonicLanguageCode): UnicodeBatch {
...
function flattenNumbers(value: number | NestedNumberArray): number[] {
...
function createNoisyLatent(config: SupertonicTtsConfig, durationSeconds: number): NoisyLatent {
	...
	return {
		latentBuffer,
		latentMask,
		latentDimensions: [1, latentDim, latentLen],
		latentMaskDimensions: [1, 1, latentLen]
	};
}
...
async function fetchJson<T>(url: string): Promise<T> {
	// SAFETY: These files come from the pinned Supertonic model revision and their fields are
	// consumed through the model's matching typed runtime contract.
...
class SupertonicTtsService {
	...
	private async loadModels(
		...
		let completed = 0;
		const receivedByFile = new Map(modelFiles.map(([, , file]) => [file, 0]));
		const totalByFile = new Map(
			modelFiles.map(([, , file]) => [file, MODEL_FILE_BYTES[file] ?? 0])
		);
		...
		const results = await Promise.allSettled(
			modelFiles.map(async ([key, label, fileName]) => {
				const bytes = await fetchOnnxModelBytes(
					...
					(received, total) => {
						receivedByFile.set(fileName, received);
						if (total > 0) totalByFile.set(fileName, total);
						const receivedBytes = [...receivedByFile.values()].reduce(
							(sum, value) => sum + value,
							0
						);
						...
						onProgress?.({
							stage: 'downloading',
							message: `Loading ${MODEL_LABEL}`,
							progress: totalBytes > 0 ? Math.min(receivedBytes / totalBytes, 0.99) : null,
							backend,
							receivedBytes,
							totalBytes
						});
				...
				const session = await ort.InferenceSession.create(new Uint8Array(bytes), sessionOptions);
				completed += 1;
				onProgress?.({
					stage: 'preparing',
					message: `${label} ${completed} of ${modelFiles.length}`,
					progress: completed / modelFiles.length,
					backend
				});
		...
		if (failure) {
			await Promise.allSettled(
				results.map((r) => (r.status === 'fulfilled' ? releaseOrtSession(r.value[1]) : undefined))
			);
			throw failure.reason;
		}

		const loaded = new Map<keyof SupertonicTtsModels, OrtSession>();
		for (const result of results) {
			if (result.status === 'fulfilled') {
				const [key, session] = result.value;
				loaded.set(key, session);
			}
		}
		const durationPredictor = loaded.get('durationPredictor');
		const textEncoder = loaded.get('textEncoder');
		const vectorEstimator = loaded.get('vectorEstimator');
		const vocoder = loaded.get('vocoder');
		if (!durationPredictor || !textEncoder || !vectorEstimator || !vocoder) {
			throw new Error('Supertonic model set did not load completely.');
		}
		return { durationPredictor, textEncoder, vectorEstimator, vocoder };
	}

	private async hasWebGpuAdapter(): Promise<boolean> {
		if (typeof navigator === 'undefined' || !('gpu' in navigator) || !window.isSecureContext) {
			return false;
		}
		try {
	...
	private async ensureRuntime(
		...
		this.runtimePromise = (async () => {
			...
			onProgress?.({
				stage: 'preparing',
				message: 'Loading Supertonic configuration',
				progress: 0
			});
			...
			const canUseWebGpu = await this.hasWebGpuAdapter();
		...
		const promise = fetchJson<VoiceStyleData>(`${VOICE_STYLE_BASE_URL}/${voice}.json`).then(
			(styleData) => ({
				styleTtl: new ort.Tensor(
					'float32',
					Float32Array.from(flattenNumbers(styleData.style_ttl.data)),
					styleData.style_ttl.dims
				),
				styleDp: new ort.Tensor(
					'float32',
					Float32Array.from(flattenNumbers(styleData.style_dp.data)),
	...
	private async synthesizeSegment({
		...
		const { latentBuffer, latentMask, latentDimensions, latentMaskDimensions } = createNoisyLatent(
			runtime.config,
			durationSeconds
		);
		...
		const latentMaskTensor = floatTensor(ort, latentMask, latentMaskDimensions);
		...
		for (let step = 0; step < totalSteps; step += 1) {
			...
			const vectorResult = await runtime.models.vectorEstimator.run({
				noisy_latent: floatTensor(ort, latentBuffer, latentDimensions),
			...
			if (!(denoisedLatent.data instanceof Float32Array)) {
				throw new Error('Supertonic vector estimator returned non-float audio data.');
			}
			latentBuffer.set(denoisedLatent.data);
		}

		const vocoderResult = await runtime.models.vocoder.run({
			latent: floatTensor(ort, latentBuffer, latentDimensions)
		});
		throwIfAborted(signal);
		...
		if (!(wavTensor.data instanceof Float32Array)) {
			throw new Error('Supertonic vocoder returned non-float audio data.');
		}
		const wavBatch = wavTensor.data;
	...
	async generateSpeechFile({
		...
		return this.withGenerationLock(async () => {
			...
			try {
				...
				onProgress?.({
					stage: 'preparing',
					message: `Loading ${getSupertonicVoiceOption(voice).label} voice style`,
					progress: 1,
				...
				return {
					...
					sampleRate: runtime.config.ae.sample_rate
				};
			} catch (error) {
