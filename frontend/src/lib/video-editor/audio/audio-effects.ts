  ...
  switch (effect.type) {
    ...
    case "delay": {
      ...
      return {
        ...
        lowCutHz: clamp(
          e.lowCutHz ?? DEFAULT_DELAY.lowCutHz,
          20,
          2000,
    ...
    case "chorus": {
      ...
      return {
        ...
        rateHz: clamp(
          ...
          CHORUS_RATE_MIN,
          CHORUS_RATE_MAX,
          DEFAULT_CHORUS.rateHz,
        ),
        depthMs: clamp(
          e.depthMs,
          CHORUS_DEPTH_MIN,
          CHORUS_DEPTH_MAX,
          DEFAULT_CHORUS.depthMs,
        ),
    ...
    case "distortion": {
      ...
      return {
        ...
        outputGainDb: clamp(
          ...
          -24,
          12,
          DEFAULT_DISTORTION.outputGainDb,
        ),
      };
    }
  }
...
export function normalizeAudioEffects(effects: unknown): AudioEffect[] {
  ...
  for (const raw of effects) {
    ...
    if (!(AUDIO_EFFECT_TYPES as readonly string[]).includes(typed.type))
      continue;
...
export function cloneAudioEffects(
...
export function hasActiveAudioEffects(
  effects: AudioEffect[] | undefined,
): boolean {
...
class OfflineCompressor {
  ...
  constructor(
    ...
    this.attackCoeff = Math.exp(
      -1 / (Math.max(0.1, params.attackMs) * 0.001 * sampleRate),
    );
    this.releaseCoeff = Math.exp(
      -1 / (Math.max(1, params.releaseMs) * 0.001 * sampleRate),
    );
  }
  ...
  processChannel(samples: Float32Array): Float32Array {
    ...
    for (let i = 0; i < samples.length; i++) {
      ...
      const coeff =
        inputDb > this.envelopeDb ? this.attackCoeff : this.releaseCoeff;
      ...
      if (over > -this.kneeDb / 2) {
        if (over <= this.kneeDb / 2) {
          ...
          gainReductionDb =
            ((1 - 1 / this.ratio) * kneeOver * kneeOver) / (2 * this.kneeDb);
 ...
 */
export function applyPanOffline(
...
class OfflineDelay {
  ...
  constructor(
    ...
    this.delaySamples = Math.max(
      1,
      Math.round((params.timeMs / 1000) * sampleRate),
    );
  ...
  process(channels: Float32Array[]): Float32Array[] {
    ...
    for (let i = 0; i < len; i++) {
      for (let ch = 0; ch < this.channelCount; ch++) {
        ...
        const readIndex =
          (this.writeIndex - this.delaySamples + bufLen) % bufLen;
...
class OfflineReverb {
  ...
  constructor(
    ...
    const combTunings = [1116, 1188, 1277, 1356, 1422, 1491, 1557, 1617].map(
      (s) => Math.max(1, Math.round((s * sampleRate) / 44100)),
    );
    ...
    this.combsPerChannel = Array.from({ length: channelCount }, () =>
      combTunings.map((samples) => ({
        ...
        index: 0,
        feedback:
          Math.pow(0.001, samples / (decay * sampleRate)) *
          (0.5 + params.roomSize * 0.5),
        damp: params.damping,
        filterState: 0,
      })),
    );
  ...
  process(channels: Float32Array[]): Float32Array[] {
    ...
    for (let i = 0; i < len; i++) {
      for (let ch = 0; ch < this.channelCount; ch++) {
        ...
        const preOut =
          preBuf[
            (write - this.preDelaySamples + preBuf.length) % preBuf.length
          ] ?? 0;
        preBuf[write] = dry;
        ...
        for (const comb of this.combsPerChannel[ch]!) {
          ...
          comb.filterState =
            delayed * (1 - comb.damp) + comb.filterState * comb.damp;
...
class OfflineModulatedDelay {
  ...
  constructor(
    ...
    const maxDelayMs =
      params.type === "chorus"
        ? CHORUS_DELAY_MAX + CHORUS_DEPTH_MAX
        : 15 + FLANGER_DEPTH_MAX;
  ...
  process(channels: Float32Array[]): Float32Array[] {
    ...
    for (let i = 0; i < len; i++) {
      ...
      for (let ch = 0; ch < this.channelCount; ch++) {
        ...
        buf[this.writeIndex] = Math.max(
          -2,
          Math.min(2, dry + delayed * feedback),
        );
    ...
    for (const effect of normalizeAudioEffects(effects)) {
      ...
      switch (effect.type) {
        case "compressor":
          entry = {
            kind: "compressor",
            impl: new PerChannelCompressor(
              effect as CompressorEffect,
              sampleRate,
              channelCount,
            ),
          };
        ...
        case "reverb":
          entry = {
            kind: "reverb",
            impl: new OfflineReverb(
              effect as ReverbEffect,
              sampleRate,
              channelCount,
            ),
          };
        ...
        case "chorus":
          entry = {
            kind: "chorus",
            impl: new OfflineModulatedDelay(
              effect as ChorusEffect,
              sampleRate,
              channelCount,
            ),
          };
...
export function applyAudioEffectStages(
  ...
  const chain = new StreamingAudioEffectChain(
    effects,
    sampleRate,
    channels.length,
  );
...
export function getAudioEffectTailSeconds(
  effects: AudioEffect[] | undefined,
): number {
  if (!effects) return 0;
  let tail = 0;
  for (const e of effects) {
    ...
    if (e.type === "reverb")
      tail = Math.max(tail, (e as ReverbEffect).decaySeconds * 0.6);
    if (e.type === "delay")
      tail = Math.max(tail, ((e as DelayEffect).timeMs / 1000) * 2.2);
  }
