...
const publicDirectory = resolve(scriptDirectory, '../backend/cmd/openpost/public');
const compressibleExtensions = new Set(['.css', '.html', '.js', '.svg', '.webmanifest']);
...
console.log(
	`Precompressed ${candidates.length} text assets: ${formatMiB(sourceBytes)} source, ` +
		`${formatMiB(brotliBytes)} Brotli, ${formatMiB(gzipBytes)} gzip.`
);
