...
- **Whisper Tiny multilingual**, pinned to the exact model commit listed in `frontend/static/video-studio-models/THIRD_PARTY_LICENSES.md`, for local transcription under Apache License 2.0.
- **Silero VAD 6.2** for local voice-activity detection under the MIT License.

The model files, hashes, source revisions, and notices ship under `frontend/static/video-studio-models/`. Operators that set `OPENPOST_VIDEO_MODEL_BASE_URL` must serve the matching content and keep those notices. The checked-in mastered audio assets and machine-readable hashes under `frontend/static/video-studio-audio/` are dedicated under CC0 1.0 as stated beside the files.
