...
- **Production release flow:** for normal “commit, push, release, deploy prod” requests, run `pnpm release:prod "<conventional commit message>"`. The script derives the version from every commit since the last release plus the pending commit, promotes `CHANGELOG.md` from `Unreleased` into that version, pushes `main`, creates the SemVer tag, and waits for GitHub `Build and Release`; after preflight and image publication, that workflow publishes the same changelog section as the GitHub release notes, calls the authenticated VPS deploy hook, and verifies readiness. `RELEASE_BUMP` may only raise the inferred impact; `RELEASE_VERSION` is reserved for an intentional greater version-line correction and cannot undercut commit impact. If a tag fails, do not retag it; fix forward with a new SemVer tag.
...
- **Use the shared Shadcn-svelte form controls everywhere.** Application, public Studio, and marketing Svelte files must use the shared `Input`, `Textarea`, `Select`/`AppSelect`, `Checkbox`, `RadioGroup`, `Slider`, and related primitives from `frontend/src/lib/components/ui/`. Do not add visible native `input`, `select`, or `textarea` elements outside those primitive implementations. The marketing site shares the same `$lib` component source. Run `pnpm check:ui-consistency` when form UI changes.
