...
describe('safeSameOriginRedirect', () => {
	it('preserves a same-origin path, query, and hash', () => {
		const url = new URL(
			`https://app.openpo.st/login?redirect=${encodeURIComponent('/calendar?view=week#scheduled')}`
		);
	...
	it.each([
		...
		const url = new URL(`https://app.openpo.st/login?redirect=${encodeURIComponent(redirect)}`);
