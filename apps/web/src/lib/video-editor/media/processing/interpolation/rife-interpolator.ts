...
import { fetchOnnxModelBytes } from '../../../transcript/engine/onnx-model-cache';
...
export class RifeInterpolator {
	...
	private async getSession(width: number, height: number): Promise<OrtSession> {
		...
		try {
			...
			logger.warn('RIFE WebGPU session failed, falling back to wasm', {
				key,
				error
			});
			this.backend = 'wasm';
	...
	async interpolate(
		...
		try {
			...
			if (!(output.data instanceof Float32Array)) {
				throw new Error('RIFE returned a non-float output tensor');
			}
