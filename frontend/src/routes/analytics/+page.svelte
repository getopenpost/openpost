...
<script lang="ts">
	...
	function contentStatus(item: AnalyticsContent) {
		if (item.status === 'permission_required') return item.error_message || m.analytics_permission_required();
							...
							>
								...
								<span class="min-w-0 flex-1">
									...
									{#if accountStatus(account)}
										<span class={['mt-0.5 block line-clamp-2 text-xs', accountStatusClass(account)]}>
				...
				{:else}
					<div class="divide-y divide-border border-y border-border">
						{#each publications as publication (publication.publication_id)}
							...
							<article>
								<div class="grid min-w-0 gap-4 px-1 py-4 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center">
									<div class="min-w-0">
										<a
										...
										{#each renditions as rendition (rendition.rendition_id)}
											...
											>
												<div class="flex min-w-0 items-start gap-2.5">
													<PlatformIcon platform={rendition.platform} class="mt-0.5 size-4 shrink-0" />
													<div class="min-w-0">
