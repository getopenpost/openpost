								...
								<AppSelect
									...
									options={availableDevices
										...
										.map((device, index) => ({
											...
											label: device.label || m.video_studio_camera_number({ number: index + 1 })
										}))}
								/>
							</label>
						{/if}
							...
							<label class="grid gap-1.5 text-sm">
								...
								<AppSelect
									...
									options={availableDevices
										...
										.map((device, index) => ({
											value: device.deviceId,
											label: device.label || m.video_studio_microphone_number({ number: index + 1 })
										}))}
								/>
							</label>
						{/if}
			...
			<section class="mt-8 border-y py-6" aria-labelledby="recording-setup-title">
				<div class="grid gap-8 md:grid-cols-[minmax(0,1fr)_18rem]">
					...
					<div class="space-y-4">
						...
						{#if camera}
							<label class="grid gap-1.5 text-sm">
								<span class="font-medium">{m.video_studio_camera()}</span>
								<AppSelect
									value={cameraDeviceID}
									onValueChange={(value) => (cameraDeviceID = value)}
									options={[
										{ value: '', label: m.video_studio_device_default() },
										...availableDevices
						...
						{#if microphone}
							<label class="grid gap-1.5 text-sm">
								<span class="font-medium">{m.video_studio_microphone()}</span>
								<AppSelect
									value={microphoneDeviceID}
									...
									options={[
										...
										...availableDevices
											...
											.map((device, index) => ({
												value: device.deviceId,
												label:
													device.label || m.video_studio_microphone_number({ number: index + 1 })
											}))
									]}
								/>
							</label>
						...
						<div class="grid grid-cols-2 gap-2">
							<label class="grid gap-1.5 text-sm">
								<span class="font-medium">{m.video_studio_record_countdown()}</span>
								<AppSelect
									value={String(countdownSeconds)}
									onValueChange={(value) => (countdownSeconds = Number(value) as 0 | 3 | 5)}
									options={[
										{ value: '0', label: m.video_studio_record_countdown_off() },
										{ value: '3', label: m.video_studio_record_seconds({ seconds: 3 }) },
										{ value: '5', label: m.video_studio_record_seconds({ seconds: 5 }) }
									]}
								/>
							</label>
							<label class="grid gap-1.5 text-sm">
								<span class="font-medium">{m.video_studio_record_planned()}</span>
								<AppSelect
									value={String(plannedMinutes)}
									onValueChange={(value) => (plannedMinutes = Number(value) as 5 | 10 | 20)}
									options={[5, 10, 20].map((minutes) => ({
										value: String(minutes),
										label: m.video_studio_record_minutes({ minutes })
									}))}
								/>
							</label>
						</div>
						<p class="text-xs leading-5 text-muted-foreground">
							{m.video_studio_record_estimate({
						...
						<Button class="w-full" disabled={creating || !entryReady} onclick={startRecording}>
							{#if creating}<LoaderIcon class="size-4 animate-spin" />{/if}
							{countdownRemaining > 0
								? m.video_studio_record_countdown_active({ seconds: countdownRemaining })
