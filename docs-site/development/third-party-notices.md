...
The model files, hashes, source revisions, and notices ship under `frontend/static/video-studio-models/`. Operators that set `OPENPOST_VIDEO_MODEL_BASE_URL` must serve the matching content and keep those notices. The procedural audio manifest under `frontend/static/video-studio-audio/` is licensed separately as stated beside the files.
