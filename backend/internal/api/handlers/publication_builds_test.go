...
func (stub *publicationBuildPublicationStub) Create(
	_ context.Context,
	_ string,
...
func TestPublicationBuildCreateUsesSocialSetSubsetCapabilitiesAndStoredAuthority(t *testing.T) {
	...
	require.Equal(t, "provider-1", authority.IdentityProviderID)
...
func TestPublicationBuildCreateKeepsUnsupportedCandidatesForAnExplicitSkip(t *testing.T) {
	...
	require.Equal(t, http.StatusBadRequest, unsupportedOnly.Code, unsupportedOnly.Body.String())
}
...
func newPublicationBuildTestServer(t *testing.T) *publicationBuildTestServer {
	...
	policy := &models.OrganizationSSOPolicy{
		OrganizationID: "org-1", Mode: models.OrganizationSSOModeRequired,
...
func createPublicationBuildVoiceTables(t *testing.T, db *bun.DB) {
	...
	for _, statement := range []string{
		...
		`CREATE UNIQUE INDEX publication_build_voice_default_idx ON voice_profiles (workspace_id) WHERE is_default = true`,
