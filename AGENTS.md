...
- Keep feature identity SVGs in `assets/brand/features/` for the README and public materials. The application uses the theme icon system for page, navigation, and action icons; do not render or distribute feature artwork to the app. Preserve Converge geometry: the shared app logo uses the active theme's focal color, transparent public marks use peach-orange, and artwork with a colored background keeps its existing colors.
