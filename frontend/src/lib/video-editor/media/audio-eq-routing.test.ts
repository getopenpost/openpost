...
import { captureSnapshot, restoreSnapshot, snapshotsEqual } from '../timeline/commands/snapshot.svelte';
...
describe('audio EQ data plumbing', () => {
	...
	it('blocks smart copy when track EQ or bus EQ active', () => {
		...
		const projectFor = (tracks: TimelineTrack[], bus?: import('../audio/types').AudioEqSettings): Project => ({
			id: 'p',
			name: 'P',
			description: '',
			createdAt: 0,
		...
		expect(
			assessSmartCopy(projectFor([activeTrack]), { format: 'webm', codec: 'vp9', width: 1920, height: 1080 }, [media])
		).toEqual({ eligible: false, blocker: 'edited-audio' });

		const busActive = { highGainDb: 2 };
		expect(
			assessSmartCopy(projectFor([baseTrack], busActive), { format: 'webm', codec: 'vp9', width: 1920, height: 1080 }, [media])
		).toEqual({ eligible: false, blocker: 'edited-audio' });
		...
		expect(
			assessSmartCopy(compBusActive, { format: 'webm', codec: 'vp9', width: 1920, height: 1080 }, [media])
		).toEqual({ eligible: false, blocker: 'edited-audio' });
	});
