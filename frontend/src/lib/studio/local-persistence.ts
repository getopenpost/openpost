...
export async function openStudioDatabase(): Promise<IDBDatabase> {
	return await new Promise((resolve, reject) => {
		...
		request.onerror = () =>
			reject(request.error ?? new Error('Could not open local Studio storage.'));
		request.onblocked = () => reject(new Error('Close other OpenPost tabs and try again.'));
	});
}
...
export async function loadGuestStudioDesign(id: string): Promise<StudioDocumentResponse> {
	...
	if (!record) throw new Error('This local design is no longer available in this browser.');
...
export async function getGuestStudioMediaBlob(mediaID: string): Promise<Blob> {
	...
	if (!record) throw new Error('A local image used by this design is missing.');
	const blob = await guestMediaBlob(record);
	if (!blob) throw new Error('A local image used by this design is no longer available.');
...
export function replaceGuestStudioMediaIDs(
	...
	for (const page of next.pages) {
		...
		for (const layer of page.layers) {
			if (layer.image) {
				layer.image.media_id =
					replacements.get(layer.image.media_id) ?? layer.image.media_id;
			}
...
async function imageDimensions(blob: Blob): Promise<{ width: number; height: number }> {
	try {
		...
		throw new Error('This image could not be opened in your browser.');
	}
}
