...
<script lang="ts">
	...
	const templates: Array<{
		...
		{
			id: 'clean-captions',
			name: () => m.video_studio_template_clean(),
			description: () => m.video_studio_template_clean_description()
		},
		{
			id: 'product-demo',
			name: () => m.video_studio_template_product(),
			description: () => m.video_studio_template_product_description()
		},
		{
			id: 'talking-head',
			name: () => m.video_studio_template_talking(),
			description: () => m.video_studio_template_talking_description()
		},
		{
			id: 'announcement',
			name: () => m.video_studio_template_announcement(),
			description: () => m.video_studio_template_announcement_description()
		}
	];
	...
	async function createFromTemplate(templateID: TemplateID): Promise<void> {
		...
		try {
			...
			await goto(resolve(`/video-studio/${saved.id}` as '/'));
		} catch (cause) {
			error = cause instanceof Error ? cause.message : m.video_studio_create_failed();
			creating = false;
		}
		...
		{:else}
			<div class="mt-8 space-y-4">
				...
				{#if capabilities && !capabilities.supported}
					<InlineNotice tone="warning">
						<div class="space-y-1">
							...
							<p class="text-current/80">{m.video_studio_unsupported_body()}</p>
							<a
				...
				{:else if capabilities?.editorMode === 'preview'}
					<InlineNotice tone="info" message={m.video_studio_unsupported_mobile()} />
				{/if}
			</div>
			...
			<section class="mt-8 border-y py-6" aria-labelledby="video-start-heading">
				...
				<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
					...
					<a
						href={resolve('/video-studio/new?mode=stock' as '/')}
						aria-disabled={!enabled || !capabilities?.supported}
						tabindex={!enabled || !capabilities?.supported ? -1 : undefined}
						class="group min-h-36 rounded-lg border bg-card p-4 text-left transition-colors hover:border-primary/50 hover:bg-muted/30 focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none aria-disabled:pointer-events-none aria-disabled:opacity-50"
						onclick={(event) => {
					...
					<a
						href={resolve('/video-studio/new?mode=blank' as '/')}
						aria-disabled={!enabled || !capabilities?.supported}
						tabindex={!enabled || !capabilities?.supported ? -1 : undefined}
						class="group min-h-36 rounded-lg border bg-card p-4 text-left transition-colors hover:border-primary/50 hover:bg-muted/30 focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none aria-disabled:pointer-events-none aria-disabled:opacity-50"
						onclick={(event) => {
			...
			<section class="mt-8" aria-labelledby="templates-heading">
				<div>
					<h2 id="templates-heading" class="text-lg font-semibold">
						{m.video_studio_templates()}
					</h2>
					<p class="mt-1 text-sm text-muted-foreground">
						{m.video_studio_templates_description()}
					</p>
				</div>
