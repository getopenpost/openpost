/** Adobe/Resolve .cube parser and WebGL LUT effect. Ported from FreeCut (MIT) rev 4d62e80. */
...
function parseFloatToken(token: string | undefined, context: string): number {
	if (token === undefined) throw new Error(`Invalid .cube file: missing value in ${context}`);
...
function parseDomainTriple(line: string, context: string, target: [number, number, number]): void {
	const tokens = line.split(/\s+/);
	for (let c = 0; c < 3; c++) target[c] = parseFloatToken(tokens[c + 1], context);
}
...
function isCubeDataLine(line: string): boolean {
	...
	return (
		(firstChar >= 48 && firstChar <= 57) || firstChar === 45 || firstChar === 43 || firstChar === 46
	);
}
