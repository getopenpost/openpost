...
export function resolveAudioOwner(input: AudioOwnerInput): AudioOwner {
	const { item, tracks, allItems, mediaEntry, usesSeparateProxyAudio, usesProcessedAudio } =
		input;
...
export function isAudibleOwner(owner: AudioOwner): boolean {
	return owner === 'embedded' || owner === 'processed' || owner === 'separateProxy';
}
