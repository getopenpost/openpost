# Mastodon Rendition

> **Illustrative copy — not a published post.** Include Mastodon in a real campaign only after the exact instance, account, and format pass the verification log.

AI can help prepare a release, but it should not flatten every community into the same post or receive social-provider credentials it does not need.

OpenPost lets an authorized agent inspect a selected workspace and prepare destination-specific renditions through MCP. I review the account, copy, media, alt text, and schedule in the web app before approving what enters the queue.

The managed app and self-hosted edition use the same AGPL code. A self-hosted installation can run as one Go binary or container with SQLite and local media by default; Redis is not required.

Provider differences remain visible, including Mastodon instance limits and media behavior.

https://openpost.social

`[ATTACH THE VERIFIED DEMO VIDEO WITH COMPLETE ALT TEXT]`

## Why this version differs

- Leads with community-specific adaptation and credential restraint.
- Gives the open-source and compact self-hosting details more space.
- Calls out instance-specific provider behavior instead of implying one universal Mastodon limit.
- Requires useful media alt text in the final campaign.
