...
export const DEFAULT_SHADOW_EFFECT: StudioShadowEffect = {
	color: '#000000',
	opacity: 0.35,
	blur: 24,
	angle: 45,
...
export function defaultTextCurve(): StudioTextCurve {
	return { type: 'none', strength: 0.65, offset: 0, reverse: false };
}

export function defaultLayerMask(): StudioLayerMask {
	return { shape: 'rectangle', inset: 0, radius: 32 };
}

export function shadowOffset(effect: StudioShadowEffect): { x: number; y: number } {
...
export function shadowColor(effect: StudioShadowEffect): string {
...
export function textCurveStartOffset(width: number, curve: StudioTextCurve): number {
