<script lang="ts">
	...
	$effect(() => {
		...
		if (loadedSourceID !== source.id) {
			void localVideoSourceURL(source)
				.then((url) => {
					sourceURL = url;
				...
				.catch((cause) => {
					sourceError = cause instanceof Error ? cause.message : m.video_studio_project_missing();
				});
		}
	});
	...
	$effect(() => {
		...
		for (const source of sources) {
			void localVideoSourceURL(source!).then((url) => {
	...
	$effect(() => {
		const elements = Array.from(
			document.querySelectorAll<HTMLAudioElement>('[data-video-studio-audio]')
		);
		for (const audio of audioFrame.sources) {
			const element = elements.find(
				(candidate) => candidate.dataset.videoStudioAudio === audio.item_id
			);
			if (!element) continue;
		...
		for (const element of elements) {
			if (!audioFrame.sources.some((audio) => audio.item_id === element.dataset.videoStudioAudio)) {
				element.pause();
			}
		}
	});
	...
	$effect(() => {
		for (const layer of frame.visual_layers) {
			...
			const element = document.querySelector<HTMLVideoElement>(
				`[data-video-studio-overlay="${layer.item.id}"]`
			);
			if (!element) continue;
			const expected =
	...
	function effectFilter(clipID = primary?.clip_id): string {
		const effects = project.primary_sequence.find((clip) => clip.id === clipID)?.effects ?? [];
	...
	function moveSelection(event: PointerEvent): void {
		if (!dragging || !onTransform) return;
	...
	function clipForPrimary(): PrimarySequenceClip | undefined {
		return project.primary_sequence.find((clip) => clip.id === primary?.clip_id);
	}
...
>
	<div
		...
		aria-label={`${variant.name} ${m.video_studio_safe_area()}`}
	>
		{#if primary && sourceURL}
			<button
				...
				aria-label={`${m.video_studio_clip()} ${primary.clip_id}`}
				aria-pressed={selectedClipID === primary.clip_id}
				onpointerdown={(event) => {
					dragging = true;
					event.currentTarget.setPointerCapture(event.pointerId);
					onSelectClip?.(primary.clip_id);
					moveSelection(event);
				}}
		...
		{:else}
			<div class="absolute inset-0 flex flex-col items-center justify-center gap-3 text-zinc-500">
				...
				<p class="text-sm">{sourceError || m.video_studio_empty_preview()}</p>
			</div>
		{/if}
		...
		{#each audioFrame.sources as audio (audio.item_id)}
			{#if overlaySourceURLs[audio.source_id]}
				<audio
					data-video-studio-audio={audio.item_id}
