...
- Applied X text and video limits per connected account by reading the authenticated account's subscription tier, with standard limits used when the tier cannot be verified.
- Made publications the single inventory behind Posts, sidebar drafts, and Calendar while preserving the text-and-thread composer for text/thread work and focused composers for story and video formats.
- Rebuilt Calendar with published history, responsive month and week views, 15-minute timed placement, mobile agenda behavior, status and destination filters, and a direct queue link.
...
- Kept an empty composer quiet while Publish and Schedule stay disabled, and removed the redundant Calendar heading from the planning sidebar.
