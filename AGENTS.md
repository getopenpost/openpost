...
- Publication Builder jobs freeze voice, destination, and provider constraints before generation. Keep source IDs bound to ordered multimodal parts, return only safe source metadata, and attach raw media only to Renditions whose exact `use_source` plan selects it.
