...
<script lang="ts">
	...
	import { loadVideoStudioConfig, type VideoStudioConfig } from '$lib/video-studio/api';
	import { cachedVideoStudioModels, removeVideoStudioModel } from '$lib/video-studio/model-manager';
	import type { ModelCacheMetadata } from '$lib/video-studio/types';
	import { formatBytes } from '$lib/video-studio/project';
	import { m } from '$lib/paraglide/messages';
	import ArrowLeftIcon from 'lucide-svelte/icons/arrow-left';
	import LoaderIcon from 'lucide-svelte/icons/loader-2';
	import TrashIcon from 'lucide-svelte/icons/trash-2';

	let config = $state<VideoStudioConfig | null>(null);
	...
	async function load(): Promise<void> {
		...
		error = '';
		try {
			[config, cached] = await Promise.all([loadVideoStudioConfig(), cachedVideoStudioModels()]);
		} catch (cause) {
			error = cause instanceof Error ? cause.message : m.video_studio_models_load_failed();
		} finally {
			loading = false;
		}
	}
	...
	async function remove(modelID: string): Promise<void> {
		...
		error = '';
		try {
			await removeVideoStudioModel(config, modelID);
			cached = await cachedVideoStudioModels();
		} catch (cause) {
			error = cause instanceof Error ? cause.message : m.video_studio_models_remove_failed();
		} finally {
			removing = '';
		}
	}
...
<svelte:head>
	<title>{m.video_studio_models_meta_title()}</title>
</svelte:head>
...
<div class="video-studio-theme min-h-dvh bg-background text-foreground">
	...
	<main class="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-10">
		...
		<div class="mt-5 max-w-2xl">
			<h1 class="text-2xl font-semibold tracking-tight">{m.video_studio_models_heading()}</h1>
			<p class="mt-2 text-sm leading-6 text-muted-foreground">
				{m.video_studio_models_description()}
			</p>
		</div>
		...
		{:else if config}
			...
			>
				<div class="border-b px-4 py-3 sm:px-5">
					<h2 id="model-list-title" class="font-medium">{m.video_studio_models_downloaded()}</h2>
					<p class="mt-1 text-xs text-muted-foreground">
						{m.video_studio_models_total({ size: formatBytes(cachedBytes) })}
					</p>
				</div>
					...
					>
						<div class="min-w-0 flex-1">
							...
							<p class="mt-1 text-xs leading-5 text-muted-foreground">
								...
								{state ? m.video_studio_models_cached() : m.video_studio_models_not_cached()}
							</p>
						</div>
						{#if state}
							<Button
							...
							>
								...
								{/if}
								{m.video_studio_models_remove()}
							</Button>
						{/if}
					</div>
				{/each}
			</section>
			<InlineNotice class="mt-5" tone="info" message={m.video_studio_models_privacy_notice()} />
		{/if}
	</main>
</div>
