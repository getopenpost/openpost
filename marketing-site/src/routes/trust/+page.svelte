<script lang="ts">
  ...
  const primaryStore = managedService.stores.find(
    ({ id }) => id === "primary-host",
  );
  const mediaStore = managedService.stores.find(
    ({ id }) => id === "media-objects",
  );
...
<header class="border-b py-14 sm:py-20">
  <div class="marketing-shell grid gap-8 lg:grid-cols-[1fr_18rem] lg:items-end">
    <div class="max-w-4xl">
      ...
      <h1
        class="mt-4 max-w-4xl text-4xl font-semibold tracking-tight text-balance sm:text-5xl"
      >
        Where managed OpenPost data is stored and processed.
      </h1>
      <p class="text-muted-foreground mt-5 max-w-3xl text-lg leading-8">
        This dated register names the infrastructure, optional services,
        user-directed recipients, and human access boundary for
        app.openpost.social. It does not make a certification claim.
      </p>
    </div>
...
<section class="border-b" aria-label="Managed service at a glance">
  <div
    class="marketing-shell grid divide-y sm:grid-cols-3 sm:divide-x sm:divide-y-0"
  >
  ...
  <div class="marketing-shell grid gap-10 lg:grid-cols-[18rem_1fr]">
    <div>
      ...
      <p class="text-muted-foreground mt-4 text-sm leading-6">
        Browser-local editor data is included so the boundary is explicit even
        before anything reaches the managed service.
      </p>
    </div>
        ...
        <article class="grid gap-5 py-6 md:grid-cols-[12rem_1fr]">
          ...
          <dl class="grid gap-4 text-sm leading-6 sm:grid-cols-3">
            <div>
              ...
              <dd class="text-muted-foreground mt-1">{store.data}</dd>
            </div>
            <div>
              <dt class="font-semibold">Retention</dt>
              <dd class="text-muted-foreground mt-1">{store.retention}</dd>
            </div>
            <div>
              <dt class="font-semibold">Protection</dt>
              <dd class="text-muted-foreground mt-1">{store.protection}</dd>
            </div>
          </dl>
        </article>
      {/each}
...
<section class="section-pad bg-muted/20 border-y">
  <div class="marketing-shell">
    <div class="max-w-3xl">
      ...
      <p class="text-muted-foreground mt-4 text-sm leading-6">
        “Feature-triggered” means no request is sent until that feature is used.
        A provider's role describes this OpenPost data path; it does not replace
    ...
    <div class="mt-10 divide-y border-y">
      {#each managedService.providers as provider (provider.id)}
        <article
          id={provider.id}
          class="grid scroll-mt-24 gap-5 py-7 lg:grid-cols-[14rem_1fr_1fr]"
        >
          <div>
            <h3 class="font-semibold">{provider.name}</h3>
            <p class="text-primary mt-2 text-xs font-medium">
          ...
          <dl class="grid gap-4 text-sm leading-6">
            <div>
              ...
              <dd class="text-muted-foreground mt-1">{provider.purpose}</dd>
            </div>
            <div>
              <dt class="font-semibold">Data</dt>
              <dd class="text-muted-foreground mt-1">{provider.data}</dd>
            </div>
          </dl>
          <dl class="grid gap-4 text-sm leading-6">
            <div>
              <dt class="font-semibold">Location</dt>
              <dd class="text-muted-foreground mt-1">{provider.location}</dd>
            </div>
            <div>
              <dt class="font-semibold">Transfer facts</dt>
              <dd class="text-muted-foreground mt-1">{provider.transfer}</dd>
            </div>
            <div class="flex flex-wrap gap-x-4 gap-y-1">
              {#each provider.source_urls as source (source)}
                <a
                  {...externalLink(source)}
                  class="focus-ring text-primary inline-flex min-h-11 items-center gap-1 rounded-md text-xs font-medium"
                >
                  {sourceLabel(source)}
...
<section class="section-pad">
  <div class="marketing-shell grid gap-10 lg:grid-cols-[18rem_1fr]">
    <div>
      ...
      <p class="text-muted-foreground mt-4 text-sm leading-6">
        These networks are not general OpenPost subprocessors. Their own terms
        apply when a user connects an account, signs in, or sends content.
      </p>
    </div>
    <div class="divide-y border-y">
      {#each managedService.directed_recipients as recipient (recipient.name)}
        <article class="grid gap-4 py-6 md:grid-cols-[12rem_1fr_1fr]">
          ...
          <div class="text-sm leading-6">
            ...
            <p class="text-muted-foreground mt-2">{recipient.data}</p>
          </div>
          <div class="text-sm leading-6">
            <p class="text-muted-foreground">{recipient.location}</p>
            <a
              {...externalLink(recipient.source_url)}
              class="focus-ring text-primary mt-2 inline-flex min-h-11 items-center gap-1 rounded-md text-xs font-medium"
            >
...
<section class="section-pad bg-muted/20 border-y">
  <div class="marketing-shell grid gap-10 lg:grid-cols-[18rem_1fr]">
    <div>
      <ShieldCheck class="text-primary size-5" aria-hidden="true" />
      <p class="section-label mt-5">Human production access</p>
      <h2 class="mt-4 text-3xl font-semibold tracking-tight text-balance">
        The access boundary includes its limits.
      </h2>
      <p class="text-muted-foreground mt-4 text-sm leading-6">
        OpenPost states where a stronger control does not exist instead of
        implying an approval or audit system that has not been implemented.
      </p>
    </div>
    <dl class="divide-y border-y">
      {#each humanAccessEntries as [key, value] (key)}
        <div class="grid gap-2 py-5 sm:grid-cols-[12rem_1fr]">
          ...
          <dd class="text-muted-foreground text-sm leading-6">{value}</dd>
        </div>
      {/each}
    </dl>
  </div>
  ...
  <div class="reading-shell border-y py-8">
    ...
    <div class="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm font-medium">
      <a
        ...
        class="focus-ring text-primary inline-flex min-h-11 items-center gap-2 rounded-md"
      >
