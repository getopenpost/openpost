...
describe('AudioEffectsPanel rack', () => {
	...
	it('supports reorder, bypass, reset, and single-gesture undo', async () => {
		...
		timelineStore.setAll({
			fps: 30,
			tracks: [{ id: 'audio', name: 'Audio', kind: 'audio', height: 72, locked: false, visible: true, muted: false, solo: false, order: 0 }],
			items: [item]
		});

		const screen = await render(AudioEffectsPanel, { item: timelineStore.itemById.get('clip-1')!, open: true });

		// Bypass first effect
		await screen.getByRole('button', { name: 'Bypass Delay' }).click();
		expect(timelineStore.itemById.get('clip-1')?.audioEffects?.find((e) => e.id === 'fx-delay')?.enabled).toBe(false);
		...
		await page.screenshot({ element: screen.container, path: '../../../../.svelte-kit/openpost-audio-effects.png' });
	});
});
