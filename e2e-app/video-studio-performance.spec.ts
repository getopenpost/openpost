...
const sourcePath = process.env.OPENPOST_VIDEO_STUDIO_BENCHMARK_SOURCE;
...
test("one-hour 1080p60 source stays bounded and responsive", async ({
  ...
  test.skip(
    !sourcePath,
    "Set OPENPOST_VIDEO_STUDIO_BENCHMARK_SOURCE to the long fixture.",
  );
  ...
  await page.locator("#video-studio-import").setInputFiles(source);
  ...
  const playbackVideo = page.locator("[data-video-studio-primary]").first();
  ...
  await testInfo.attach("video-studio-performance.json", {
    body: Buffer.from(JSON.stringify(report, null, 2)),
    contentType: "application/json",
  });
  console.info(`[video-studio-benchmark] ${JSON.stringify(report)}`);
