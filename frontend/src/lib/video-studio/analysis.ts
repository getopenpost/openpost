...
import { ensureVideoStudioModel, type ModelDownloadProgress } from './model-manager';
import { readProjectFile, saveAnalysisResult } from './storage';
...
export async function transcribeVideoProject(
	...
	const modelInfo = await ensureVideoStudioModel(
		config,
	...
	try {
		for (let clipIndex = 0; clipIndex < clips.length; clipIndex += 1) {
			...
			const audio = await sourceAudio(source, sources, options.signal);
			if (!audio) continue;
	...
	const language = options.language && options.language !== 'auto' ? options.language : 'und';
	...
	await saveAnalysisResult({
		...
		source_id: 'timeline',
		source_hash: sourceFingerprint(project),
		timeline_fingerprint: timelineFingerprint(project),
		kind: 'transcript',
		algorithm_version: `transformers-whisper-tiny:${modelInfo.model.version}`,
		settings: { language },
		result,
...
export async function detectVideoProjectSilence(
	...
	await saveAnalysisResult({
		...
		timeline_fingerprint: timelineFingerprint(project),
		kind: 'silence',
		algorithm_version: `silero-vad:${modelInfo.model.version}`,
		settings: { min_silence_us: 350_000, padding_us: 120_000 },
		result,
		review_status: 'unreviewed',
		created_at: new Date().toISOString()
	});
...
export async function measureVideoProjectLoudness(
	...
	const primarySegments = derivePrimarySequence(project)
		.map((timing) => ({
			id: project.primary_sequence[timing.index]!.id,
			kind: 'primary' as const,
			clip: project.primary_sequence[timing.index]!,
			source: project.sources[project.primary_sequence[timing.index]!.source_id],
			sourceStartUS: project.primary_sequence[timing.index]!.source_in_us,
			sourceEndUS: project.primary_sequence[timing.index]!.source_out_us
		}))
		.filter(
			(item) =>
	...
	try {
		for (let clipIndex = 0; clipIndex < segments.length; clipIndex += 1) {
			...
			const audio = await sourceAudio(segment.source, sources, options.signal);
			if (!audio) continue;
...
function timelineFingerprint(project: VideoProjectDocumentV1): string {
	return toHex(
		sha256(
			JSON.stringify(
				project.primary_sequence.map((clip) => [
					clip.id,
					clip.source_id,
					clip.source_in_us,
					clip.source_out_us,
