...
For shell work, scripts, or CI, install the [CLI](/automate/cli) with `npm install -g @getopenpost/cli` instead of connecting an MCP client. AI coding assistants can drive it through the [`openpost-cli` skill](https://www.skills.sh/getopenpost/openpost), which carries the safe command patterns for authentication, drafting, validation, scheduling, recovery, and comment moderation.
