...
See [Environment Variables](environment-variables.md) for the JSON schema and legacy variables, [Callback URLs](../callback-urls.md) for exact redirect paths, and [Provider Readiness and Launch Gate](../operations/provider-launch-matrix.md) for the separate approval and live-verification requirements.
