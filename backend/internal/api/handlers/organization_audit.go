...
type AuditFilterInput struct {
	...
	ResourceType string `query:"resource_type" enum:"provider,policy,domain,session,identity,reauthentication,identity_configuration,organization,workspace,workspace_member,workspace_invitation,organization_ownership_transfer,impersonation,billing,mcp_tool_call,publication,publication_authorization,provider_write" doc:"Restrict evidence to one resource type"`
