...
import type { VideoStudioCapabilities } from './types';

type StorageManagerWithDirectory = StorageManager & {
	getDirectory?: () => Promise<FileSystemDirectoryHandle>;
};

export async function detectVideoStudioCapabilities(): Promise<VideoStudioCapabilities> {
	...
	const desktopTimeline = matchMedia('(min-width: 64rem) and (pointer: fine)').matches;
	...
	if (!desktopTimeline)
		reasons.push('The full timeline needs a desktop-sized display and precise pointer.');
