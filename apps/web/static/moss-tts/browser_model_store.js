(function initNanoReaderBrowserModelStore(globalScope) {
  ...
  const MODEL_REPOS = [
    {
      ...
      expectedFileBytes: {
        ...
        "moss_tts_global_shared.data": 440813568,
        "moss_tts_local_shared.data": 229678080,
      },
    },
    {
      ...
      expectedFileBytes: {
        ...
        "moss_audio_tokenizer_encode.data": 44507136,
        "moss_audio_tokenizer_decode_full.onnx": 681902,
        "moss_audio_tokenizer_decode_step.onnx": 351400,
        "moss_audio_tokenizer_decode_shared.data": 44198912,
      },
    },
  ];
  ...
  function parseManagedModelKey(pathValue) {
    ...
    return decodeURIComponent(
      String(pathValue).trim().slice(MANAGED_SCHEME.length),
    );
  }
  ...
  async function getStorageRootDirectory() {
    ...
    if (!storage || typeof storage.getDirectory !== "function") {
      throw new Error(
        "Browser-managed storage is not available in this browser context.",
      );
    }
    ...
    return originRoot.getDirectoryHandle(INTERNAL_ROOT_DIR_NAME, {
      create: true,
    });
  }

  async function getDirectoryByRelativePath(
    rootHandle,
    relativePath,
    create = false,
  ) {
  ...
  async function getOptionalDirectoryHandleByRelativePath(
    rootHandle,
    relativePath,
  ) {
    try {
  ...
  async function getManagedModelRootDirectory(
  ...
  async function getDirectoryHandleByManagedPath(pathValue, create = false) {
    ...
    if (!handle) {
      throw new Error(
        "Browser-managed ONNX model storage is not initialized yet.",
      );
    }
    return handle;
  }
  ...
  async function getFileHandleByRelativePath(
    ...
    const directoryHandle =
      segments.length > 0
        ? await getDirectoryByRelativePath(
            rootHandle,
            segments.join("/"),
            create,
          )
  ...
  async function fileHasExpectedSize(rootHandle, relativePath, expectedBytes) {
    try {
  ...
  async function fetchRepoTree(repoId, revision, accessToken = "") {
    const url = `${HF_API_BASE}${repoId}/tree/${revision}?recursive=1`;
    const response = await fetchWithOptionalToken(url, accessToken);
    if (!response.ok) {
      throw new Error(
        `Failed to fetch Hugging Face repo tree for ${repoId} (status=${response.status}).`,
      );
    }
    const data = await response.json();
    if (!Array.isArray(data)) {
      throw new Error(
        `Unexpected Hugging Face repo tree response for ${repoId}.`,
      );
    }
    return data;
  }
  ...
  function buildRemotePathMap(treeRows, requiredFiles, repoId) {
    ...
    for (const fileName of requiredFiles) {
      if (!fileMap.has(fileName)) {
        throw new Error(
          `Required file ${fileName} was not found in Hugging Face repo ${repoId}.`,
        );
      }
    }
    return fileMap;
  ...
  function buildResolveUrl(repoId, revision, remotePath) {
    const encodedPath = normalizeRelativePath(remotePath)
      .split("/")
      .map(encodeURIComponent)
      .join("/");
    return `${HF_RESOLVE_BASE}${repoId}/resolve/${revision}/${encodedPath}?download=1`;
  }
  ...
  async function downloadRepoFiles(
    ...
    const treeRows = await fetchRepoTree(
      repoSpec.repoId,
      repoSpec.revision,
      accessToken,
    );
    const remotePathMap = buildRemotePathMap(
      treeRows,
      repoSpec.requiredFiles,
      repoSpec.repoId,
    );
    ...
    for (let index = 0; index < repoSpec.requiredFiles.length; index += 1) {
      ...
      const expectedBytes = repoSpec.expectedFileBytes[fileName];
      if (
        await fileHasExpectedSize(
          destinationHandle,
          destinationPath,
          expectedBytes,
        )
        ...
        if (typeof onProgress === "function") {
          onProgress({
            ...
            message: `Using verified ${fileName} (${index + 1}/${totalFiles})...`,
          });
        }
        continue;
      }
      ...
      const response = await fetchWithOptionalToken(
        buildResolveUrl(repoSpec.repoId, repoSpec.revision, remotePath),
        accessToken,
      );
      if (!response.ok) {
        throw new Error(
      ...
      if (
        ...
        throw new Error(
          `Downloaded ${fileName} from ${repoSpec.repoId} has an unexpected size.`,
        );
      }
    }
  }
  ...
  async function hasCompleteExternalModelSet(rootHandle) {
    for (const repoSpec of MODEL_REPOS) {
      for (const fileName of repoSpec.requiredFiles) {
        const valid = await fileHasExpectedSize(
          rootHandle,
          `${repoSpec.localDirName}/${fileName}`,
          repoSpec.expectedFileBytes[fileName],
        );
        if (!valid) {
          return false;
        }
  ...
  async function ensureExternalBrowserOnnxModels({
    ...
    const alreadyComplete =
      !forceDownload && (await hasCompleteExternalModelSet(rootHandle));
