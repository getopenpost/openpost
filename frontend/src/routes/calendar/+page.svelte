<script lang="ts">
	...
	const scheduledCount = $derived(visibleItems.filter((item) => item.status === 'scheduled').length);
	const publishedCount = $derived(visibleItems.filter((item) => item.status === 'published').length);
				...
				{#snippet actions()}
					<div class="flex flex-wrap items-center gap-2">
						...
						<Button
							variant="outline"
							class="gap-1.5"
							onclick={() => goto(resolve('/settings') + '?tab=schedule#posting-schedule')}
						>
						...
						>
							...
							{#each weekDays as day (day.key)}
								...
								>
									...
									<div class="mt-0.5 flex items-center justify-center gap-1.5 text-sm font-semibold">
								...
								{#each weekDays as day (day.key)}
									...
									>
										<button
											...
											onclick={(event) =>
												createPostOnDate(day.date, snappedTime(event, hour))}
										></button>
