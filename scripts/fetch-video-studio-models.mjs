...
const root = path.resolve(process.cwd(), "frontend/static/video-studio-models");
...
await writeFile(
  ...
  `# OpenPost Video Studio local models

These model files are fetched from immutable upstream revisions by
\`scripts/fetch-video-studio-models.mjs\`. The editor does not download them until a user
...
console.log(
  `Video Studio models ready: ${manifest.models
    .map(
      (model) =>
