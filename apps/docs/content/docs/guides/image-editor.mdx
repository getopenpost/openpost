...
Select a version to preview it before restoring. OpenPost saves the current design as a restore point, so you can return to it. If another browser has changed the design, reload the saved version or keep your work as a separate copy before restoring.
