		...
		>
			...
			<div class="relative h-20 border-b" style:width={`${widthPX}px`}>
				{#each derivedClips as item (item.clip_id)}
					...
					<button
						...
						draggable="true"
						class={[
							'absolute top-2 flex h-14 min-w-12 items-center overflow-hidden rounded-md border px-2 text-left text-xs focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none',
					...
					>
						<span class="truncate">{project.sources[clip.source_id]?.original_name ?? clip.id}</span
						>
					</button>
				{/each}
				<div
			...
			<div class="relative h-12 border-b" style:width={`${widthPX}px`}>
				{#each project.visual_tracks.flatMap((track) => track.items) as item (item.id)}
					<div
						class="absolute top-2 h-7 truncate rounded border border-violet-500/30 bg-violet-500/10 px-2 py-1 text-[11px] text-violet-800 dark:text-violet-200"
						style:left={`${clipOffset(item.timeline_start_us)}px`}
			...
			<div class="relative h-12 border-b" style:width={`${widthPX}px`}>
				{#each project.audio_tracks.flatMap( (track) => track.items.map( (item) => ({ ...item, trackName: track.name }) ) ) as item (item.id)}
					<div
						class="absolute top-2 flex h-7 items-center overflow-hidden rounded border border-emerald-500/30 bg-emerald-500/10 px-2 text-[11px] text-emerald-800 dark:text-emerald-200"
						style:left={`${clipOffset(item.timeline_start_us)}px`}
						style:width={`${Math.max(36, clipOffset(item.duration_us))}px`}
					>
			...
			<div class="relative h-14" style:width={`${widthPX}px`}>
				{#each project.caption_tracks.flatMap((track) => track.cues) as cue (cue.id)}
					<div
						class="absolute top-2 h-9 truncate rounded border border-sky-500/30 bg-sky-500/10 px-2 py-1 text-xs text-sky-800 dark:text-sky-200"
