...
func (h *VideoStudioHandler) SetEntitlement(service entitlements.Service) {
	if service != nil {
		h.entitlement = service
	}
}

func (h *VideoStudioHandler) SetStockProviders(providers []string) {
...
type VideoStudioModelManifestItem struct {
	ID                 string   `json:"id"`
...
type VideoStudioConfigOutput struct {
	Body struct {
		Enabled        bool                           `json:"enabled"`
		SchemaVersion  int                            `json:"schema_version"`
		Limits         VideoStudioLimits              `json:"limits"`
		ModelManifest  []VideoStudioModelManifestItem `json:"model_manifest"`
		StockProviders []string                       `json:"stock_providers"`
	}
}

type VideoStudioLimits struct {
...
type VideoStudioSyncSource struct {
...
type PlanVideoStudioSyncInput struct {
	...
	Body        struct {
		...
		Sources   []VideoStudioSyncSource `json:"sources" maxItems:"250"`
	}
}

//nolint:gocyclo // Sync planning keeps validation, workspace scoping, reuse, and quota branches explicit.
func (h *VideoStudioHandler) planSync(
	ctx context.Context,
	input *PlanVideoStudioSyncInput,
) (*PlanVideoStudioSyncOutput, error) {
	...
	if projectID := strings.TrimSpace(input.Body.ProjectID); projectID != "" {
		exists, err := h.db.NewSelect().
			...
			Exists(ctx)
		if err != nil {
			return nil, huma.Error500InternalServerError("failed to validate the Video Studio project")
		}
		if !exists {
			return nil, huma.Error400BadRequest("the Video Studio project does not belong to this workspace")
		}
	}

	out := &PlanVideoStudioSyncOutput{}
	out.Body.Reused = []VideoStudioSyncReuse{}
	...
	for _, source := range input.Body.Sources {
		...
		case err == nil:
			...
			out.Body.Reused = append(out.Body.Reused, VideoStudioSyncReuse{
				SourceID: sourceID,
				MediaID:  media.ID,
			})
		...
		default:
			return nil, huma.Error500InternalServerError("failed to plan Video Studio source reuse")
		}
	}
...
type VideoStudioSyncReuse struct {
	SourceID string `json:"source_id"`
	MediaID  string `json:"media_id"`
}

type VideoStudioSyncStorage struct {
	UsedBytes      int64  `json:"used_bytes"`
	LimitBytes     *int64 `json:"limit_bytes"`
	RemainingBytes *int64 `json:"remaining_bytes"`
}

type PlanVideoStudioSyncOutput struct {
	Body struct {
		Reused           []VideoStudioSyncReuse `json:"reused"`
		MissingSourceIDs []string               `json:"missing_source_ids"`
		AdditionalBytes  int64                  `json:"additional_bytes"`
		Storage          VideoStudioSyncStorage `json:"storage"`
...
func (h *VideoStudioHandler) RegisterRoutes(api huma.API) {
	huma.Register(api, huma.Operation{
		OperationID: "get-video-studio-config",
		Method:      http.MethodGet,
		Path:        "/video-studio/config",
		Summary:     "Get Video Studio browser limits and optional local model manifest",
		Tags:        []string{tagVideoStudio},
	}, h.getConfig)

	auth := huma.Middlewares{middleware.AuthMiddleware(api, h.auth)}
	huma.Register(api, huma.Operation{
		OperationID: "list-video-studio-projects", Method: http.MethodGet,
		Path: "/video-studio/projects", Summary: "List cloud-saved Video Studio projects",
		Tags: []string{tagVideoStudio}, Middlewares: auth, Errors: []int{400, 403},
	}, h.listProjects)
	huma.Register(api, huma.Operation{
		OperationID: "plan-video-studio-sync", Method: http.MethodPost,
		Path: "/video-studio/sync-plan", Summary: "Estimate and deduplicate an explicit Video Studio cloud sync",
		Tags: []string{tagVideoStudio}, Middlewares: auth, Errors: []int{400, 403},
	}, h.planSync)
	huma.Register(api, huma.Operation{
		OperationID: "create-video-studio-project", Method: http.MethodPost,
		Path: "/video-studio/projects", Summary: "Save a local Video Studio project to OpenPost",
		Tags: []string{tagVideoStudio}, Middlewares: auth, Errors: []int{400, 403},
		DefaultStatus: http.StatusCreated,
	}, h.createProject)
	huma.Register(api, huma.Operation{
		OperationID: "get-video-studio-project", Method: http.MethodGet,
		Path: "/video-studio/projects/{id}", Summary: "Get a cloud-saved Video Studio project",
		Tags: []string{tagVideoStudio}, Middlewares: auth, Errors: []int{403, 404},
	}, h.getProject)
	huma.Register(api, huma.Operation{
		OperationID: "update-video-studio-project", Method: http.MethodPatch,
		Path: "/video-studio/projects/{id}", Summary: "Save a Video Studio project with optimistic concurrency",
		Tags: []string{tagVideoStudio}, Middlewares: auth, Errors: []int{400, 403, 404, 409},
	}, h.updateProject)
	huma.Register(api, huma.Operation{
		OperationID: "delete-video-studio-project", Method: http.MethodDelete,
		Path: "/video-studio/projects/{id}", Summary: "Move a Video Studio project to trash",
		Tags: []string{tagVideoStudio}, Middlewares: auth, Errors: []int{403, 404},
	}, h.deleteProject)
	huma.Register(api, huma.Operation{
		OperationID: "list-video-studio-project-revisions", Method: http.MethodGet,
		Path: "/video-studio/projects/{id}/revisions", Summary: "List Video Studio recovery revisions and checkpoints",
		Tags: []string{tagVideoStudio}, Middlewares: auth, Errors: []int{403, 404},
	}, h.listRevisions)
	huma.Register(api, huma.Operation{
		OperationID: "create-video-studio-project-checkpoint", Method: http.MethodPost,
		Path: "/video-studio/projects/{id}/checkpoints", Summary: "Create a named Video Studio checkpoint",
		Tags: []string{tagVideoStudio}, Middlewares: auth, Errors: []int{400, 403, 404},
	}, h.createCheckpoint)
	huma.Register(api, huma.Operation{
		OperationID: "restore-video-studio-project-revision", Method: http.MethodPost,
		Path:    "/video-studio/projects/{id}/revisions/{revision_id}/restore",
		Summary: "Restore a Video Studio revision as a new head",
		Tags:    []string{tagVideoStudio}, Middlewares: auth, Errors: []int{400, 403, 404, 409},
	}, h.restoreRevision)
	huma.Register(api, huma.Operation{
		OperationID: "create-video-studio-return-token", Method: http.MethodPost,
		Path: "/video-studio/return-tokens", Summary: "Create a one-time Video Studio composer return token",
		Tags: []string{tagVideoStudio}, Middlewares: auth, Errors: []int{400, 403},
		DefaultStatus: http.StatusCreated,
	}, h.createReturnToken)
	huma.Register(api, huma.Operation{
		OperationID: "complete-video-studio-return-token", Method: http.MethodPost,
		Path:    "/video-studio/return-tokens/{token}/complete",
		Summary: "Store Video Studio variant-to-rendition export assignments",
		Tags:    []string{tagVideoStudio}, Middlewares: auth, Errors: []int{400, 403, 404, 409},
	}, h.completeReturnToken)
	huma.Register(api, huma.Operation{
		OperationID: "consume-video-studio-return-token", Method: http.MethodPost,
		Path:    "/video-studio/return-tokens/{token}/consume",
		Summary: "Consume completed Video Studio export assignments once",
		Tags:    []string{tagVideoStudio}, Middlewares: auth, Errors: []int{400, 403, 404, 409},
	}, h.consumeReturnToken)
}

func (h *VideoStudioHandler) getConfig(
	_ context.Context,
	_ *struct{},
) (*VideoStudioConfigOutput, error) {
	out := &VideoStudioConfigOutput{}
	...
	out.Body.ModelManifest = []VideoStudioModelManifestItem{
		{
			...
			Resumable:        true, LicenseReference: "video-studio-models/THIRD_PARTY_LICENSES.md",
...
func (h *VideoStudioHandler) ensureEnabled() error {
	if !h.enabled {
		return huma.Error404NotFound("OpenPost Video Studio is disabled")
	}
	return nil
}

func (h *VideoStudioHandler) requireAccess(
	ctx context.Context,
	workspaceID string,
	edit bool,
) (bool, error) {
...
func (h *VideoStudioHandler) listProjects(
	ctx context.Context,
	...
	if err != nil {
		return nil, huma.Error500InternalServerError("failed to count Video Studio projects")
	}
	limit := input.Limit
	...
	if err := query.OrderExpr("updated_at DESC").Limit(limit).Offset(input.Offset).Scan(ctx, &projects); err != nil {
		return nil, huma.Error500InternalServerError("failed to list Video Studio projects")
	}
...
func (h *VideoStudioHandler) createProject(
	ctx context.Context,
	...
	if err != nil {
		return nil, huma.Error400BadRequest("Video Studio project cannot be serialized")
	}
	now := time.Now().UTC()
	...
	if err != nil {
		...
		log.Printf("failed to create Video Studio project %s: %v", projectID, err)
		return nil, huma.Error500InternalServerError("failed to create Video Studio project")
	}
...
func (h *VideoStudioHandler) getProject(
	ctx context.Context,
...
func (h *VideoStudioHandler) updateProject(
	ctx context.Context,
	...
	if err != nil {
		log.Printf("failed to save Video Studio project %s: %v", project.ID, err)
		return nil, huma.Error500InternalServerError("failed to save Video Studio project")
	}
...
func (h *VideoStudioHandler) persistProjectUpdate(
	ctx context.Context,
	project *models.VideoProject,
...
func videoProjectConflict(latestRevision int) error {
	return huma.NewError(
		http.StatusConflict,
		fmt.Sprintf("Video Studio project changed elsewhere; latest revision is %d", latestRevision),
	)
}

func (h *VideoStudioHandler) deleteProject(
	ctx context.Context,
	...
	if err != nil {
		return nil, huma.Error500InternalServerError("failed to delete Video Studio project")
	}
	affected, _ := result.RowsAffected()
	if affected != 1 {
		return nil, huma.Error404NotFound("Video Studio project not found")
	}
...
func (h *VideoStudioHandler) listRevisions(
	ctx context.Context,
	...
	if err := h.db.NewSelect().Model(&revisions).
		...
		return nil, huma.Error500InternalServerError("failed to list Video Studio revisions")
	}
...
func (h *VideoStudioHandler) createCheckpoint(
	ctx context.Context,
	...
	if err != nil {
		return nil, huma.Error500InternalServerError("Video Studio project is corrupt")
	}
	now := time.Now().UTC()
	revision, err := newVideoProjectRevision(project, document, "checkpoint", name, now)
	if err != nil {
		return nil, huma.Error500InternalServerError("failed to create Video Studio checkpoint")
	}
...
func (h *VideoStudioHandler) restoreRevision(
	ctx context.Context,
	...
	if errors.Is(err, sql.ErrNoRows) {
		return nil, huma.Error404NotFound("Video Studio revision not found")
	}
	if err != nil {
		return nil, huma.Error500InternalServerError("failed to load Video Studio revision")
	}
	document, err := decompressVideoProjectSnapshot(revision.Snapshot)
	if err != nil || videoproject.Validate(document, true) != nil {
		return nil, huma.Error400BadRequest("Video Studio revision is invalid")
	}
...
func (h *VideoStudioHandler) createReturnToken(
	ctx context.Context,
	...
	if err != nil || len(constraints) > 64<<10 {
		return nil, huma.Error400BadRequest("Video Studio return constraints are invalid")
	}
	rawToken, tokenHash, err := newVideoReturnToken()
	if err != nil {
		return nil, huma.Error500InternalServerError("failed to create Video Studio return token")
	}
	now := time.Now().UTC()
	...
	if _, err := h.db.NewInsert().Model(record).Exec(ctx); err != nil {
		return nil, huma.Error500InternalServerError("failed to store Video Studio return token")
	}
...
func (h *VideoStudioHandler) completeReturnToken(
	ctx context.Context,
	...
	if !record.CompletedAt.IsZero() || !record.ConsumedAt.IsZero() {
		return nil, huma.NewError(http.StatusConflict, "Video Studio return token has already been used")
	}
	if strings.TrimSpace(input.Body.ProjectID) == "" || len(input.Body.Exports) == 0 || len(input.Body.Exports) > 4 {
		return nil, huma.Error400BadRequest("Video Studio return result must include a project and 1–4 exports")
	}
	project, err := h.loadProject(ctx, input.Body.ProjectID)
	if err != nil || project.WorkspaceID != record.WorkspaceID {
		return nil, huma.Error400BadRequest("Video Studio project must belong to the workspace")
	}
	...
	for _, export := range input.Body.Exports {
		if !validVideoVariant(export.VariantID) {
			return nil, huma.Error400BadRequest("Video Studio export variant is invalid")
		}
		if _, exists := seenVariants[export.VariantID]; exists {
			return nil, huma.Error400BadRequest("Video Studio export variants must be unique")
		}
		...
		for _, renditionID := range export.RenditionIDs {
			if len(allowedRenditions) > 0 {
				if _, allowed := allowedRenditions[renditionID]; !allowed {
					return nil, huma.Error400BadRequest("Video Studio export references an unexpected rendition")
				}
			}
		}
	}
	for required := range requiredVariants {
		if _, ok := seenVariants[required]; !ok {
			return nil, huma.Error400BadRequest("Video Studio exports do not cover every required format")
		}
	}
	var media []models.MediaAttachment
	if err := h.db.NewSelect().Model(&media).
		...
		Scan(ctx); err != nil {
		return nil, huma.Error500InternalServerError("failed to validate Video Studio exports")
	}
	if len(media) != len(mediaIDs) {
		return nil, huma.Error400BadRequest("every Video Studio export must be ready and belong to the workspace")
	}
	...
	for _, export := range input.Body.Exports {
		...
		if !ok || !strings.HasPrefix(strings.ToLower(item.MimeType), "video/") {
			return nil, huma.Error400BadRequest("every Video Studio export must be a video")
		}
		if item.VideoProjectID != input.Body.ProjectID {
			return nil, huma.Error400BadRequest("every Video Studio export must come from the returning project")
		}
		if len(allowedMIMEs) > 0 {
			if _, allowed := allowedMIMEs[strings.ToLower(item.MimeType)]; !allowed {
				return nil, huma.Error400BadRequest("a Video Studio export format is not supported")
			}
		}
		if constraints.MaxWidth > 0 && item.Width > constraints.MaxWidth ||
			constraints.MaxHeight > 0 && item.Height > constraints.MaxHeight {
			return nil, huma.Error400BadRequest("a Video Studio export exceeds the destination dimensions")
		}
		if constraints.MaxDurationMS > 0 && item.DurationMS > constraints.MaxDurationMS {
			return nil, huma.Error400BadRequest("a Video Studio export exceeds the destination duration")
		}
		if constraints.MaxFileSizeBytes > 0 && item.Size > constraints.MaxFileSizeBytes {
			return nil, huma.Error400BadRequest("a Video Studio export exceeds the destination file size")
		}
		if constraints.MaxFPS > 0 && item.FrameRate > constraints.MaxFPS {
			return nil, huma.Error400BadRequest("a Video Studio export exceeds the destination frame rate")
		}
		if export.Width != item.Width || export.Height != item.Height ||
			absInt64(export.DurationMS-item.DurationMS) > 100 {
			return nil, huma.Error400BadRequest("Video Studio export metadata does not match the stored media")
		}
	}
	encoded, err := json.Marshal(input.Body)
	if err != nil {
		return nil, huma.Error400BadRequest("Video Studio return result is invalid")
	}
	now := time.Now().UTC()
	...
	if err != nil {
		return nil, huma.Error500InternalServerError("failed to complete Video Studio return token")
	}
	affected, _ := result.RowsAffected()
	if affected != 1 {
		return nil, huma.NewError(http.StatusConflict, "Video Studio return token has already been used")
...
func (h *VideoStudioHandler) consumeReturnToken(
	ctx context.Context,
	...
	if record.CompletedAt.IsZero() {
		return nil, huma.NewError(http.StatusConflict, "Video Studio return token is not complete")
	}
	if !record.ConsumedAt.IsZero() {
		return nil, huma.NewError(http.StatusConflict, "Video Studio return token has already been consumed")
	}
	result, err := h.db.NewUpdate().Model((*models.VideoReturnToken)(nil)).
		Set("consumed_at = ?", time.Now().UTC()).
		Where("id = ? AND consumed_at IS NULL", record.ID).Exec(ctx)
	if err != nil {
		return nil, huma.Error500InternalServerError("failed to consume Video Studio return token")
	}
	affected, _ := result.RowsAffected()
	if affected != 1 {
		return nil, huma.NewError(http.StatusConflict, "Video Studio return token has already been consumed")
	...
	if err := json.Unmarshal([]byte(record.ResultJSON), &out.Body.Result); err != nil {
		return nil, huma.Error500InternalServerError("Video Studio return token result is corrupt")
	}
	return out, nil
}

func (h *VideoStudioHandler) loadReturnToken(
	ctx context.Context,
	rawToken string,
) (*models.VideoReturnToken, error) {
	if strings.TrimSpace(rawToken) == "" {
		return nil, huma.Error400BadRequest("Video Studio return token is required")
	}
	...
	err := h.db.NewSelect().Model(&record).
		...
		Scan(ctx)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, huma.Error404NotFound("Video Studio return token not found")
	}
	if err != nil {
		return nil, huma.Error500InternalServerError("failed to load Video Studio return token")
	}
	if time.Now().UTC().After(record.ExpiresAt) {
		return nil, huma.Error404NotFound("Video Studio return token has expired")
	}
...
func (h *VideoStudioHandler) loadProject(ctx context.Context, id string) (*models.VideoProject, error) {
	...
	if err != nil {
		return nil, huma.Error500InternalServerError("failed to load Video Studio project")
	}
	return &project, nil
}

func (h *VideoStudioHandler) projectResponse(
	ctx context.Context,
	id string,
...
func (h *VideoStudioHandler) validateMediaReferences(
	ctx context.Context,
	workspaceID string,
	document videoproject.Document,
	...
	count, err := h.db.NewSelect().Model((*models.MediaAttachment)(nil)).
		...
		Count(ctx)
	if err != nil {
		return huma.Error500InternalServerError("failed to validate Video Studio media")
	}
	if count != len(stringSet(ids)) {
		return huma.Error400BadRequest("every Video Studio source must be ready and belong to the workspace")
	}
	return nil
}
