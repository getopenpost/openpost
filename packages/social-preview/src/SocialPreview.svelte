<script lang="ts">
	...
	let warningRevealed = $state(false);
	...
	const activeMedia = $derived(
		primaryMedia[Math.min(carouselIndex, Math.max(0, primaryMedia.length - 1))]
	);
	...
	function choosePreviousMedia() {
		carouselIndex = (carouselIndex - 1 + primaryMedia.length) % primaryMedia.length;
	}

	function chooseNextMedia() {
		carouselIndex = (carouselIndex + 1) % primaryMedia.length;
	}
</script>
		...
		<div class={['media-stage', classes]}>
			...
			{#if primaryMedia.length > 1}
				...
				<span class="carousel-count">
					{carouselIndex + 1}/{primaryMedia.length}
				</span>
			{/if}
		</div>
	{:else if isVertical || model.format === 'video' || model.format === 'photo'}
...
{#snippet postContent(text: string, showExtras = true)}
	{#if showExtras && model.contentWarning && !warningRevealed}
		<div class="content-warning">
			<div>
				<strong>Content warning</strong>
				<span>{model.contentWarning}</span>
			</div>
			<button type="button" onclick={() => (warningRevealed = true)}> Show post </button>
		</div>
	{:else}
		<p class="post-copy">{text || 'Your post will appear here.'}</p>
	{/if}
	{#if showExtras && model.card}{@render card(model.card)}{/if}
	{#if showExtras && model.poll}{@render poll(model.poll)}{/if}
{/snippet}
...
<style>
	.social-preview {
		...
		box-shadow: 0 18px 50px -38px rgb(0 0 0 / 45%);
	}
