...
Instance administrators can manage account policy, Paddle billing, authentication, email, OpenPost Image Editor, meme generation, stock media, feedback, provider behavior, and provider-app settings in **Settings → Instance → Configuration**. OpenPost encrypts every database-backed value with `OPENPOST_ENCRYPTION_KEY`; secret values are write-only and the API only reports whether one exists.
