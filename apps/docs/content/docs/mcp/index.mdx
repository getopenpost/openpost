...
For a client not listed here, use the endpoint and authentication rules in [Tools and permissions](/mcp/tools). That page also explains the server's tool modes: operations are listed directly by default, with the previous search-first flow still available.
