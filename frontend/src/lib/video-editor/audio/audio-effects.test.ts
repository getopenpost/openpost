...
describe('audio effects rack offline', () => {
	...
	it('delay tail exists after dry input', () => {
		...
		const out = applyAudioEffectStages([impulse.slice(), new Float32Array(impulse.length)], 48000, [delay]);
		const tailStart = 100 + Math.round((120 / 1000) * 48000) + 5;
		const tailEnergy = out[0]!.slice(tailStart, tailStart + 500).reduce((s, v) => s + Math.abs(v), 0);
	...
	it('reverb tail exists after dry input', () => {
		...
		const out = applyAudioEffectStages([impulse.slice(), new Float32Array(impulse.length)], 48000, [reverb]);
	...
	it('stacked chain preview/export parity: streaming equals one-shot', () => {
		...
		const oneShot = applyAudioEffectStages(channels.map((c) => c.slice()), 48000, chain);
		...
		let err = 0;
		for (let i = 0; i < stitched[0]!.length; i++) err += Math.abs(stitched[0]![i]! - oneShot[0]![i]!);
