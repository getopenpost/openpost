<script lang="ts">
	...
	import {
		publicStudioPageCountBucket,
		trackPublicStudioEvent
		...
		<div class="flex items-center gap-1">
			...
			<DropdownMenu.Root>
				...
				<DropdownMenu.Content align="end">
					...
					>
					{#if guestMode}
						<DropdownMenu.Item onclick={() => onSaveToOpenPost?.()}
							>{m.studio_public_save_openpost()}</DropdownMenu.Item
						>
					{:else}
						<DropdownMenu.Item onclick={openHistory}
							>{m.studio_version_history()}</DropdownMenu.Item
						>
						<DropdownMenu.Item
							onclick={() => (checkpointDialogOpen = true)}
