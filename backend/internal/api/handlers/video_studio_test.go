...
func newVideoStudioHandlerTest(t *testing.T) (*VideoStudioHandler, context.Context) {
	t.Helper()
	...
	require.NoError(t, err)
	return NewVideoStudioHandler(db, testAuthenticator{}, true, "/video-studio-models"), ctx
}

func TestVideoReturnTokenCompletesAndConsumesExactlyOnce(t *testing.T) {
	t.Parallel()
	handler, ctx := newVideoStudioHandlerTest(t)
	now := time.Now().UTC()
	...
	_, err = handler.db.NewInsert().Model(&models.MediaAttachment{
		...
		OriginalFilename: "portrait.mp4", FileHash: "hash-1", Source: "video_studio_export",
...
func TestVideoStudioSyncPlanReusesWorkspaceMediaAndCountsMissingHashesOnce(t *testing.T) {
	t.Parallel()
	handler, ctx := newVideoStudioHandlerTest(t)
	...
	input := &PlanVideoStudioSyncInput{WorkspaceID: "workspace-1"}
	input.Body.Sources = []VideoStudioSyncSource{
	...
	require.Equal(t, []VideoStudioSyncReuse{{SourceID: "reused", MediaID: "media-reused"}}, result.Body.Reused)
