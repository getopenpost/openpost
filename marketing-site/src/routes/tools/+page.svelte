...
<section class="border-b py-14 sm:py-20">
  <div
    class="marketing-shell grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end"
  >
    <div>
    ...
    <div>
      <p class="marketing-copy">
        Preview posts, check limits, split copy, prepare media, and plan a
        weekly schedule in your browser.
      </p>
      <p
        class="mt-4 inline-flex items-center gap-2 text-sm text-muted-foreground"
      >
...
<section class="section-pad">
  <div class="marketing-shell grid gap-16">
    {#each groups as group (group.title)}
      <section
        aria-labelledby={`tool-group-${group.title.toLowerCase().replaceAll(" ", "-")}`}
      >
        <div
          class="grid gap-2 border-b pb-5 sm:grid-cols-[0.55fr_1.45fr] sm:items-end"
        >
          <h2
          ...
          {#each toolsForGroup(group.slugs) as tool (tool.slug)}
            ...
            >
              ...
              <span>
                ...
                <span
                  class="mt-1 block max-w-2xl text-sm leading-6 text-muted-foreground"
                >
                  {tool.description}
                </span>
              </span>
