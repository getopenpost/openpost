...
```sh
devenv shell -- bash -lc 'cd apps/cli && go build ./cmd/openpost'
```

The binary is written to `apps/cli/openpost`.
...
```sh
devenv shell -- bash -lc 'cd apps/cli && go install ./cmd/openpost'
devenv shell -- bash -lc 'cd apps/cli && go install ./cmd/openpost-mcp'
```
