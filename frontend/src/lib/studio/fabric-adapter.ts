...
export class OpenPostFabricAdapter {
	...
	private layerLocalGeometry(
		...
		return {
			...
			height,
			scale: Math.sqrt(
				Math.max(0.0001, Math.abs(decomposition.scaleX * decomposition.scaleY))
			)
		};
	}
	...
	private bindEvents(): void {
		...
		canvas.on('object:rotating', (event) => {
			...
			if ((event.e as MouseEvent).shiftKey) {
				target.angle = Math.round((target.angle ?? 0) / 15) * 15;
				target.setCoords();
			}
			this.syncDecorationTransform(target);
		});
	...
	private transformEntries(
		...
		if (!target.__studioLayerID && 'getObjects' in target) {
			...
			return selection
				.getObjects()
				.filter(
					(object): object is FabricObject & { __studioLayerID: string } =>
	...
	private createMaskClip(layer: StudioLayer, object: FabricObject): FabricObject | undefined {
		...
		const width = Math.max(
			1,
			objectWidth - ((layer.mask?.inset ?? 0) * 2) / scaleX
		);
		const height = Math.max(
			1,
			objectHeight - ((layer.mask?.inset ?? 0) * 2) / scaleY
		);
		const common = {
			left: 0,
			top: 0,
	...
	private applyImageFilters(
		...
		if (vibrance) {
			filters.push(
				new this.fabric.filters.Vibrance({ vibrance: clamp(vibrance, -1, 1) })
			);
		}
		...
		if (hue) {
			filters.push(
				new this.fabric.filters.HueRotation({ rotation: clamp(hue, -1, 1) })
			);
		}
