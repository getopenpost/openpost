...
describe('Studio image geometry', () => {
	...
	it('recomputes fitted object geometry after a frame aspect-ratio change', () => {
		const previous = imageLayer(800, 800);
		const next = imageLayer(1200, 600);
		...
		const internals = adapter as unknown as {
			fabric: object;
			updateObject(object: typeof object, previous: StudioLayer, next: StudioLayer): void;
		};
		internals.fabric = {};

		internals.updateObject(object, previous, next);

		expect(object).toMatchObject(computeImageGeometry(next, 1920, 1080));
		expect(object.scaleX).toBeCloseTo(object.scaleY);
	});
});
