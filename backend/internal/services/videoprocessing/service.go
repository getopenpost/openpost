...
func (s *Service) EnqueuePendingAnalysis(ctx context.Context) error {
	...
	if err := s.db.NewSelect().
		...
		Where("analysis_status != ? OR width <= 0 OR height <= 0 OR duration_ms <= 0", statusReady).
...
func (s *Service) analyze(ctx context.Context, mediaID string) error {
	...
	if err != nil {
		return s.failAnalysis(ctx, media.ID, fmt.Errorf("stage video for analysis: %w", err))
	}
	...
	if result.AnalysisStatus != "" && result.AnalysisStatus != mediaanalysis.AnalysisStatusReady {
		return s.failAnalysis(ctx, media.ID, errors.New(firstNonEmpty(result.AnalysisError, "video analysis did not complete")))
	}
	if result.Width <= 0 || result.Height <= 0 || result.DurationMS <= 0 {
		return s.failAnalysis(ctx, media.ID, errors.New("video analysis returned incomplete dimensions or duration"))
	}
	...
	if len(result.PosterContent) > 0 {
		...
		_, saveErr := mediastore.SaveWithContentType(s.storage, key, bytes.NewReader(result.PosterContent), "image/jpeg")
		if saveErr != nil {
			return s.failAnalysis(ctx, media.ID, fmt.Errorf("save video poster: %w", saveErr))
		}
		posterKey = key
	}
