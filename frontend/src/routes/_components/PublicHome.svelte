<!--
THESIS: Make OpenPost and its price legible before sign-in without duplicating the full marketing site.
STORY: A visitor understands the product, sees every plan, and can start a trial or inspect the full product site.
FIRST VIEWPORT: Brand and sign-in sit above the offer; entry price and trial terms stay beside the primary action.
-->
<script lang="ts">
...
<svelte:head>
	...
	<link rel="canonical" href="https://app.openpo.st/" />
</svelte:head>

<div
	class="min-h-screen bg-background text-foreground selection:bg-primary/20 selection:text-foreground"
>
			...
			<div class="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
				<div class="flex flex-col justify-between gap-4 border-b pb-6 sm:flex-row sm:items-end">
					...
					<a
						...
						class="inline-flex min-h-11 items-center gap-1.5 rounded-md text-sm font-semibold text-primary underline-offset-4 hover:underline focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:outline-none"
					>
	...
	<footer class="border-t bg-muted/20">
		...
		>
			...
			<nav class="flex flex-wrap gap-x-5 gap-y-2" aria-label={m.public_home_legal_label()}>
				<a
					class="inline-flex min-h-11 items-center rounded-md underline-offset-4 hover:text-foreground hover:underline focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
