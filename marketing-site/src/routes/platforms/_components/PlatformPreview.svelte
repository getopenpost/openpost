      ...
      ? platform.preview.chips.map((chip, index) => ({
          ...
          text:
            index === 0
              ...
              : `${chip}: custom reply text for this account.`,
        }))
  ...
  const model = $derived(
    createPreviewModel({
      ...
      identity: {
        ...
        handle:
          platform.slug === "mastodon"
            ? "openpost@mastodon.social"
            : "openpost",
      },
      segments,
      contentWarning:
        platform.slug === "mastodon"
          ? "Product update and publishing details"
          : undefined,
      card:
        platform.slug === "bluesky"
          ...
          : undefined,
      title:
        platform.slug === "youtube" ? platform.preview.headline : undefined,
      subtitle:
        platform.slug === "youtube" ? "OpenPost product tour" : undefined,
    }),
  );
</script>
