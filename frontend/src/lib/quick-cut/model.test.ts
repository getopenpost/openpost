...
describe('quick-cut model', () => {
	it('detects overlap per source and validates segments', () => {
		...
		const errors = validateSegments([a, b], 10, [
			// SAFETY: type assertion is safe for this quick-cut path
			{ id: SRC_A, duration: 10 } as never,
			// SAFETY: type assertion is safe for this quick-cut path
			{ id: SRC_B, duration: 10 } as never
		]);
		expect(errors.some((e) => e.kind === 'overlap')).toBe(true);
		// Different sources should not overlap
	...
	it('rejects invalid ranges beyond duration per source', () => {
		...
		// SAFETY: type assertion is safe for this quick-cut path
		const errs = validateSegments([seg], 10, [{ id: SRC_A, duration: 10 } as never]);
