<script lang="ts">
	...
	function displayPageName(name: string, index: number): string {
		return /^Page \d+$/.test(name) ? m.studio_default_page_name({ number: index + 1 }) : name;
	}
...
<div class="border-t bg-background/95 backdrop-blur">
	<div class="flex h-11 items-center gap-1 border-b px-2 lg:h-9">
		<Button
			...
			aria-label={editor.pagesExpanded ? m.studio_collapse_pages() : m.studio_expand_pages()}
		>
			{#if editor.pagesExpanded}<ChevronDownIcon />{:else}<ChevronUpIcon />{/if}
		</Button>
		<span class="text-xs font-medium">{m.studio_pages()}</span>
		...
		<div class="ml-auto flex gap-1">
			...
			>
				<PlusIcon />
				<span class="sr-only">{m.studio_add_page()}</span>
			</Button>
			<Button
				variant="ghost"
				size="icon-xs"
			...
			>
				<CopyIcon />
				<span class="sr-only">{m.studio_duplicate_page()}</span>
			</Button>
			<Button
				variant="ghost"
				size="icon-xs"
			...
			>
				<TrashIcon />
				<span class="sr-only">{m.studio_delete_page()}</span>
			</Button>
		</div>
	</div>
		...
		<div class="no-scrollbar flex h-24 items-center gap-2 overflow-x-auto px-3 py-2">
			{#each editor.document.pages as page, index (page.id)}
				<button
					...
					aria-label={m.studio_page_label({
						number: index + 1,
						name: displayPageName(page.name, index)
					})}
					aria-keyshortcuts="Alt+ArrowLeft Alt+ArrowRight"
					title={m.studio_page_reorder_hint()}
				>
