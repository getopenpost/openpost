<script lang="ts">
	...
	let presets = $state<StudioPreset[]>([]);
	...
	async function initialize(): Promise<void> {
		...
		try {
			...
			if (workspaceID && enabled) templates = await listStudioTemplates(workspaceID);
	...
	async function createPreset(key: string): Promise<void> {
		...
		try {
			const design = await createStudioDesign(workspaceID, {
				title: m.studio_untitled_design(),
				preset_key: key,
	...
	async function createTemplate(template: StudioTemplate): Promise<void> {
		if (!workspaceID || creating) return;
		creating = template.id;
		error = '';
		try {
			const design = await instantiateStudioTemplate(template.id, workspaceID);
			await openDesign(design.id);
		} catch (cause) {
	...
	function templateCategory(template: StudioTemplate): string {
		if (!template.built_in) return template.category;
		switch (template.category) {
			case 'Announcements':
				return m.studio_template_category_announcements();
			case 'Photo-led':
				return m.studio_template_category_photo();
			case 'Quotes':
				return m.studio_template_category_quotes();
			case 'Education':
				return m.studio_template_category_education();
			case 'Stories':
				return m.studio_template_category_stories();
			case 'Professional':
				return m.studio_template_category_professional();
			case 'Thumbnails':
				return m.studio_template_category_thumbnails();
			default:
...
<svelte:head><title>{m.studio_new_design()} · {m.studio_title()}</title></svelte:head>

<div class="studio-theme min-h-dvh bg-background">
	<header
	...
	<main class="mx-auto max-w-6xl px-4 py-8 sm:px-6">
		...
		{:else if !enabled}
			<div class="mx-auto max-w-lg rounded-2xl border bg-card p-8 text-center">
				...
				<h1 class="text-xl font-semibold">{m.studio_not_enabled()}</h1>
				<p class="mt-2 text-sm text-muted-foreground">
					{m.studio_not_enabled_body()}
				</p>
				<Button class="mt-5" onclick={() => goto(resolve('/media'))}
					>{m.studio_return_media()}</Button
				>
			</div>
		{:else}
			...
			<section class="mt-8 rounded-2xl border bg-card p-4" aria-labelledby="custom-heading">
				<h2 id="custom-heading" class="text-sm font-semibold">{m.studio_custom_size()}</h2>
				<p class="mt-1 text-xs text-muted-foreground">{m.studio_custom_limits()}</p>
			...
			<section class="mt-10" aria-labelledby="templates-heading">
				<div class="mb-3">
					...
					<p class="text-xs text-muted-foreground">
						{m.studio_starter_templates_body()}
					</p>
				</div>
				<div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
