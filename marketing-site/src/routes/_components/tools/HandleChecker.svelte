<script lang="ts">
	...
	import { Check, ClipboardCopy, ExternalLink, Globe2, LoaderCircle, SearchCheck } from 'lucide-svelte';
	...
	type LiveResult =
		...
		| { status: 'found'; message: string; identity?: string; profileUrl?: string }
		| { status: 'not-found'; message: string };

	let handle = $state('@alice@mastodon.social');
	let live = $state<LiveResult>({ status: 'idle', message: 'No network request has been made.' });
	...
	async function checkLive() {
		...
		try {
			...
			const subject =
				typeof data === 'object' && data !== null && 'subject' in data && typeof data.subject === 'string'
					? data.subject
					: '';
			const links =
				typeof data === 'object' && data !== null && 'links' in data && Array.isArray(data.links)
					...
					: [];
			const aliases =
				typeof data === 'object' && data !== null && 'aliases' in data && Array.isArray(data.aliases)
					? data.aliases
					: [];
...
<div class="mt-8 grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(22rem,1.1fr)]">
	<section class="rounded-lg border bg-card p-4 sm:p-6" aria-labelledby="handle-input-title">
		...
		<label for="social-handle" class="mt-6 block text-sm font-medium">Fediverse or Bluesky handle</label>
		<input
			id="social-handle"
			value={handle}
			oninput={(event) => updateHandle(event.currentTarget.value)}
			class="mt-2 min-h-12 w-full rounded-xl border bg-background px-4 font-mono text-sm focus-visible:ring-2 focus-visible:ring-ring"
		...
		<div class="mt-4 rounded-xl border bg-muted/20 p-4">
			<div class="flex items-start gap-3">
				...
				<div class="min-w-0">
					...
					<p class="mt-1 text-sm leading-6 text-muted-foreground">{parsed.message}</p>
					{#if parsed.valid}
		...
		<div class="mt-4 flex flex-wrap gap-2">
			<Button type="button" disabled={!parsed.valid || live.status === 'loading'} onclick={checkLive}>
		...
		>
			...
			{#if live.status === 'found' && live.identity}
				<div class="mt-5 rounded-lg border bg-background p-4">
					<p class="text-xs font-medium text-muted-foreground">Resolved identity</p>
			...
			{#if parsed.valid}
				<div class="mt-5 flex flex-wrap gap-3 text-sm font-medium">
					{#if resultProfileUrl}
						...
						>
							{resultProfileLabel} <ExternalLink class="size-4" />
						</a>
					{/if}
					{#if parsed.lookupUrl}
						<a
