/* oxlint-disable anti-slop/no-unknown-parameters, anti-slop/require-safety-comment-for-type-assertion, anti-slop/no-conditional-empty-object-spread -- Kept structurally aligned with FreeCut's audited MIT EQ model and legacy field migration. */
import type {
	AudioEqBand1Type,
	AudioEqBand6Type,
...
function isAudioEqSourceDisabled(source?: AudioEqSettings | AudioEqFieldSource | null): boolean {
...
export function getSparseAudioEqSettings(source?: AudioEqFieldSource | null): AudioEqSettings {
	...
	if (hasLegacyBand1Patch) {
		...
		if (
			source?.audioEqBand1SlopeDbPerOct === undefined &&
			sparse.lowCutSlopeDbPerOct !== undefined
		) {
	...
	if (hasLegacyBand6Patch) {
		...
		if (
			source?.audioEqBand6SlopeDbPerOct === undefined &&
			sparse.highCutSlopeDbPerOct !== undefined
		) {
...
export function resolveAudioEqSettings(
	...
	const band1EnabledSource = getSettingsValue(
		settings,
		fields,
		'band1Enabled',
		'audioEqBand1Enabled'
	...
	const band1FrequencySource = getSettingsValue(
		settings,
		fields,
		'band1FrequencyHz',
	...
	const band1SlopeSource = getSettingsValue(
		settings,
		fields,
		'band1SlopeDbPerOct',
	...
	const legacyHighCutSlopeDbPerOct = clampAudioEqCutSlopeDbPerOct(
		Number(
			getSettingsValue(settings, fields, 'highCutSlopeDbPerOct', 'audioEqHighCutSlopeDbPerOct')
		),
		DEFAULT_AUDIO_EQ_SETTINGS.highCutSlopeDbPerOct
	);
	const band6EnabledSource = getSettingsValue(
		settings,
		fields,
		'band6Enabled',
		'audioEqBand6Enabled'
	...
	const band6FrequencySource = getSettingsValue(
		settings,
		fields,
		'band6FrequencyHz',
	...
	const band6SlopeSource = getSettingsValue(
		settings,
		fields,
		'band6SlopeDbPerOct',
...
export function normalizeAudioEqSettings(
	...
	return {
		...(source && 'enabled' in source && source.enabled !== undefined
...
export function resolvePreviewAudioEqStages(
	...
	const stages = baseStages && baseStages.length > 0 ? [...baseStages] : [resolveAudioEqSettings()];
...
export function isAudioEqStageActive(
	...
	return (
		...
		(!!stage.band1Enabled &&
			stage.band1Type !== 'high-pass' &&
			Math.abs(stage.band1GainDb ?? 0) > AUDIO_EQ_ACTIVE_EPSILON) ||
		...
		(!!stage.band6Enabled &&
			stage.band6Type !== 'low-pass' &&
			Math.abs(stage.band6GainDb ?? 0) > AUDIO_EQ_ACTIVE_EPSILON)
	);
}
...
export const AUDIO_EQ_PRESETS: ReadonlyArray<AudioEqPresetDefinition> = Object.freeze([
	...
	definePreset('telephone', 'Telephone', {
		outputGainDb: 5,
		lowCutEnabled: true,
		lowCutFrequencyHz: 320,
		lowCutSlopeDbPerOct: 24,
		lowGainDb: -8,
		lowFrequencyHz: 240,
		lowMidGainDb: -3.5,
		lowMidFrequencyHz: 520,
		lowMidQ: 1.15,
		highMidGainDb: 4.5,
		highMidFrequencyHz: 1700,
		highMidQ: 1.05,
		highGainDb: -9,
		highFrequencyHz: 3600,
		highCutEnabled: true,
		highCutFrequencyHz: 3200,
...
export function getAudioEqPresetById(
	presetId: AudioEqPresetId
): AudioEqPresetDefinition | undefined {
...
export function findAudioEqPresetId(
	...
	const preset = AUDIO_EQ_PRESETS.find(({ settings }) =>
		areAudioEqStagesEqual([settings], [resolved])
	);
...
function evaluateBiquadMagnitudeDb(
	...
	const numeratorReal =
		coefficients.b0 + coefficients.b1 * cosOmega + coefficients.b2 * cosDoubleOmega;
...
function getResolvedAudioEqResponseGainDb(
	...
	if (stage.band1Enabled && isPassBand1Type(stage.band1Type)) {
		const coefficients = buildOnePolePassCoefficients(
			'highpass',
	...
	if (
		stage.lowEnabled &&
		(stage.lowType === 'notch' || Math.abs(stage.lowGainDb) > AUDIO_EQ_ACTIVE_EPSILON)
	) {
		gainDb += evaluateBiquadMagnitudeDb(
			buildEqBiquadCoefficients(
				getInnerBandBiquadType(stage.lowType),
				stage.lowFrequencyHz,
				stage.lowGainDb,
				sampleRate,
				stage.lowQ
			),
	...
	if (
		stage.lowMidEnabled &&
		(stage.lowMidType === 'notch' || Math.abs(stage.lowMidGainDb) > AUDIO_EQ_ACTIVE_EPSILON)
	) {
		gainDb += evaluateBiquadMagnitudeDb(
			buildEqBiquadCoefficients(
				getInnerBandBiquadType(stage.lowMidType),
	...
	if (
		...
		(stage.highMidType === 'notch' || Math.abs(stage.highMidGainDb) > AUDIO_EQ_ACTIVE_EPSILON)
	) {
		gainDb += evaluateBiquadMagnitudeDb(
			buildEqBiquadCoefficients(
				getInnerBandBiquadType(stage.highMidType),
	...
	if (
		stage.highEnabled &&
		(stage.highType === 'notch' || Math.abs(stage.highGainDb) > AUDIO_EQ_ACTIVE_EPSILON)
	) {
		gainDb += evaluateBiquadMagnitudeDb(
			buildEqBiquadCoefficients(
				getInnerBandBiquadType(stage.highType),
				stage.highFrequencyHz,
				stage.highGainDb,
				sampleRate,
				stage.highQ
			),
