<script lang="ts">
	...
	$effect(() => {
		...
		void saveToOpenPost(true);
	});

	async function saveToOpenPost(returningFromSignup = false): Promise<void> {
		if (!design || migrationBusy) return;
		if (!authState.isAuthenticated) {
			const returnURL = new URL(page.url);
			returnURL.searchParams.set('import', '1');
			sessionStorage.setItem('openpost-studio-signup-design', design.id);
			trackPublicStudioEvent('studio_signup_clicked', { source: 'editor' });
			await goto(resolve(`/register?redirect=${encodeURIComponent(returnURL.pathname + returnURL.search)}` as '/'));
			return;
		}
		migrationBusy = true;
		error = '';
		try {
			...
			if (
				returningFromSignup &&
				sessionStorage.getItem('openpost-studio-signup-design') === design.id
			) {
				sessionStorage.removeItem('openpost-studio-signup-design');
