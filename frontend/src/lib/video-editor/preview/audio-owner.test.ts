...
import { isAudibleOwner, resolveAudioOwner } from './audio-owner';
import type { MediaPoolEntry } from '../media/pool.svelte';
import type { TimelineItem, TimelineTrack } from '../project/types';

function makeMediaEntry(overrides: Partial<MediaPoolEntry['media']> & { status?: MediaPoolEntry['status'] } = {}): MediaPoolEntry {
...
describe('resolveAudioOwner', () => {
	it('returns none for non-audio/video types', () => {
		...
		for (const type of ['image', 'text', 'shape', 'composition', 'subtitle', 'lottie'] as const) {
			const owner = resolveAudioOwner({
				item: makeItem({ type: type as TimelineItem['type'], mediaId: undefined }),
				tracks: [makeTrack()],
				allItems: [],
				mediaEntry: entry,
	...
	it('returns linkedCompanion when hasLinkedAudioCompanion', () => {
		...
		const audioCompanion = makeItem({ id: 'a1', type: 'audio', linkedGroupId: 'g1', trackId: 'track2' });
	...
	it('isAudibleOwner true for embedded/processed/separateProxy', () => {
		expect(isAudibleOwner('embedded')).toBe(true);
		expect(isAudibleOwner('processed')).toBe(true);
		expect(isAudibleOwner('separateProxy')).toBe(true);
		expect(isAudibleOwner('linkedCompanion')).toBe(false);
		expect(isAudibleOwner('unsupported')).toBe(false);
		expect(isAudibleOwner('muted')).toBe(false);
		expect(isAudibleOwner('none')).toBe(false);
	});
});
