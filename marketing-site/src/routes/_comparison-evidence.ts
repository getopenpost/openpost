...
function evidence(basis: ClaimBasis, qualifier: string, sources: readonly ClaimSource[]) {
...
const openPostEvidence = {
	...
	'Analytics and engagement': evidence(
		...
		[
			source('OpenPost analytics guide', 'https://docs.openpost.social/usage/analytics'),
	...
	Automation: evidence(
		...
		[
			source('OpenPost developer docs', 'https://docs.openpost.social/development/'),
	...
	'Scheduling horizon': evidence(
		...
		[source('OpenPost scheduling guide', 'https://docs.openpost.social/usage/scheduling')]
	),
	'Threads and hosting': evidence(
		'Direct source',
		'Current thread workflow and deployment options; destination support remains account-specific.',
		[
			source('OpenPost threads guide', 'https://docs.openpost.social/usage/threads'),
			source('OpenPost self-hosting guide', 'https://docs.openpost.social/self-hosting/')
		]
	),
...
export const comparisonEvidenceRegister = {
	buffer: {
		...
		rows: {
			Publishing: evidence('Direct source', 'Free, Essentials, and Team plan comparison.', [
			...
			Automation: evidence(
				...
				[
					...
					source('Buffer MCP guide', 'https://developers.buffer.com/guides/integrations/mcp.html')
				]
			),
	...
	hootsuite: {
		...
		rows: {
			Publishing: evidence('Direct source', 'Standard through Enterprise plan comparison.', [
			...
			Automation: evidence(
				...
				[source('Hootsuite MCP connectors', 'https://www.hootsuite.com/integrations/mcp')]
			),
			Hosting: evidence('Direct source', 'Public hosted plans and custom Enterprise offering.', [
				source('Hootsuite plans', 'https://www.hootsuite.com/plans')
			])
		}
	},
...
export function attachComparisonEvidence<const T extends ComparisonDraft>(comparison: T) {
	...
	return {
		...
		rows: comparison.rows.map((row) => {
			...
			if (!competitorEvidence || !openpostEvidence) {
				throw new Error(`${comparison.slug}/${row.area} is missing claim-level evidence`);
			}
			return {
				...row,
				evidence: {
