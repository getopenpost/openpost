<script lang="ts">
	...
	const selectedMonthDay = $derived(
		days.find((day) => day.key === selectedMonthDayKey) ?? null
	);
				...
				>
					...
					<div class="month-grid grid min-h-0 grid-cols-7">
						{#each days as day (day.key)}
							...
							>
								<div class="mb-1 flex h-6 items-center justify-between gap-1.5">
												...
												<Tooltip.Trigger>
													{#snippet child({ props })}
														<button
															{...props}
															type="button"
															class="relative flex h-6 min-w-6 items-center justify-center rounded-md bg-muted px-1.5 text-[11px] font-semibold text-muted-foreground transition-colors before:absolute before:-inset-1 hover:bg-muted/80 hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
										...
										<Tooltip.Trigger>
											{#snippet child({ props })}
												<Button
													...
													variant="ghost"
													size="icon-xs"
													class="relative size-6 shrink-0 opacity-60 before:absolute before:-inset-1 group-hover/day:opacity-100 hover:bg-muted"
								...
								<div class="min-h-0 flex-1 space-y-1 overflow-hidden">
									{#each dayItems.slice(0, 2) as item (item.key)}
										...
										<button
											...
											data-calendar-item
											class={cn(
												'flex h-7 w-full items-center gap-1.5 overflow-hidden rounded-md border px-1.5 text-left text-xs transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none',
												itemTone(item),
										...
										>
											...
											></span>
											{#if primaryAccount}
												<PlatformIcon platform={primaryAccount.platform} class="size-3.5 shrink-0" />
											{/if}
...
<style>
	.month-shell {
		...
		height: clamp(40rem, calc(100dvh - 12.5rem), 52rem);
	}
