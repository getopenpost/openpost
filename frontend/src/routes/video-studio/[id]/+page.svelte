...
<script lang="ts">
	...
	let desktopTimeline = $state(false);
	...
	onMount(() => {
		...
		const updateDesktopState = () => (desktopTimeline = desktopQuery.matches);
		updateDesktopState();
	...
	function handleKeyboard(event: KeyboardEvent): void {
		...
		if (event.key === ' ' && !event.metaKey && !event.ctrlKey) {
			...
			if (selectedVisualItemID || selectedAudioItemID || selectedCaptionCueID) {
				event.preventDefault();
	...
	async function startExport(): Promise<Partial<Record<VariantID, File>>> {
		...
		try {
			...
			for (let index = 0; index < variants.length; index += 1) {
				...
				await persistExport(currentVariant, file);
			}
...
{:else if localProject && project}
	<div class="flex h-dvh min-h-0 flex-col overflow-hidden bg-background text-foreground">
		<header class="flex min-h-14 shrink-0 items-center gap-2 border-b px-2 sm:px-3">
			...
			<Button
				size="sm"
				disabled={!desktopTimeline}
				aria-label={desktopTimeline
					? m.video_studio_export()
					: m.video_studio_mobile_export_disabled()}
				title={desktopTimeline ? m.video_studio_export() : m.video_studio_mobile_export_disabled()}
				onclick={() => {
					selectedExportVariants ||= [variantID];
					exportOpen = true;
				}}
			...
			>
				...
				{#if selectedVisualItem}
					<div class="mt-4 space-y-5">
							...
							<div class="grid grid-cols-2 gap-2">
								...
								<label class="grid gap-1.5 text-xs font-medium">
									...
									<Input
										...
										oninput={(event) => updateVisualShapeStyle('stroke', event.currentTarget.value)}
									/>
								</label>
							</div>
							<label class="grid gap-2 text-xs">
						...
						<label class="flex min-h-10 items-center gap-2 text-xs font-medium">
							<Checkbox
								checked={selectedVisualItem.visible}
				...
				{:else if selectedCaptionCue}
					<div class="mt-4 space-y-4">
						...
						<Button class="w-full" variant="destructive" size="sm" onclick={deleteSelectedCaption}>
							...
							<div class="flex min-h-11 items-center gap-3 rounded-md border px-3">
								<span class="min-w-0 flex-1">
									<span class="block truncate text-sm font-medium">
										{variantOptions.find((item) => item.value === savedExport.variant_id)?.label ??
											savedExport.variant_id}
									</span>
