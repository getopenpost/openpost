...
<footer class="site-footer">
  ...
  <div class="marketing-shell grid gap-12 py-16 lg:grid-cols-[1.05fr_1.95fr] lg:py-20">
    <div>
      <a
        href="/"
  ...
  <div class="border-t border-white/10">
    <div class="marketing-shell flex flex-col gap-3 py-5 text-xs text-white/48 sm:flex-row sm:items-center sm:justify-between">
      <span>Copyright 2026 OpenPost Contributors. AGPL-3.0-only.</span>
      <span class="flex gap-5">
        <a class="focus-ring inline-flex min-h-11 items-center rounded-md hover:text-white" href="/privacy">Privacy</a>
        <a class="focus-ring inline-flex min-h-11 items-center rounded-md hover:text-white" href="/terms">Terms</a>
      </span>
    </div>
  </div>
</footer>
...
<style>
  ...
  .footer-cells i {
    ...
    animation: footer-bounce var(--bounce-duration) ease-in-out var(--bounce-delay) infinite;
  }

  .footer-cells .level-2 { background: oklch(0.58 0.09 48 / 0.52); }
  .footer-cells .level-3 { background: oklch(0.65 0.14 45 / 0.75); }
  .footer-cells .level-4 { background: oklch(0.7 0.18 43); }

  @keyframes footer-bounce {
    0%, 65%, 100% { transform: translateY(0); }
    76% { transform: translateY(-0.6rem); }
  ...
  @media (max-width: 39.99rem) {
    ...
    .footer-cells i:nth-child(n + 37) { display: none; }
  }

  @media (prefers-reduced-motion: reduce) {
    .footer-cells i { animation: none; }
  }
</style>
