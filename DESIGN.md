...
Tool families expose their variants through a visible menu trigger, with the active variant retained on the main tool button. Expanded pages occupy the ordered thumbnail strip; collapsed pages use its compact status row. Tool options stay in one scrollable row, with essential actions first, so wrapping controls cannot expand across the artwork. The narrow-screen More menu uses the same command registry as desktop menus.
