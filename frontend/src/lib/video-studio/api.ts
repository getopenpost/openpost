...
export type VideoStudioConfig = components['schemas']['VideoStudioConfigOutputBody'];
...
export type VideoStudioSyncPlan = components['schemas']['PlanVideoStudioSyncOutputBody'];

export async function loadVideoStudioConfig(): Promise<VideoStudioConfig> {
	const { data, error } = await client.GET('/video-studio/config');
	if (error || !data)
		throw new Error(error?.detail ?? 'Video Studio configuration could not load.');
	return data;
}
...
export async function listCloudVideoProjects(
	...
	const { data, error } = await client.GET('/video-studio/projects', {
		params: { query: { workspace_id: workspaceID, search, limit: 50, offset: 0 } }
	});
	if (error || !data)
		throw new Error(error?.detail ?? 'Cloud Video Studio projects could not load.');
	return {
...
export async function createCloudVideoProject(
	...
	const { data, error } = await client.POST('/video-studio/projects', {
		body: {
...
export async function getCloudVideoProject(projectID: string): Promise<CloudVideoProjectResponse> {
	const { data, error } = await client.GET('/video-studio/projects/{id}', {
...
export async function updateCloudVideoProject(
	...
	const { data, error, response } = await client.PATCH('/video-studio/projects/{id}', {
		params: { path: { id: projectID } },
		body: {
...
export async function planVideoStudioSync(
	workspaceID: string,
	input: components['schemas']['PlanVideoStudioSyncInputBody']
): Promise<VideoStudioSyncPlan> {
	const { data, error } = await client.POST('/video-studio/sync-plan', {
...
export async function listCloudVideoProjectRevisions(
	...
	const { data, error } = await client.GET('/video-studio/projects/{id}/revisions', {
		params: { path: { id: projectID } }
	});
...
export async function createCloudVideoProjectCheckpoint(
	...
	const { data, error } = await client.POST('/video-studio/projects/{id}/checkpoints', {
		params: { path: { id: projectID } },
		body: { name }
	});
...
export async function createVideoReturnToken(input: {
	...
	const { data, error } = await client.POST('/video-studio/return-tokens', { body: input });
	if (error || !data) throw new Error(error?.detail ?? 'Video Studio could not be opened.');
	return data;
}
...
export async function completeVideoReturnToken(
	...
	const { data, error } = await client.POST('/video-studio/return-tokens/{token}/complete', {
		params: { path: { token } },
		body: result
	});
	if (error || !data)
		throw new Error(error?.detail ?? 'Video Studio exports could not be returned.');
	return data;
}
...
export async function consumeVideoReturnToken(
	...
	const { data, error } = await client.POST('/video-studio/return-tokens/{token}/consume', {
		params: { path: { token } }
	});
	if (error || !data) throw new Error(error?.detail ?? 'Video Studio exports could not be added.');
	return data;
}
