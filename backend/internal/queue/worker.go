	...
	if job.Type == jobregistry.TypePublicationBuild {
		...
		case errors.Is(processErr, publicationbuilder.ErrRuntimeUnavailable):
			return classifiedJobFailure{
				retryable: true, retryAfter: publicationBuilderUnavailableRetry,
				message:          "Publication Builder is temporarily unavailable. OpenPost will retry when it is configured.",
				preserveAttempts: true,
			}
		case errors.Is(processErr, publicationbuilder.ErrTooManyActiveBuilds):
			return classifiedJobFailure{
				retryable: true, retryAfter: publicationBuilderUnavailableRetry,
				message:          "Publication Builder is waiting for an active build slot.",
				preserveAttempts: true,
			}
		case errors.Is(processErr, publicationbuilder.ErrBuildLeaseActive):
			return classifiedJobFailure{
				retryable: true, retryAfter: publicationBuilderUnavailableRetry,
				message:          "Publication Builder is waiting for the active generation lease.",
				preserveAttempts: true,
			}
		}
		return classifiedJobFailure{
			retryable: true,
			message:   "Publication Builder could not complete this build. OpenPost will retry when possible.",
		}
	}
