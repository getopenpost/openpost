...
func DeterministicContentURL(provider, _ string, username, instanceURL, externalID string) string {
	...
	case "mastodon":
		...
		if err == nil && base.Scheme == "https" && base.Host != "" && username != "" && safeContentID.MatchString(externalID) {
	...
	case "bluesky":
		if uri := blueskyPublishedURI(externalID); uri != "" {
			parts := strings.Split(strings.TrimPrefix(uri, "at://"), "/")
			if len(parts) == 3 && parts[1] == "app.bsky.feed.post" && safeContentID.MatchString(parts[0]) && safeContentID.MatchString(parts[2]) {
