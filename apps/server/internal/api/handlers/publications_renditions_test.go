...
func TestPublicationMediaAltTextPersistsForCanonicalAndRenditionOverrides(t *testing.T) {
	...
	_, mediaBySegment, err := handler.loadPublicationListSegments(ctx, db, []string{publication.ID})
	require.NoError(t, err)
	require.Equal(t, "Canonical description", mediaBySegment[segments[0].ID][0].AltText)
