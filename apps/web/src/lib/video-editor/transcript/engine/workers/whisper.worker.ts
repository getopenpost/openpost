...
import { joinTranscriptWords } from '../transcript-text';
...
self.onmessage = async (event: MessageEvent) => {
	...
	if (message.type === 'init') {
		// Reset per-job state. The worker and its compiled pipeline are reused across jobs.
		...
		if (!asrPipeline || currentModelId !== modelId) {
			...
			if (!adapter) {
				...
				try {
					...
					logger.warn(
						`[OpenPost transcription] WebGPU initialization failed: ${
