/**
 * Per-property expression presets and authoring guide.
 *
 * Ported from FreeCut (MIT)
 * `features/keyframes/components/dopesheet-editor/dopesheet-expression-catalog.ts`.
 * Property keys are mapped to our {@link DirectLinkableProperty} domain; preset
 * sources are verbatim so they evaluate identically in our expression parser.
 */
import type { DirectLinkableProperty } from '$lib/video-editor/project/types';
...
const ROTATION_PRESETS: readonly ExpressionPreset[] = [
	{
		...
		source: 'value + 90',
		group: 'Adjust',
		description: 'Add 90 degrees.'
	},
	{
		label: 'Half turn',
		source: 'value + 180',
		group: 'Adjust',
		description: 'Add 180 degrees.'
	},
	{
		label: 'Reverse',
		source: '-value',
...
} satisfies Partial<Record<DirectLinkableProperty, readonly ExpressionPreset[]>>;

export function getExpressionPresets(
	property: DirectLinkableProperty
): readonly ExpressionPreset[] {
	// Unkeyed properties (x, y, width, …) fall back to the scalar presets, mirroring
	// FreeCut's `PROPERTY_PRESETS[property] ?? DEFAULT_SCALAR_PRESETS`.
	// SAFETY: keyof covers the closed table above; unkeyed properties yield undefined.
	const presets = PROPERTY_PRESETS[property as keyof typeof PROPERTY_PRESETS];
	return presets ?? DEFAULT_SCALAR_PRESETS;
}
...
export const EXPRESSION_GUIDE_ITEMS = {
	basics: [
		{
			title: 'Current value',
			syntax: 'value',
	...
	functions: [
		...
		},
		{
			...
			description: 'Keep a result inside a range.'
		},
		{
			title: 'Interpolate',
...
} satisfies Record<ExpressionGuideSectionId, readonly ExpressionGuideItem[]>;
