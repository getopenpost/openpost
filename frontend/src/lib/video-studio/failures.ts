export type VideoStudioFailureCode =
...
export interface VideoStudioFailurePresentation {
	explanation: string;
	preservation: string;
	recovery: string;
	retryable: boolean;
...
const PRESENTATIONS: Record<VideoStudioFailureCode, VideoStudioFailurePresentation> = {
	'capability.webcodecs': {
		explanation: 'This browser does not provide the video codec engine required by Video Studio.',
	...
	'capability.webgl2': {
		explanation: 'WebGL2 is unavailable, so Video Studio cannot compose frames reliably.',
		preservation: 'Your existing local projects have not been changed.',
	...
	unknown: {
		explanation: 'Video Studio could not finish the operation.',
...
export interface VideoStudioDiagnostic {
	at: string;
	code: VideoStudioFailureCode;
	operation: string;
...
export function failurePresentation(code: VideoStudioFailureCode): VideoStudioFailurePresentation {
	return PRESENTATIONS[code];
}

export function classifyVideoStudioFailure(cause: unknown): VideoStudioFailureCode {
	const message =
...
export function recordVideoStudioDiagnostic(
	diagnostic: Omit<VideoStudioDiagnostic, 'at'>,
	storage: Pick<Storage, 'getItem' | 'setItem'> | undefined = globalThis.localStorage
): void {
	if (!storage) return;
	const key = 'openpost-video-studio-diagnostics-v1';
	let entries: VideoStudioDiagnostic[] = [];
	try {
		entries = JSON.parse(storage.getItem(key) ?? '[]') as VideoStudioDiagnostic[];
	} catch {
		entries = [];
	}
