...
export async function preflightExport(
	...
	for (const seg of enabled) {
		...
		const hasUnsupportedAudio = selectedAudios.some(
				...
				(a) => a.codec !== null && !sourceFormat.getSupportedAudioCodecs().includes(a.codec as AudioCodec)
			);
	...
	if (merge && enabled.length > 1) {
		...
		for (const seg of enabled) {
			...
			if (
				...
				(selVideo?.fps !== null &&
					firstFps !== null &&
					Math.abs(selVideo.fps - firstFps) > 0.001)
			) {
				requiresTranscode = true;
			...
			if (firstAudios.length > 0 && (selAudios[0]?.sampleRate !== firstSR || selAudios[0]?.channels !== firstCh)) {
				requiresTranscode = true;
...
async function exportMergedAudioStreamCopy(
	...
	try {
		...
		for (const segment of segments) {
			...
			const track =
				audioTracks[selIdx] ?? (await input.getPrimaryAudioTrack().catch(() => null));
	...
	try {
		...
		for (const seg of segments) {
			...
			if (src.id !== firstSource.id) {
				...
				const selVideo = getSelectedVideoStream(src);
				if (!selVideo) throw new UnsupportedStreamCopyError(`No video track selected for ${src.name}`);
