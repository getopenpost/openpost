...
<div class="mt-8 grid gap-5 xl:grid-cols-[minmax(21rem,0.85fr)_minmax(0,1.15fr)]">
	<section class="rounded-lg border bg-card p-4 sm:p-6" aria-labelledby="planner-controls-title">
		<div class="flex size-10 items-center justify-center rounded-xl border bg-background text-primary">
			<CalendarDays class="size-5" />
		</div>
		<h2 id="planner-controls-title" class="mt-4 text-lg font-semibold">Build a weekly test schedule</h2>
		<p class="mt-1 text-sm leading-6 text-muted-foreground">
			Choose the hours your audience is usually available. We evenly distribute a repeatable cadence
			and convert every slot to your timezone.
		</p>

		<div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
			<label for="audience-timezone" class="grid gap-2 text-sm font-medium">
				Audience timezone
				<select
			...
			<label for="local-timezone" class="grid gap-2 text-sm font-medium">
				Your timezone
				<select
					id="local-timezone"
					...
					class="min-h-11 rounded-lg border bg-background px-3"
				>
					{#each localTimezoneOptions as timezone (timezone)}
						<option value={timezone}>{timezone.replaceAll('_', ' ')}</option>
					{/each}
				</select>
			</label>
	...
	<section class="rounded-lg border bg-card p-4 sm:p-6" aria-labelledby="planner-output-title">
		<div class="flex flex-wrap items-start justify-between gap-4">
			...
			<div class="flex flex-wrap gap-2">
				<Button type="button" size="sm" variant="outline" disabled={slots.length === 0} onclick={copyPlan}>
					{#if copied}<Check data-icon="inline-start" />{:else}<ClipboardCopy data-icon="inline-start" />{/if}
					{copied ? 'Copied' : 'Copy CSV'}
				</Button>
				<Button type="button" size="sm" variant="outline" disabled={slots.length === 0} onclick={exportPlan}>
					<Download data-icon="inline-start" />
					Download
				</Button>
			</div>
		</div>
		<p class="sr-only" aria-live="polite">{copied ? 'Posting plan copied as CSV.' : ''}</p>

		<div class="mt-5 flex gap-3 rounded-xl border border-primary/20 bg-primary/[0.04] p-4 text-sm leading-6">
			<Info class="mt-0.5 size-4 shrink-0 text-primary" />
			<p>Use this evenly spaced baseline, then compare it with your own reach, replies, and conversions.</p>
		</div>
		{#if adjustedSlotCount > 0}
			<div class="mt-3 rounded-xl border border-amber-500/30 bg-amber-500/[0.06] p-4 text-sm leading-6">
				{adjustedSlotCount} {adjustedSlotCount === 1 ? 'slot was' : 'slots were'} shifted by an
				audience clock change. The schedule shows the actual audience and local times.
			</div>
		{/if}

		{#if !validWindow}
			<div class="mt-5 rounded-xl border border-destructive/30 bg-destructive/[0.04] p-4 text-sm text-destructive">
		...
		{:else}
			<div class="mt-5 overflow-hidden rounded-xl border">
				<div class="hidden grid-cols-[0.7fr_1fr_1.2fr] gap-4 border-b bg-muted/35 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground sm:grid">
					<span>Day</span>
					<span>Audience</span>
					<span>Local</span>
				</div>
				<ol class="divide-y">
					{#each slots as slot (slot.iso)}
						<li class="grid gap-2 px-4 py-4 sm:grid-cols-[0.7fr_1fr_1.2fr] sm:items-center sm:gap-4">
			...
			<p class="mt-3 text-xs leading-5 text-muted-foreground">
				Audience times use {audienceTimezone.replaceAll('_', ' ')}. Local results use {localTimezone.replaceAll('_', ' ')}.
			</p>
		{/if}
	</section>
</div>
