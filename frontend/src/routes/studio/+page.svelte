<!--
THESIS: Public Studio is a local workbench that starts with making, not an authentication pitch.
...
FORM: Operate surface extending the established Studio start screen; no marketing hero, editor fork, watermark, or export gate.
-->
<script lang="ts">
	...
	import { listPublicStudioTemplates, loadStudioConfig } from '$lib/studio/api';
	import {
		createGuestStudioDesign,
		createGuestStudioDesignFromImage,
		createGuestStudioDesignFromTemplate,
		deleteGuestStudioDesign,
		listGuestStudioDesigns,
		requestGuestStudioPersistence,
		type LocalStudioDesign
	} from '$lib/studio/local-persistence';
	import { trackPublicStudioEvent } from '$lib/studio/public-telemetry';
	import type { StudioPreset, StudioTemplate } from '$lib/studio/types';
	...
	let presets = $state.raw<StudioPreset[]>([]);
	let templates = $state.raw<StudioTemplate[]>([]);
	...
	async function initialize(): Promise<void> {
		...
		try {
			...
			trackPublicStudioEvent('studio_public_view', {
				returning_guest: localDesigns.length > 0
			});
		} catch (cause) {
			error = cause instanceof Error ? cause.message : m.studio_public_load_failed();
		} finally {
			loading = false;
		}
	}

	async function startPreset(preset: StudioPreset): Promise<void> {
		if (creating) return;
		creating = preset.key;
		error = '';
		try {
			void requestGuestStudioPersistence();
			const design = await createGuestStudioDesign(preset, m.studio_untitled_design());
			trackPublicStudioEvent('studio_design_started', {
				entry: 'preset',
				preset: preset.key
			});
			await goto(resolve(`/studio/${design.id}` as '/'));
		} catch (cause) {
			error = cause instanceof Error ? cause.message : m.studio_create_failed();
			creating = '';
		}
	}
	...
	async function startCustom(): Promise<void> {
		if (
			...
			error = m.studio_resize_limits();
			return;
		}
		await startPreset({
			key: 'custom',
			name: m.studio_custom_size(),
	...
	async function startTemplate(template: StudioTemplate): Promise<void> {
		if (creating) return;
		creating = template.id;
		error = '';
		try {
			void requestGuestStudioPersistence();
			const design = await createGuestStudioDesignFromTemplate(
				template,
	...
	async function openImage(event: Event): Promise<void> {
		...
		try {
			void requestGuestStudioPersistence();
			const design = await createGuestStudioDesignFromImage(
				file,
				file.name.replace(/\.[^.]+$/u, '') || m.studio_untitled_design()
			);
			trackPublicStudioEvent('studio_design_started', { entry: 'image' });
			await goto(resolve(`/studio/${design.id}` as '/'));
		} catch (cause) {
			error = cause instanceof Error ? cause.message : m.studio_media_open_failed();
			creating = '';
		}
	}

	function requestDelete(design: LocalStudioDesign): void {
	...
	async function deleteDesign(): Promise<void> {
		...
		await deleteGuestStudioDesign(pendingDelete.id);
	...
	function presetName(preset: StudioPreset): string {
		switch (preset.key) {
			case 'instagram-square':
				return m.studio_preset_instagram_square();
			case 'instagram-portrait':
				return m.studio_preset_instagram_portrait();
			case 'story-reel-slide':
				return m.studio_preset_story_slide();
			case 'linkedin-square':
				return m.studio_preset_linkedin_square();
			case 'linkedin-landscape':
				return m.studio_preset_linkedin_landscape();
			case 'x-landscape':
				return m.studio_preset_x_landscape();
			case 'youtube-thumbnail':
				return m.studio_preset_youtube_thumbnail();
			default:
				return preset.name;
		}
	}
	...
	function templateName(template: StudioTemplate): string {
		switch (template.id) {
			...
			case 'builtin-photo-caption':
				return m.studio_template_photo_caption();
			case 'builtin-quiet-quote':
				return m.studio_template_quiet_quote();
			case 'builtin-carousel-opener':
				return m.studio_template_carousel_opener();
			case 'builtin-carousel-step':
				return m.studio_template_numbered_steps();
			case 'builtin-story-prompt':
				return m.studio_template_story_prompt();
			case 'builtin-story-photo':
				return m.studio_template_story_photo();
			case 'builtin-linkedin-insight':
				return m.studio_template_linkedin_insight();
			case 'builtin-linkedin-launch':
				return m.studio_template_linkedin_launch();
			case 'builtin-x-update':
				return m.studio_template_x_update();
			case 'builtin-youtube-focus':
				return m.studio_template_youtube_focus();
			case 'builtin-youtube-list':
				return m.studio_template_youtube_list();
			default:
...
<svelte:head>
	<title>{m.studio_public_meta_title()}</title>
	<meta name="description" content={m.studio_public_meta_description()} />
</svelte:head>

<div class="studio-theme min-h-dvh bg-background text-foreground">
	<header class="border-b bg-background">
		<div class="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 sm:px-6 lg:px-8">
			...
			<span class="hidden text-sm text-muted-foreground sm:inline">/ {m.studio_title()}</span>
			<div class="ml-auto flex items-center gap-1.5">
				<LanguageSwitcher compact />
				{#if authState.isAuthenticated}
					<Button href="/studio/new" variant="outline" size="sm">
						{m.studio_public_workspace()}
					</Button>
				{:else}
					<Button href="/login?redirect=%2Fstudio" variant="ghost" size="sm">
						{m.landing_sign_in()}
					</Button>
				{/if}
			</div>
	...
	<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
		...
		{:else if !enabled}
			<div class="mt-10 max-w-xl rounded-xl border bg-card p-6">
				...
				<h2 class="mt-4 text-lg font-semibold">{m.studio_not_enabled()}</h2>
				<p class="mt-2 text-sm leading-6 text-muted-foreground">{m.studio_not_enabled_body()}</p>
			</div>
		{:else}
					...
					<div class="mb-4 flex items-end justify-between gap-4">
						<div>
							<h2 id="recent-designs-heading" class="text-lg font-semibold">
								{m.studio_public_recent()}
							</h2>
							<p class="mt-1 text-sm text-muted-foreground">
								{m.studio_public_recent_description()}
							</p>
						</div>
					</div>
							...
							<div class="group relative overflow-hidden rounded-xl border bg-card">
								...
								>
									<div class="aspect-[4/3] bg-neutral-800">
										<TemplatePreview document={design.document} label={design.document.title} compact />
									</div>
								...
								<Button
									...
									aria-label={m.studio_public_delete_design({ title: design.document.title })}
								>
									<TrashIcon />
								</Button>
							</div>
			...
			<section class="mt-12" aria-labelledby="formats-heading">
				<div class="mb-4">
					<h2 id="formats-heading" class="text-lg font-semibold">{m.studio_choose_format()}</h2>
					<p class="mt-1 text-sm text-muted-foreground">{m.studio_choose_format_body()}</p>
				</div>
			...
			<section class="mt-8 border-y py-6" aria-labelledby="custom-heading">
				<div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(22rem,32rem)] lg:items-end">
					<div>
						<h2 id="custom-heading" class="text-base font-semibold">{m.studio_custom_size()}</h2>
						<p class="mt-1 text-sm text-muted-foreground">{m.studio_custom_limits()}</p>
					</div>
					<div class="grid grid-cols-[1fr_1fr_auto] gap-2">
						<label class="grid gap-1 text-xs">
							<span>{m.studio_width()}</span>
							<Input type="number" min="64" max="4096" bind:value={customWidth} />
						</label>
						<label class="grid gap-1 text-xs">
							<span>{m.studio_height()}</span>
			...
			<section class="mt-12" aria-labelledby="templates-heading">
				<div class="mb-4">
					<h2 id="templates-heading" class="text-lg font-semibold">
						{m.studio_starter_templates()}
					</h2>
					<p class="mt-1 text-sm text-muted-foreground">
						{m.studio_public_templates_description()}
					</p>
				</div>
				<div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
...
<DestructiveConfirmDialog
	...
	title={m.studio_public_delete_title()}
	description={m.studio_public_delete_description()}
