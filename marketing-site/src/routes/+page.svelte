...
<script lang="ts">
	...
	const productViews = [
		...
		},
		{
			...
			image: '/assets/screenshots/main-dark.png',
			alt: 'OpenPost publication planning view',
			icon: CalendarDays
		},
		{
			id: 'analyze',
			label: 'Analyze',
			title: 'Read stored reporting without waiting on provider APIs.',
			description:
				'OpenPost keeps normalized account and publication snapshots while preserving the difference between views, impressions, and reach.',
			image: '/assets/screenshots/settings-dark.png',
			alt: 'OpenPost analytics and settings surfaces',
			icon: ChartNoAxesCombined
		},
		{
			id: 'engage',
			label: 'Engage',
			title: 'Keep communication work inside the same workspace.',
			description:
				'Supported comment and inbox reads use separate provider capabilities and stored sync state. Page loads do not call provider APIs.',
			image: '/assets/screenshots/accounts-dark.png',
			alt: 'OpenPost connected account workspace',
			icon: Inbox
		},
		{
			id: 'agents',
			label: 'Agents',
			title: 'Let automation prepare work without handing it provider tokens.',
			description:
				'API, CLI, and MCP access use the same workspace authorization, validation, quotas, and audit boundaries as the web app.',
			image: '/assets/screenshots/media-dark.png',
			alt: 'OpenPost media workspace used by publishing automation',
			icon: Bot
		}
	] as const;
	...
	<div class="marketing-shell">
		<div class="max-w-3xl">
			...
			<p class="marketing-copy mt-5">
				Create, plan, report, communicate, and automate without losing account-specific state.
			</p>
		</div>
		...
		<div class="grid gap-12 lg:grid-cols-[0.9fr_1.1fr]">
			<div>
				...
				<div class="mt-8 flex flex-wrap gap-3">
					<Button href={appUrl} size="lg">Try OpenPost</Button>
