...
describe("derived sequence operations", () => {
  ...
  it("trims source and freeze edges on frame boundaries", () => {
    ...
    expect(trimmedStart.primary_sequence[0]!.source_in_us).toBe(frameUS * 30);
    ...
    expect(trimmedEnd.primary_sequence[0]!.source_out_us).toBe(
      4_000_000 - frameUS * 60,
    );
    ...
    expect(shortenedFreeze.primary_sequence[0]!.freeze_duration_us).toBe(
  ...
  it("keeps adjoining transition durations valid while trimming", () => {
    ...
    expect(trimmed.primary_sequence[0]!.transition_out?.duration_us).toBe(
      expectedMaximum,
    );
    expect(trimmed.primary_sequence[1]!.transition_in?.duration_us).toBe(
      expectedMaximum,
    );
  });
