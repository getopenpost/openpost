  ...
  <div class="marketing-shell">
    <div class="max-w-4xl">
      ...
      <p class="marketing-copy mt-6">
        This page is generated from the repository changelog, the same record
        used for release notes. Open the source file for every fix and migration
        detail.
      </p>
      <div class="mt-8 flex flex-wrap gap-3">
        ...
        <Button href={`${githubUrl}/releases`} variant="outline" size="lg"
          >All releases</Button
        >
      </div>
    </div>
      ...
      {#each data.sections as entry (entry.label)}
        <article class="py-10">
          <div class="flex flex-wrap items-center gap-x-4 gap-y-2">
            {#if entry.date}
              <time datetime={entry.date} class="font-mono text-sm text-primary"
                >{entry.date}</time
              >
            {/if}
            <p
              ...
              <section aria-labelledby={`${entry.label}-${group.title}`}>
                ...
                {#if group.remaining > 0}
                  <p class="mt-3 text-xs text-muted-foreground">
                    {group.remaining} more {group.remaining === 1
                      ? "entry"
                      : "entries"} in the full changelog.
                  </p>
                {/if}
              </section>
            {/each}
...
<section class="border-t bg-muted/20 py-12">
  <div
    class="marketing-shell flex flex-col justify-between gap-5 sm:flex-row sm:items-center"
  >
    <div>
