<script lang="ts">
	...
	function choosePlatform(event: Event) {
		selectedPlatform = (event.currentTarget as HTMLSelectElement).value as PreviewPlatform;
	...
	function chooseFormat(event: Event) {
		selectedFormat = (event.currentTarget as HTMLSelectElement).value as PreviewFormat;
...
<div class="mt-8 grid gap-5 xl:grid-cols-[23rem_minmax(0,1fr)]">
	<section class="rounded-xl border bg-card p-4 sm:p-6" aria-labelledby="preview-controls-title">
		<h2 id="preview-controls-title" class="text-lg font-semibold">Build the destination</h2>
		...
		<div class="mt-5 grid gap-4">
				...
				<label class="grid gap-2 text-sm font-medium" for="preview-platform">
					...
					>
						{#each previewPlatforms as platform (platform)}
							<option value={platform}>{platformNames[platform]}</option>
						{/each}
					</select>
				</label>
				<label class="grid gap-2 text-sm font-medium" for="preview-format">
					Format
					<select
						id="preview-format"
						value={selectedFormat}
						onchange={chooseFormat}
						class="min-h-11 rounded-md border bg-background px-3 py-2 capitalize"
					>
			...
			<label class="grid gap-2 text-sm font-medium" for="preview-copy">
				...
				<textarea
					id="preview-copy"
					bind:value={draft}
					class="min-h-36 resize-y rounded-md border bg-background p-3 leading-6"
				...
				<div class="mt-4 grid gap-4">
					...
					{#if capability.polls}
						<label class="flex min-h-11 items-center gap-3 text-sm font-medium">
							<input type="checkbox" bind:checked={pollEnabled} class="size-4 rounded" />
							Include a poll
						</label>
						{#if pollEnabled}
							<p class="text-xs leading-5 text-muted-foreground">
					...
					{#if availableCardKinds.length > 0}
						{#if availableCardKinds.length > 1}
							<label class="grid gap-2 text-sm font-medium" for="preview-card-kind">
								Card type
								<select
									...
									class="min-h-11 rounded-md border bg-background px-3 py-2"
								>
									{#each availableCardKinds as kind (kind)}
										<option value={kind}>{kind === 'quote' ? 'Quoted post' : 'Link card'}</option>
									{/each}
								</select>
							</label>
						{/if}
					...
					{#if localMedia.length === 0}
						<div class="grid gap-3 rounded-lg bg-muted/45 p-3">
							...
							<label class="grid gap-2 text-sm font-medium" for="preview-media-kind">
								Media type
								<select
									id="preview-media-kind"
									bind:value={publicMediaKind}
									class="min-h-11 rounded-md border bg-background px-3 py-2"
								>
									{#each allowedMediaKinds as kind (kind)}
										<option value={kind}>
											{kind === 'document' ? 'PDF document' : kind === 'video' ? 'Video' : 'Image'}
										</option>
									{/each}
								</select>
							</label>
					...
					<label class="grid gap-2 text-sm font-medium" for="preview-alt-text">
						...
						<textarea
							id="preview-alt-text"
							bind:value={altText}
							class="min-h-20 resize-y rounded-md border bg-background p-3 leading-6"
		...
		<div class="grid w-full place-items-center gap-4">
			<div class="flex w-full max-w-2xl flex-wrap items-center justify-between gap-3">
				<div>
					...
					<p class="mt-1 text-sm text-muted-foreground">Provider interfaces can change.</p>
				</div>
