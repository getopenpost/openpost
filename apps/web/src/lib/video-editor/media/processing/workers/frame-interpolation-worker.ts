...
import { clampRenderSize, RifeInterpolator } from '../interpolation/rife-interpolator';
import { interpolateGap } from '../interpolation/interpolate-gap';
import type { InterpolationFactor } from '../interpolation/interpolation-factor';
...
async function interpolate(request: InterpolateRequest): Promise<void> {
	...
	let prev: {
		planar: Float32Array;
		encoded: VideoSampleInstance | null;
	} | null = null;
	let prevTimestamp = 0;
	...
	try {
		...
		post({
			type: 'complete',
			jobId,
			opfsPath: interpolationTmpPath(jobId),
			result
		});
	} catch (error) {
