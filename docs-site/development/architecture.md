...
OpenPost Video Editor keeps local editing and rendering under `frontend/src/lib/video-editor/` and `frontend/src/routes/video-editor/`, with Cloud Video Project persistence owned by `backend/internal/services/videoprojects/`. Signed-in cloud projects are Workspace-owned and contain portable authored documents, immutable revisions, mutation batches, conflict branches, checkpoints, Trash state, and Project Asset references. The shared portable contract lives in `packages/video-project/` and is consumed by web and mobile. A Local-only project keeps its document and source media in a user-selected folder.
