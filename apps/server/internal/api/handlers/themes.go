...
type CreateThemeInput struct {
	Body struct {
		...
		Manifest           *themes.ThemeManifest `json:"manifest,omitempty" doc:"Complete family manifest with light and/or dark schemes; omit when duplicating a built-in. Both create paths snapshot into an independent custom lineage, so later built-in updates never alter the copy."`
