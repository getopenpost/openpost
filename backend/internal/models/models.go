...
// ProviderUsageEvent is an append-only estimate of a provider request that can
// incur a variable hosted cost. OperationKey makes recording idempotent across
// duplicate job execution without retaining post text, provider responses, or
// credentials.
...
// ProviderUsagePeriodCounter is a reconciled monthly projection of immutable
// provider usage events. It is safe to rebuild from the event ledger.
