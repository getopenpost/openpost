...
<script lang="ts">
	...
	async function initializeEntry(): Promise<void> {
		const capabilities = await detectVideoStudioCapabilities();
		if (!capabilities.supported) {
			await goto(resolve('/video-studio/unsupported' as '/'));
			return;
		}
			...
			<section class="mt-8 border-y py-6" aria-labelledby="recording-setup-title">
				<div class="grid gap-8 md:grid-cols-[minmax(0,1fr)_18rem]">
					...
					<div class="space-y-4">
						...
						<Button class="w-full" disabled={creating} onclick={startRecording}>
							{#if creating}<LoaderIcon class="size-4 animate-spin" />{/if}
		...
		{:else}
			<section class="mt-8 space-y-5">
				...
				>
					...
					<Button class="mt-5" disabled={creating} onclick={triggerFilePicker}>
						{#if creating}<LoaderIcon class="size-4 animate-spin" />{/if}
				...
				<div class="grid gap-3 sm:grid-cols-2">
					<a
						...
						class="flex min-h-24 items-center gap-4 rounded-lg border p-4 transition-colors hover:bg-muted/40 focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
					>
						<MonitorIcon class="size-5 text-primary" />
						<span>
