...
func (s *Service) publishProviderWithUsage(
	...
	if err := s.recordProviderPublishCost(ctx, workspaceID, providerName, subject, phase, req); err != nil {
		return "", err
	}
	s.recordProviderWriteCall(ctx, workspaceID)
	return s.publishProvider(ctx, provider, token, accountID, req, media)
}
...
func (s *Service) createPublicationResultNotifications(
	...
	for _, rendition := range renditions {
		label := publisherDestinationLabel(rendition, accountByID[rendition.SocialAccountID])
		switch rendition.Status {
		case models.RenditionStatusPublished:
			successful = append(successful, label)
		case models.RenditionStatusFailed:
			failed = append(failed, label)
			failedAccountIDs = append(failedAccountIDs, rendition.SocialAccountID)
			failureActions = append(failureActions, rendition.ErrorAction)
			retryable = retryable || rendition.ErrorRetryable
			reconnect = reconnect || rendition.ErrorAction == FailureActionReconnect
			if !rendition.ErrorRetryAt.IsZero() && (retryAt.IsZero() || rendition.ErrorRetryAt.Before(retryAt)) {
				retryAt = rendition.ErrorRetryAt
			}
		}
	}
	...
	if !retryAt.IsZero() {
		input.Payload["retry_at"] = retryAt.UTC().Format(time.RFC3339)
	}
	switch status {
	case models.PublicationStatusPublished:
		input.Type = notifications.TypePostPublished
		input.Title = "Publication completed"
		input.Body = publishedDestinationSummary(successful)
	...
	case models.PublicationStatusFailed:
		...
		input.Title = publicationFailureTitle(successful, failed)
		input.Body = publicationFailureSummary(successful, failed, retryAt)
		input.DedupKey = fmt.Sprintf("publication:%s:revision:%d:failed", publication.ID, publication.Revision)
		primary := models.NotificationAction{
			Label: "Edit publication", Href: "/publications/" + publication.ID, Kind: "primary",
		}
		switch {
		case reconnect:
			primary = models.NotificationAction{Label: "Reconnect account", Href: "/accounts", Kind: "primary"}
		case retryable:
			primary = models.NotificationAction{
				Label: "Retry failed destinations", Kind: "primary",
				Operation: "retry_failed_publication", TargetID: publication.ID,
			}
		}
		input.Actions = append([]models.NotificationAction{primary}, input.Actions...)
	default:
		return nil
	}
	...
	if reconnect {
		for _, accountID := range failedAccountIDs {
