...
describe('video project document', () => {
	...
	it('rejects unknown root fields, missing sources, and invalid speed', () => {
		const project = projectWithClips() as VideoProjectDocumentV1 & { surprise?: boolean };
...
describe('derived sequence operations', () => {
	it('derives ripple timing and bounded transition overlap', () => {
		...
		expect(derived[0]).toMatchObject({ timeline_start_us: 0, timeline_end_us: 4_000_000 });
	...
	it('ripple-cuts across clip boundaries as one deterministic action', () => {
		const project = projectWithClips();
		const cut = removePrimaryRanges(project, [{ start_us: 3_000_000, end_us: 5_000_000 }], () => 'new');
	...
	it('isolates per-variant presentation overrides', () => {
		const project = projectWithClips();
		const changed = setVariantPresentationOverride(project, 'a', 'portrait', { scale: 1.25 });
		expect(changed.primary_sequence[0]!.variant_overrides?.portrait?.scale).toBe(1.25);
		expect(changed.primary_sequence[0]!.variant_overrides?.square).toBeUndefined();
...
describe('time, keyframes, captions, and suggestions', () => {
	...
	it('reflows captions and links timed transcript selection to a padded cut', () => {
		expect(reflowCaptionText('Make one clear social video without leaving the browser', 18, 2)).toEqual([
			'Make one clear',
			'social video without leaving the browser'
		]);
