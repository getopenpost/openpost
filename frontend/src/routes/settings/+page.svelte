<script lang="ts">
	...
	function providerCostProgress(cost: ProviderCostSummary) {
		if (cost.budget_microusd <= 0) return 100;
		return Math.min(100, Math.round((cost.cost_microusd / cost.budget_microusd) * 100));
	}
								...
								{#each providerCosts as providerCost (providerCost.provider)}
									<div class="min-w-0">
										<div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
											...
											<div class="shrink-0 text-left sm:text-right">
												...
												<p class="text-sm text-muted-foreground">
													{m.settings_provider_cost_budget({
														used: formatProviderCost(
															providerCost.cost_microusd,
															providerCost.currency
														),
														budget: formatProviderCost(
															providerCost.budget_microusd,
															providerCost.currency
														)
													})}
												</p>
										...
										<div
											...
											role="progressbar"
											aria-label={m.settings_provider_cost_budget({
												used: formatProviderCost(providerCost.cost_microusd, providerCost.currency),
												budget: formatProviderCost(
													providerCost.budget_microusd,
													providerCost.currency
												)
											...
											aria-valuemin="0"
											aria-valuemax={providerCost.budget_microusd}
