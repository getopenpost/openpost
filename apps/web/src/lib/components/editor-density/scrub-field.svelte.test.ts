...
describe('ScrubField keyboard', () => {
	it('nudges one step with arrow keys', async () => {
		...
		await input.click();
		pressKey(input.element() as HTMLElement, 'ArrowUp');
		expect(onValueChange).toHaveBeenLastCalledWith(21);
		pressKey(input.element() as HTMLElement, 'ArrowDown');
	...
	it('commits typed text on Enter and reverts on Escape', async () => {
		...
		await input.clear();
		await input.fill('42');
		pressKey(input.element() as HTMLElement, 'Enter');
		expect(onValueCommit).toHaveBeenCalledWith(42);

		await input.click();
		await input.clear();
		await input.fill('77');
		pressKey(input.element() as HTMLElement, 'Escape');
