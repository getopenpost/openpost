...
test("guest imports, edits, autosaves, restores, and exports a local video", async ({
  ...
  const positionX = page.getByRole("slider", { name: "Position X" });
  ...
  const clipButton = page
    ...
    .first();
  const widthBeforeTrim = (await clipButton.boundingBox())?.width ?? 0;
  await page
    .getByRole("button", { name: "Trim clip end" })
    .press("Shift+ArrowLeft");
  await expect
    .poll(async () => (await clipButton.boundingBox())?.width ?? 0)
    .toBeLessThan(widthBeforeTrim);
  await page.keyboard.press("Control+z");
  await expect
    .poll(async () => (await clipButton.boundingBox())?.width ?? 0)
    .toBeGreaterThan(widthBeforeTrim - 2);
  ...
  await page.getByLabel("Text", { exact: true }).fill("Launch day");
...
test("mobile shows preview and handoff guidance without a compressed timeline", async ({
  ...
  const currentTime = page.getByText(/0:00:00/).first();
  await page.getByRole("button", { name: "Play" }).click();
  await expect
    .poll(async () => (await currentTime.textContent()) ?? "")
    .not.toContain("0:00:00 of");
  expect(
    await page.evaluate(
      () => document.documentElement.scrollWidth <= window.innerWidth,
    ),
