...
Then turn that idea into a short series: explain the problem, share the method, and end with one action readers can take. Each post should still make sense on its own.`;
	...
	const parts = $derived(splitSmartThread(draft, platform, selectedPlatform.limit, numbering));
...
<div class="mt-8 grid min-w-0 gap-5 xl:grid-cols-[minmax(20rem,0.84fr)_minmax(0,1.16fr)]">
	...
	>
		<div class="flex flex-wrap items-start justify-between gap-4">
			<div>
				...
				<p class="mt-1 max-w-xl text-sm leading-6 text-muted-foreground">
					The splitter keeps paragraph and sentence breaks when they fit. Long words are carried
					over, not discarded.
				</p>
			</div>
		...
		<div class="mt-5 grid gap-4 sm:grid-cols-2">
			<div>
				<label for="thread-platform" class="text-sm font-medium">Social network</label>
				<AppSelect
					id="thread-platform"
					value={platform}
			...
			<ol class="mt-5 grid min-w-0 gap-3">
				{#each parts as part, index (`${index}-${part.text}`)}
					<li class="min-w-0 rounded-xl border bg-background p-4">
						...
						<p class="mt-3 min-w-0 text-[0.94rem] leading-6 break-words whitespace-pre-wrap">
							{part.text}
						</p>
					</li>
				{/each}
