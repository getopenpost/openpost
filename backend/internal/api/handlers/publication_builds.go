...
func (h *PublicationBuildHandler) preparePublicationBuildSource(
	...
	if idea == "" && len(contextURLs) == 0 && len(assets) == 0 {
	...
	return preparedPublicationBuildSource{
		idea: idea, sources: []publicationbuilder.SourceMaterial{}, contextURLs: contextURLs,
...
func (h *PublicationBuildHandler) preparePublicationBuildDestinations(
	...
	if nativeDestinationCount == 0 {
		return nil, huma.Error400BadRequest("Select at least one account with a native Publication Builder adapter")
	}
