...
- Release preparation validates cheap inputs first, stages only `CHANGELOG.md` and `changes/`, then selects one immutable SHA. Never `git add --all`, never stash someone else's work, never restart the whole release for a narrow failure: classify it, fix it in its owner, create a new candidate only when source changes, otherwise resume the failed job. New work either explicitly supersedes the candidate or ships next release.
