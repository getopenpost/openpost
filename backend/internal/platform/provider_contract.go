...
func ApplicationContract(provider, connectionMode string) (ProviderAppContract, bool) {
	...
	contracts := map[string]ProviderAppContract{
		...
		providerPinterest + ":" + ConnectionModeOAuth:     {Provider: providerPinterest, ConnectionMode: ConnectionModeOAuth, RequiredFields: []string{"client_id", "client_secret", "redirect_uri"}},
