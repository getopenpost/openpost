...
| `both`   | Direct tools plus the search trio                                                                 |

Search mode keeps the previous progressive-discovery flow: search first, then query read-only operations or execute mutations. Scopes and workspace checks behave the same in every mode; `mcp:read` connections never see or run mutations.
