...
func (s *Service) RecordProviderCost(ctx context.Context, input ProviderCostEventInput) (ProviderCostRecordResult, error) {
	...
	if input.WorkspaceID == "" {
		return ProviderCostRecordResult{Enabled: true}, errors.New("workspace id is required")
	}
	if input.OperationKey == "" || len(input.OperationKey) > 200 {
		return ProviderCostRecordResult{Enabled: true}, errors.New("bounded operation key is required")
	}
	if input.Units <= 0 {
		return ProviderCostRecordResult{Enabled: true}, errors.New("provider cost units must be positive")
	}
	unitCost, ok := pricing.OperationCostMicrousd[input.Operation]
	if !ok {
		return ProviderCostRecordResult{Enabled: true}, fmt.Errorf("provider cost operation %q is not priced", input.Operation)
	}
	if unitCost > 0 && input.Units > math.MaxInt64/unitCost {
		return ProviderCostRecordResult{Enabled: true}, errors.New("provider cost exceeds the supported range")
	}
...
func incrementProviderCostOperation(
	...
	_, err := tx.NewInsert().
		...
		Set("event_count = provider_usage_period_counters.event_count + EXCLUDED.event_count").
		Set("units = provider_usage_period_counters.units + EXCLUDED.units").
		Set("cost_microusd = provider_usage_period_counters.cost_microusd + EXCLUDED.cost_microusd").
