/**
 * Ported from FreeCut (MIT) — shared/utils/managed-worker-pool.ts.
 *
 * Pools reusable Web Workers: acquire pops an idle worker or boots a new one,
 * release returns it to the idle list up to `maxIdleWorkers`, and everything
 * is torn down together via terminateAll.
 */
...
export function createManagedWorkerPool<TWorker extends Worker = Worker>(
	...
	return { acquireWorker, releaseWorker, terminateWorker, terminateAll };
}
