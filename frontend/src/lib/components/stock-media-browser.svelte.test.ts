...
describe('StockMediaBrowser', () => {
	...
	it('themes ordinary controls while keeping stock media glyphs protected', async () => {
		...
		try {
			...
			await screen.getByRole('textbox', { name: 'Search stock media' }).fill('ocean');
			await screen.getByRole('button', { name: 'Search' }).click();
			await expect.element(screen.getByText('Ocean clip')).toBeVisible();
			...
			expect(screen.container.querySelector('[data-protected-icon="media-image"]')).not.toBeNull();
			const videoIcon = screen.container.querySelector('[data-protected-icon="media-video"]');
			expect(videoIcon).not.toBeNull();
			expect(videoIcon?.getAttribute('data-theme-icon')).toBeNull();
			expect(videoIcon?.getAttribute('data-icon-pack')).toBeNull();
		} finally {
			if (previousPack) {
