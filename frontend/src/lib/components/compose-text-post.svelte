														...
														<div {...props} class="flex cursor-default items-center gap-1.5">
															<svg
																class="h-4 w-4 {getCharCounterColor(
																	characterCount(getEditorContentForPost(post)),
																	editorMaxChars
																)}"
																viewBox="0 0 20 20"
															>
															...
															<span class="text-xs text-muted-foreground/60 tabular-nums"
																>{characterCount(getEditorContentForPost(post))}/{editorMaxChars}</span
															>
														</div>
													{/snippet}
												</Tooltip.Trigger>
													...
													<div class="space-y-1">
														...
														{#each editorPlatformLimits as pl (pl.key)}
															<div class="flex items-center justify-between gap-2 text-xs">
																...
																<span
																	class="tabular-nums {characterCount(
																		getEditorContentForPost(post)
																	) >
																	pl.limit
																		? 'text-red-500'
																		: 'text-muted-foreground'}"
																	>{characterCount(getEditorContentForPost(post))}/{pl.limit}</span
																>
															</div>
														{/each}
													</div>
