...
async function modelPartialDirectory(): Promise<FileSystemDirectoryHandle> {
	...
	const studio = await root.getDirectoryHandle('openpost-video-studio', { create: true });
	return await studio.getDirectoryHandle(MODEL_PARTIAL_DIRECTORY, { create: true });
}
