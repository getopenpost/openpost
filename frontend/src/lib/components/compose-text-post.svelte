<script lang="ts">
	...
	let lastToastedWorkspaceLoadError = '';
	let lastToastedWorkspaceSettingsError = '';
	let lastToastedAccountLoadError = '';
	let lastToastedCapabilityError = '';
	let lastToastedWorkspaceChange = '';
	let lastToastedError = '';
	let lastToastedSuccess = '';
	...
	$effect(() => {
		if (workspaceLoadError && workspaceLoadError !== lastToastedWorkspaceLoadError) {
			lastToastedWorkspaceLoadError = workspaceLoadError;
			...
			lastToastedWorkspaceLoadError = '';
		}
		if (workspaceSettingsError && workspaceSettingsError !== lastToastedWorkspaceSettingsError) {
			lastToastedWorkspaceSettingsError = workspaceSettingsError;
			const message = workspaceSettingsError;
			showToast(message, 'error', {
				actionLabel: m.common_retry(),
				onAction: () => void retryComposerWorkspaceSettings()
			});
		} else if (!workspaceSettingsError) {
			lastToastedWorkspaceSettingsError = '';
		}
		if (accountLoadError && accountLoadError !== lastToastedAccountLoadError) {
			lastToastedAccountLoadError = accountLoadError;
			const message = accountLoadError;
			showToast(message, 'error', {
				actionLabel: m.common_retry(),
				onAction: () => void loadAccounts(selectedWorkspaceId, accountRetryIds)
			});
		} else if (!accountLoadError) {
			lastToastedAccountLoadError = '';
		}
		if (capabilityResolveError && capabilityResolveError !== lastToastedCapabilityError) {
			lastToastedCapabilityError = capabilityResolveError;
			const message = capabilityResolveError;
			showToast(message, 'error', {
				actionLabel: m.common_retry(),
				onAction: () => void resolveCapabilities()
			});
		} else if (!capabilityResolveError) {
			lastToastedCapabilityError = '';
		}
		if (workspaceChangeNotice && workspaceChangeNotice !== lastToastedWorkspaceChange) {
			lastToastedWorkspaceChange = workspaceChangeNotice;
			const message = workspaceChangeNotice;
			showToast(message, 'info');
		} else if (!workspaceChangeNotice) {
			lastToastedWorkspaceChange = '';
		}
		if (error && error !== lastToastedError) {
		...
		if (success && success !== lastToastedSuccess) {
			lastToastedSuccess = success;
