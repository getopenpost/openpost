...
func (h *AuthHandler) Register(api huma.API) {
	huma.Register(api, huma.Operation{
		...
		user, err := h.registerUserWithPolicy(ctx, input.Body.Email, input.Body.Username, input.Body.Password, acceptedLegal)
		if errors.Is(err, errEmailAlreadyRegistered) {
			return nil, huma.Error409Conflict("email already registered")
		}
		if errors.Is(err, errUsernameAlreadyRegistered) {
			return nil, huma.Error409Conflict("username already registered")
		}
...
func (h *AuthHandler) UpdateProfile(api huma.API) {
	huma.Register(api, huma.Operation{
		...
		if input.Body.Username != nil {
			username := usernames.Normalize(*input.Body.Username)
			...
			exists, err := h.db.NewSelect().Model((*models.User)(nil)).
				...
				Exists(ctx)
			if err != nil {
				return nil, huma.Error500InternalServerError("failed to validate username")
			}
			if exists {
				return nil, huma.Error409Conflict("username already registered")
			}
		...
		if input.Body.PublicProfileEnabled != nil {
			if *input.Body.PublicProfileEnabled {
				username := ""
				if input.Body.Username != nil {
					username = usernames.Normalize(*input.Body.Username)
				} else if err := h.db.NewSelect().Model((*models.User)(nil)).Column("username").Where("id = ?", userID).Scan(ctx, &username); err != nil {
					return nil, huma.Error500InternalServerError("failed to load username")
				}
				if username == "" {
					return nil, huma.Error400BadRequest("set a username before making the profile public")
				}
			}
			update = update.Set("public_profile_enabled = ?", *input.Body.PublicProfileEnabled)
			hasChange = true
		}
		if hasChange {
