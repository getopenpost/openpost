<script lang="ts">
	...
	import { defaultStudioPageGradient, studioPageBackground } from '../document';
	import { gradientColorAt, normalizedGradientStops } from '../gradient';
	import { normalizeHex } from '../color';
	import { useStudioEditor } from '../editor.svelte';
	import type { StudioGradientType, StudioGradientValue, StudioPageBackground } from '../types';
	...
	let background = $derived(
		page ? studioPageBackground(page) : ({ type: 'solid', color: '#ffffff', opacity: 1 } as const)
	);
	...
	function updateBackground(next: StudioPageBackground, coalesceKey?: string): void {
		editor.mutate(
	...
	function setBackgroundType(type: StudioPageBackground['type']): void {
		...
		if (type === 'gradient') {
			updateBackground({
				...
				gradient:
					background.type === 'gradient' && background.gradient
						? background.gradient
						: defaultStudioPageGradient(editor.document.width_px, editor.document.height_px)
			});
			return;
		}
	...
	function updateGradient(
		mutation: (gradient: StudioGradientValue) => void,
		coalesceKey?: string
	): void {
	...
	function gradientAngle(gradient: StudioGradientValue): number {
		return Math.round(
	...
	function updateGradientStop(
		index: number,
		updates: Partial<StudioGradientValue['stops'][number]>,
		coalesceKey?: string
	): void {
		updateGradient((gradient) => {
...
<section class="space-y-4">
	<div>
		<h3 class="text-sm font-semibold">{m.studio_page_background()}</h3>
		<p class="mt-1 text-xs leading-relaxed text-muted-foreground">
			{m.studio_page_background_help()}
		</p>
	</div>
	...
	{#if background.type === 'solid'}
		<StudioColorPicker
			label={m.studio_background_color()}
		...
		<div class="space-y-3 rounded-lg border p-3">
			...
			<div class="grid grid-cols-2 gap-2">
				<AppSelect
					...
					onValueChange={(value) =>
						updateGradient(
							(gradient) => (gradient.type = value as StudioGradientType),
			...
			<label class="grid gap-1 text-xs">
				...
				<Slider
					...
					step={1}
					disabled={!editor.canEdit}
					ariaLabel={m.studio_gradient_direction({
						value: gradientAngle(background.gradient)
					})}
					...
					<div class="space-y-2 rounded-md border p-2">
						<div class="flex items-center gap-2">
							<span class="min-w-0 flex-1 text-xs font-medium">
								{m.studio_gradient_stop({ number: index + 1 })}
							</span>
							<Button
								variant="ghost"
								size="icon-xs"
								disabled={!editor.canEdit || background.gradient.stops.length <= 2}
								aria-label={m.studio_remove_gradient_stop({ number: index + 1 })}
						...
						<StudioColorPicker
							label={m.studio_gradient_stop_color({ number: index + 1 })}
							value={stop.color}
							disabled={!editor.canEdit}
							{brandColors}
							recentColors={editor.recentColors}
						...
						<label class="grid gap-1 text-xs">
							<span
								>{m.studio_gradient_stop_position({ value: Math.round(stop.offset * 100) })}</span
							>
							<Slider
								...
								step={1}
								disabled={!editor.canEdit}
								ariaLabel={m.studio_gradient_stop_position({
		...
		<div class="space-y-3 rounded-lg border p-3">
			...
			>
				{background.image?.media_id
					? m.studio_replace_background_image()
					: m.studio_choose_background_image()}
			</Button>
		</div>
	{/if}
	...
	{#if background.type !== 'transparent'}
		<label class="grid gap-1 text-xs">
			<span>{m.studio_background_opacity({ value: Math.round(background.opacity * 100) })}</span>
			<Slider
				...
				step={0.01}
				disabled={!editor.canEdit}
				ariaLabel={m.studio_background_opacity({
