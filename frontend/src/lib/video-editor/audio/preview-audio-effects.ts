...
function createCompressorNodes(
  ...
  return {
    ...
    dispose() {
      for (const node of [compressor, makeup, dry, wet, input, output, bypass])
        node.disconnect();
    },
  };
}
...
function hashParams(decay: number, roomSize: number, damping: number): number {
  return (
    Math.floor(decay * 7919) ^
    Math.floor(roomSize * 5009) ^
    Math.floor(damping * 3001) ^
    0x9e3779b1
  );
}
...
function generateReverbImpulse(
  ...
  for (let ch = 0; ch < 2; ch++) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      const env =
        Math.exp(-t * (3 / Math.max(0.1, decay))) * (0.5 + roomSize * 0.5);
...
function createReverbNodes(
  ...
  return {
    ...
    dispose() {
      for (const node of [convolver, dry, wet, input, output, bypass, delay])
        node.disconnect();
    },
  };
}
...
function createModulatedDelayNodes(
  ...
  feedback.gain.value =
    (params as import("./audio-effects").FlangerEffect).feedback ?? 0;
  ...
  return {
    ...
    update(next: AudioEffect) {
      ...
      if ("feedback" in n)
        feedback.gain.value =
          (n as import("./audio-effects").FlangerEffect).feedback ?? 0;
      this.enabled = n.enabled;
    },
    dispose() {
      try {
...
export function normalizeEffectChainForPreview(
