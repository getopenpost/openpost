<script lang="ts">
  ...
  const localTimezoneOptions = Array.from(
    new Set([detectedTimezone || "UTC", ...TIMEZONES]),
  );
  ...
  const adjustedSlotCount = $derived(
    slots.filter((slot) => slot.adjustedForTimezone).length,
  );
...
<div
  class="mt-8 grid gap-5 xl:grid-cols-[minmax(21rem,0.85fr)_minmax(0,1.15fr)]"
>
  <section
    class="rounded-lg border bg-card p-4 sm:p-6"
    aria-labelledby="planner-controls-title"
  >
    <div
    ...
    <div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
      <div class="grid gap-2">
        <label for="audience-timezone" class="text-sm font-medium"
          >Audience timezone</label
        >
        <AppSelect
      ...
      <div class="grid gap-2">
        <label for="local-timezone" class="text-sm font-medium"
          >Your timezone</label
        >
        <AppSelect
          id="local-timezone"
  ...
  <section
    class="rounded-lg border bg-card p-4 sm:p-6"
    aria-labelledby="planner-output-title"
  >
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        <h2 id="planner-output-title" class="text-lg font-semibold">
          Your local schedule
        </h2>
    ...
    {#if adjustedSlotCount > 0}
      ...
      >
        {adjustedSlotCount}
        {adjustedSlotCount === 1 ? "slot was" : "slots were"} shifted by an audience
        clock change. The schedule shows the actual audience and local times.
      </div>
    {/if}
    ...
    {:else}
      <div class="mt-5 overflow-hidden rounded-xl border">
        <div
          class="hidden grid-cols-[0.7fr_1fr_1.2fr] gap-4 border-b bg-muted/35 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground sm:grid"
        >
          <span>Day</span>
          <span>Audience</span>
          <span>Local</span>
        ...
        <ol class="divide-y">
          {#each slots as slot (slot.iso)}
            ...
            >
              ...
              <p class="text-sm">
                <span class="mr-2 text-xs text-muted-foreground sm:hidden"
                  >Audience</span
                >
      ...
      <p class="mt-3 text-xs leading-5 text-muted-foreground">
        Audience times use {audienceTimezone.replaceAll("_", " ")}. Local
        results use {localTimezone.replaceAll("_", " ")}.
      </p>
    {/if}
  </section>
</div>
