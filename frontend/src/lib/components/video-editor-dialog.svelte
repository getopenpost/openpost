<script lang="ts">
	...
	function initializeEditor() {
		...
		void probeVideo(currentFile)
			...
			.catch((cause) => {
				if (sequence !== probeSequence) return;
				error = cause instanceof Error ? cause.message : m.compose_upload_failed();
			})
			.finally(() => {
...
<Dialog.Root bind:open>
	...
	>
		...
		<Dialog.Footer class="border-t px-4 py-3 sm:px-5">
			...
			<Button type="button" variant="outline" class="min-h-11" onclick={closeEditor}>
				{rendering ? m.video_upload_cancel() : m.common_cancel()}
			</Button>
			<Button type="button" class="min-h-11" disabled={!metadata || rendering} onclick={applyEdit}>
