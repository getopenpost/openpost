...
export function getSharedPreviewAudioContext(): AudioContext | null {
	...
	// SAFETY: Safari exposes its standards-compatible constructor on this vendor-prefixed field.
	...
	if (sharedPreviewAudioContext === null || sharedPreviewAudioContext.state === 'closed') {
		sharedPreviewAudioContext = new AudioContextCtor({ latencyHint: 'interactive' });
	}
