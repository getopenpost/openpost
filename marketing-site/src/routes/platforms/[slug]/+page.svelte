...
<section class="section-pad" aria-labelledby="formats-title">
  <div class="marketing-shell">
    ...
    <div
      class="mt-10 hidden overflow-hidden rounded-xl border bg-card md:block"
    >
        ...
        <tbody class="divide-y">
          {#each platform.formats as format (format.name)}
            <tr>
              ...
              <td class="px-5 py-5 text-sm leading-6 text-muted-foreground"
                >{format.text}</td
              >
              <td class="px-5 py-5 text-sm leading-6 text-muted-foreground"
                >{format.media}</td
    ...
    <div>
      ...
      <ul class="mt-8 border-t">
        {#each platform.limitations as limitation (limitation)}
          <li
            class="flex gap-3 border-b py-5 text-sm leading-6 text-muted-foreground"
          >
            <Info
...
<section class="section-pad">
  <div
    class="marketing-shell grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center"
  >
    ...
    <div>
      <p class="marketing-copy">
        Social network developer pages change. The OpenPost guide lists the
        settings and checks you need now.
      </p>
      <Button
        href={platform.docsUrl}
        target="_blank"
