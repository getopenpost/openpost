...
- Added a cloud-only X request-cost guardrail with immutable idempotent events, reconciled monthly counters, workspace-visible estimates, and per-workspace safety limits that never gate self-hosted publishing.
...
- Reduced app startup and publication-list load with selective precompression, immutable asset caching, route-on-demand service-worker caching, lazy browser video-editor loading, and fixed-count batched publication hydration.
