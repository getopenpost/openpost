...
Every direct `@lucide/svelte` icon use under `frontend/src` mapped by MEANING to a registry role.
...
Excluded (test files, owned by test workers): any `*.test.*` under `frontend/src` (e.g. `ui-consistency.svelte.test.ts` uses the image icon).
