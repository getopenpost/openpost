...
func normalizeDefinition(input Definition) (Definition, string, error) {
	...
	if out.Vocabulary, err = normalizeList(input.Vocabulary, maxShortItems, maxShortItemCharacters); err != nil {
		return Definition{}, "", invalidDefinition("vocabulary")
	}
	if out.RecurringExpressions, err = normalizeList(input.RecurringExpressions, maxShortItems, maxShortItemCharacters); err != nil {
		return Definition{}, "", invalidDefinition("recurring_expressions")
	}
	if out.Expertise, err = normalizeList(input.Expertise, maxShortItems, maxShortItemCharacters); err != nil {
		return Definition{}, "", invalidDefinition("expertise")
	}
	if out.Opinions, err = normalizeList(input.Opinions, maxShortItems, maxOpinionCharacters); err != nil {
	...
	if out.Boundaries, err = normalizeList(input.Boundaries, maxShortItems, maxBoundaryCharacters); err != nil {
		return Definition{}, "", invalidDefinition("boundaries")
	}
	if out.ForbiddenPhrases, err = normalizeList(input.ForbiddenPhrases, maxShortItems, maxShortItemCharacters); err != nil {
		return Definition{}, "", invalidDefinition("forbidden_phrases")
	}
	if out.DislikedPatterns, err = normalizeList(input.DislikedPatterns, maxShortItems, maxBoundaryCharacters); err != nil {
...
func normalizeList(values []string, maxItems, maxCharacters int) ([]string, error) {
