...
// Ported from FreeCut (MIT), adapted to OpenPost's pinned model store.
...
const MODEL_STORE_KEY = "openpost-moss-tts-f52645-ceff0d";
...
function ensureTokenizerWorker() {
  ...
  tokenizerWorker.addEventListener("message", (event) => {
    ...
    if (
      !payload ||
      payload.source !== TOKENIZER_SOURCE ||
  ...
  tokenizerWorker.addEventListener("error", () => {
    const error = new Error("MOSS tokenizer worker failed.");
...
self.__openpostMossTokenizerBridge = postTokenizerRequest;
...
async function getRuntime({ requestId, threadCount = 4, warmup = false }) {
  ...
  if (warmup && !runtime.__openpostWarmedUp) {
    postToMain({
      type: "progress",
      requestId,
    ...
    runtime.__openpostWarmedUp = true;
  }
  ...
  postToMain(
    {
      ...
      data: {
        status: "completed",
        textChunkCount: Array.isArray(result?.textChunks)
          ? result.textChunks.length
          : 0,
