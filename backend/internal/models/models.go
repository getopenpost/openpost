...
// ProviderUsageEvent is an immutable estimate of a hosted provider request
// that can incur a variable cost. OperationKey makes one recording attempt
// idempotent without retaining post text, provider responses, or credentials.
...
// ProviderUsagePeriodCounter is a monthly projection of immutable provider
// usage events. It can be rebuilt without changing the event ledger.
...
// VideoDerivative records an immutable rendered variant of a source video.
// RecipeHash makes identical requests idempotent while OutputMediaID keeps the
// rendered bytes in the normal media lifecycle and storage abstraction.
type VideoDerivative struct {
	bun.BaseModel `bun:"table:video_derivatives"`

	ID            string    `bun:",pk" json:"id"`
	WorkspaceID   string    `bun:"workspace_id,notnull" json:"workspace_id"`
	SourceMediaID string    `bun:"source_media_id,notnull" json:"source_media_id"`
	OutputMediaID string    `bun:"output_media_id,nullzero" json:"output_media_id,omitempty"`
	Provider      string    `bun:"provider,notnull,default:''" json:"provider"`
	Profile       string    `bun:"profile,notnull,default:''" json:"profile"`
	Preset        string    `bun:"preset,notnull,default:'source'" json:"preset"`
	RecipeJSON    string    `bun:"recipe_json,notnull,default:'{}'" json:"recipe_json"`
	RecipeHash    string    `bun:"recipe_hash,notnull" json:"recipe_hash"`
	Status        string    `bun:"status,notnull,default:'pending'" json:"status"`
	Progress      int       `bun:"progress,notnull,default:0" json:"progress"`
	Error         string    `bun:"error,notnull,default:''" json:"error,omitempty"`
	CreatedAt     time.Time `bun:",nullzero,notnull,default:current_timestamp" json:"created_at"`
	UpdatedAt     time.Time `bun:",nullzero,notnull,default:current_timestamp" json:"updated_at"`
}
