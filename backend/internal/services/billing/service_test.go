...
func newBillingTestDB(t *testing.T) *bun.DB {
	...
	_, err = db.ExecContext(context.Background(), `CREATE TABLE IF NOT EXISTS voice_profiles (
		id TEXT PRIMARY KEY,
	...
	require.NoError(t, err)
	_, err = db.ExecContext(context.Background(), `CREATE UNIQUE INDEX IF NOT EXISTS voice_profiles_billing_default_idx
		ON voice_profiles (workspace_id) WHERE is_default = true`)
	require.NoError(t, err)
