<script lang="ts">
	import { FlaskConical, ShieldAlert } from 'lucide-svelte';
		...
		<div class="grid gap-10 lg:grid-cols-[1fr_22rem] lg:items-end">
			<div class="max-w-4xl">
				...
				<p class="mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">
					Every network has different formats, permissions, and media rules. OpenPost documents
					them per platform and keeps integrations that still need provider approval in Preview.
				</p>
			...
			<dl class="grid grid-cols-2 gap-px overflow-hidden rounded-xl border bg-border">
				<div class="bg-card p-5">
					...
					<dd class="mt-2 text-3xl font-semibold">{availablePlatforms.length}</dd>
				</div>
				<div class="bg-card p-5">
					<dt class="text-sm text-muted-foreground">Preview</dt>
					<dd class="mt-2 text-3xl font-semibold">{previewPlatforms.length}</dd>
				</div>
			</dl>
		</div>
	</div>
		...
		<div class="grid gap-8 lg:grid-cols-[16rem_1fr]">
			<div>
				...
				<h2 class="mt-4 text-3xl leading-tight font-semibold text-balance">
					Needs provider access and live verification
				</h2>
				<p class="mt-4 text-sm leading-6 text-muted-foreground">
					These adapters have code and capability profiles. Provider review, public-media setup,
					or a live-account audit still blocks a general availability claim.
				</p>
			</div>
			<div class="grid gap-4 md:grid-cols-2">
				{#each previewPlatforms as platform (platform.slug)}
					<a href={`/platforms/${platform.slug}`} class="group rounded-xl border bg-card p-5 transition hover:bg-background">
						<div class="flex items-start justify-between gap-4">
							<div class="flex items-center gap-3">
								<PlatformIcon platform={platform.short} class="size-5" />
								<h3 class="font-semibold">{platform.name}</h3>
							</div>
							<span class="rounded-full border border-amber-500/30 bg-amber-500/10 px-2 py-1 text-[0.7rem] font-medium text-amber-400">
								Preview
							</span>
						</div>
						<p class="mt-4 text-sm leading-6 text-muted-foreground">{platform.description}</p>
