<script lang="ts">
	...
	function panelMaximum(panel: 'assets' | 'inspector'): number {
		...
		const available =
			window.innerWidth - DESKTOP_TOOL_RAIL_WIDTH - MINIMUM_CANVAS_WIDTH - otherPanelWidth;
		return Math.max(panel === 'assets' ? 220 : 280, Math.min(panel === 'assets' ? 420 : 480, available));
	}
...
<style>
	...
	@media (min-width: 64rem) {
		...
		.studio-workspace[data-inspector='false'] {
			grid-template-columns: var(--studio-assets-width) minmax(0, 1fr);
		}
