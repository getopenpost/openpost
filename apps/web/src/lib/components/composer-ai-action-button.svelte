<script lang="ts">
	...
	const activeWidth = $derived(hasText ? buildWidth : ideateWidth);
</script>
...
>
	...
	{#if building}
		<span
			class="absolute inset-0 flex min-w-0 items-center justify-center gap-1.5 px-2.5"
			aria-hidden="true"
		>
			<ProtectedIcon
				icon="loading"
			...
			/>
			<span class="truncate">{buildingLabel}</span>
		</span>
	{/if}
</Button>
