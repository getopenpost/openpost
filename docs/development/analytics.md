...
OpenPost only tracks content published through OpenPost. There is no inventory for content published elsewhere: no provider history reads, no discovery jobs, and no external analytics sources. The publishing `platform.Adapter` remains unchanged. `platform.CommentAdapter`, `platform.EngagementAdapter`, and `platform.MessagingAdapter` stay separate from analytics. Each read path has its own access checks, cursors, data limits, and audit records.
...
Snapshots include workspace and provider identity for scoped queries and deletion. Raw provider payloads, remote media, access tokens, bot credentials, and webhook secrets are intentionally discarded.
...
Account checks start on a daily cadence. Rendition checks use 1, 3, 12, and 24 hour bands until day 7. Repeated unchanged results can increase the current interval to 8× its base cadence. Stored backoff and `Retry-After` apply to scheduled and manual refresh alike.
...
`GET /api/v1/analytics` reads stored data only. The account, sort, range, and offset are bound into the opaque next cursor. It returns flat content references for OpenPost publications, metric metadata, and server-owned insights calculated before content pagination. Account growth remains explicitly account-wide. Page reads never call a provider.
