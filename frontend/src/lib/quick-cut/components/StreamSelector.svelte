<script lang="ts">
	...
	let {
		...
		onChange: (patch: Pick<QuickCutSource, 'selectedVideoTrackIndex' | 'selectedAudioTrackIndices'>) => void;
	} = $props();
	...
	const videoOptions = $derived.by(() => {
		...
		for (const vs of source.videoStreams ?? []) {
			...
			opts.push({ value: String(vs.index), label: `${m.quick_cut_stream_video_track({ index: vs.index + 1 })} · ${label}` });
		}
	...
	const audioOptions = $derived.by(() => {
		...
		for (const aud of source.audioStreams ?? []) {
			...
			opts.push({ value: String(aud.index), label: `${m.quick_cut_stream_audio_track({ index: aud.index + 1 })} · ${label}` });
		}
	...
	function onVideoChange(value: string): void {
		if (value === '__off__') onChange({ selectedVideoTrackIndex: null, selectedAudioTrackIndices: source.selectedAudioTrackIndices });
		else onChange({ selectedVideoTrackIndex: Number(value), selectedAudioTrackIndices: source.selectedAudioTrackIndices });
	}

	function onAudioChange(value: string): void {
		if (value === '__off__') onChange({ selectedVideoTrackIndex: source.selectedVideoTrackIndex, selectedAudioTrackIndices: [] });
		else onChange({ selectedVideoTrackIndex: source.selectedVideoTrackIndex, selectedAudioTrackIndices: [Number(value)] });
	}
</script>
