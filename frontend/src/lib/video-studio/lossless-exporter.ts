...
export async function exportQuickCutLosslessly(
	...
	try {
		...
		const format = selectOutputFormat(videoCodec, audioCodec, source.mime_type);
		...
		if (audioSource) {
			output.addAudioTrack(audioSource, {
				maximumPacketCount: audioPacketStats?.packetCount
			});
		}
...
function selectOutputFormat(
	...
	const formats: OutputFormat[] = sourceMimeType.toLowerCase().includes('webm')
		? [webm, mp4]
		: [mp4, webm];
