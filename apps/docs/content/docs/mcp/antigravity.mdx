...
Follow [the local bridge setup](/mcp/mcp-guide/self-hosted-and-local) to install `openpost-mcp` and sign in with the `local` profile. Use your Hosted or self-hosted origin during login.
