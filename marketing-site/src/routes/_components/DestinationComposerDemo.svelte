<script lang="ts">
  ...
  function updateCopy(event: Event) {
    destinationCopy[selectedPlatform] = (
  ...
  <div class="grid lg:grid-cols-[0.78fr_1.22fr]">
    ...
    >
      <div class="flex items-start justify-between gap-4">
        ...
        <span
          class="rounded-md bg-background px-2 py-1 text-xs text-muted-foreground"
        >
      ...
      <div class="mt-4 flex flex-wrap gap-2">
        ...
        >
          Platform settings stay with this account version.
        </span>
      </div>
    </section>
    ...
    >
      <div class="grid w-full place-items-center gap-3">
        <div class="flex w-full max-w-2xl items-center justify-between gap-3">
          <h2 id="destination-result-title" class="text-sm font-semibold">
            What people will see
          </h2>
          <span class="text-xs text-muted-foreground"
            >Preview based on the platform layout</span
          >
        </div>
