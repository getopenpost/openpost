...
Add this object to your existing `OPENPOST_PROVIDER_APPS` JSON array. This complete array works for a new installation; append the object without removing your other providers if the array already exists.
...
[
  {
    ...
    "redirect_uri": "https://your-domain.example/api/v1/accounts/x/callback"
  }
]
```

Keep the JSON in a deployment secret store, preferably a file referenced by `OPENPOST_PROVIDER_APPS_FILE`. An instance administrator may instead use **Settings → Instance → Configuration → Provider apps** to save an encrypted app. An environment entry overrides a matching saved row. Restart OpenPost after saving. See [self-hosted configuration](/self-hosting/configuration).
